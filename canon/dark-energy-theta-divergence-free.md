...
- **F6 — Canon-level error in the original C1+C2 path description (Noether's first theorem is the wrong theorem).** CORRECTION logged 2026-06-23. The original description of the C1+C2 path in this file implicitly invoked Noether's first theorem (global/rigid symmetry → on-shell conserved current). This is the wrong theorem for deriving D_A* theta = 0 from gauge invariance. The correct theorem is Noether's SECOND theorem (local/gauge symmetry → off-shell identity D_A* E_A = 0). The C1+C2 path using Noether's first theorem cannot derive D_A* theta = 0 without additionally identifying theta as the Noether current J ~ D_A* F_A, which requires Assumption 3 (the same gap as the C3 path). The C1+C2 path using Noether's second theorem IS valid (it gives D_A* E_A = 0 off-shell from local gauge invariance of S[A]) but collapses into the C3 path — it is not an independent route. There is no C1+C2 path that is both correct and independent of the C3 path. The C3 path (Noether's second theorem) is the only structurally sound route, and it remains conditional on Assumption 3. Source: `explorations/dark-energy-cosmology/dark-energy-c1-c2-path-gimmel-ginvariance-2026-06-23.md` §§ "Clarification on Noether's First vs. Second Theorem" and "C2 — Blocking Failure Condition."
