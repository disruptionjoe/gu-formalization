...
> **CASCADE NOTE — CORRECTION W2-01 (2026-06-26), re-evaluation pending.** This section's reasoning leans on a now-RETRACTED premise: that `canon/w2-y14-spin-structure.md` proves Y14 spin *for any orientable X⁴* (instancing the non-spin CP2). W2-01 corrected that result to **w2(Y14) = pi*w2(X⁴), i.e. Y14 spin iff X⁴ spin** — so for a non-spin base (CP2) Y14 is NOT spin and `D_gimmel` is not defined without extra structure. Consequence: the generic-X⁴ entries do NOT enjoy an unconditional Dirac operator, and the "do not globally fix K3 because w2 holds generically" argument below no longer follows as stated. The CONDITIONALLY_EVADED/lane-narrowed verdicts that cite the generic-X⁴ spin result must be re-checked under new failure condition **W2-FC1** (does the Witten class-exit and the no-import-K3 argument survive once X⁴ is required spin, or must GU restrict to spin bases?). Deliberately NOT rewritten in the W2-01 pass, per the same-session/independent-pass rule; treat the "spin for any orientable X⁴ / CP2" citations in this section as retracted until W2-FC1 closes.
