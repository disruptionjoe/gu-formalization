...
3. **Cross-chirality Krein signature.** The `so(p,q)`-invariant form on the generation triplet is purely
   cross-chirality, with signature exactly `(+96, -96)`; the mod-2 content is the EVEN/balanced split
   (net chirality `0`), NOT the number 96 itself (`96 = 2^5 * 3` is not a power of two -- the 3 is a
   multiplicand, per the 2026-06-28 correction below; wording fixed 2026-08-03, audit HB-08). [2-primary]
...
> verified). [CORRECTED 2026-08-03, audit HB-05: the counts below were 5-and-2; the Rokhlin rung is
> RETYPED. Rokhlin's theorem lives in `Omega^Spin_4 = Z` (generated by K3; detected by `sigma/16`),
> NOT in `Omega^{Pin+}_4 = Z/16` -- that is a true group fact (Kirby-Taylor) but a DIFFERENT group
> carrying a DIFFERENT theorem (the two 16s are related via Guillou-Marin characteristic-surface
> arithmetic, not equal). Under the ladder's own typing rule the Rokhlin rung is a free-integer-index
> rung (a 2-power DIVISIBILITY of the Z-valued signature), so the honest count is 4 structural + 3
> free-integer. The 2-primarity of the Rokhlin statement itself (a mod-2^4 congruence) is UNAFFECTED,
> and item 5 of the enumeration above stands as written.]** (a) EMPIRICAL -> PARTIALLY STRUCTURAL:
> 4 of the 7 enumerated obstructions are proven-2-primary BY their KO/Spin/Pin TYPE from cited primary
> sources (Bott KO-torsion `Z/2`; ABS Clifford rank `2^floor(m/2)`; the Kramers and ghost-parity `Z/2`
> parities); 3 (Krein signature, adjoint index, Rokhlin/signature divisibility) are torsion-free
> integer indices, blindness contingent on the torsion-count reading. The
