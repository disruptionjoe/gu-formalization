...
**THIRD confirmation (deep-research report, 2026-06-28; `lab/deep-research/gu-firewall-tangential-fork-deepresearch-2026-06-28.md`).** A deep-research report -- the first to cite the GU draft directly and to compute BOTH branches explicitly -- reaches the same tangential verdict on stronger evidence:
...
## 7. Correction (hardening audit, 2026-06-28): RP^3 is the SPINE, not the link

A hardening audit (`lab/deep-research/hardening-report-batch-2026-06-28.md`, prompt 4) both forecloses the worst alternative and corrects a dimension error in our phrasing:
- **Hardening.** The metric fiber `GL(4,R)/O(3,1)` deformation-retracts to `RP^3 = L(2;1)` (Mostow/Cartan; the `Z_2` is the `O(1)` Lorentz time-reflection), and there is NO `Z_3` anywhere in `O(4)/(O(3) x O(1))`, so `L(3;1)` and every 3-divisible lens are geometrically UNREACHABLE. The most dangerous alternative is foreclosed.
- **Hardening.** The charge-`q` Dirac eta on `L(2;1)` is `(2q^2 - 4q + 1)/8` -- 2-primary for EVERY integer charge. So the manifold itself can never supply the order-3 element; the ENTIRE 3-primary burden sits in the tangential FRAMING channel `-p_1/24`, exactly as claimed (this strengthens the "it is the framing, not the spectrum" point).
- **CORRECTION (dimension/category error in our phrasing).** "The link of the non-compact end of `Y^14` is `RP^3`" is WRONG. The true fiber link is a 9-dim `S^6`-bundle over `RP^3`, and `Y`'s link is 13-dimensional. `RP^3 = L(2;1)` is the parallelizable deformation-retract **SPINE** of the metric fiber, used as a framed 3-manifold. The result must be carried on the **framed-bordism / J-homomorphism footing** (Route C), NOT as an APS eta-defect of a literal boundary 3-manifold (Routes A/B lose geometric ground). Restate any "the link is RP^3" claim as "the spine of the metric fiber is RP^3, carried on the framed-bordism footing."
- **CORRECTION.** The `RP^3` retract requires the INDEFINITE (Lorentzian) base signature; a Riemannian base gives a contractible cone with no `RP^3`. The non-compactness is load-bearing, and the whole end/twist apparatus is reconstruction-grade (absent from GU's published draft).