...
**6. Schwarzschild as a genuine open question. [Genuine open obstruction]**

Schwarzschild is NOT a critical section of the Willmore energy E[s] = ∫|II_s|². Three independent lines of evidence establish this at reconstruction grade: (i) the mean curvature H^i_{Schw} ~ M/r² does not satisfy the Willmore Euler-Lagrange equation Δ^⊥H^i = 0 (explicit computation: Δ(M/r²) = 2M/r⁴ ≠ 0 at linear order in M); (ii) the Willmore functional is conformally invariant in 4D and its critical sections are conformally flat metrics, but Schwarzschild is not conformally flat (Weyl tensor non-zero); (iii) the traceless second fundamental form B̂^i_{μν} ≠ 0 for Schwarzschild (the t-r sign flip in B^{(tr)}_{μν}/g_{μν} is explicit), so Q^{TF}(B) ≠ 0 and the section equation is not satisfied. The failure tensor R_fail^{Schw}_{μν} is of order O(M/r³) — proportional to the Schwarzschild Weyl tensor components — and does not vanish.

This means GU as currently formulated (action E[s] + Yang-Mills for A) reproduces conformally flat vacuum solutions (flat, de Sitter, FLRW, pp-waves, linearized gravity) but does not automatically reproduce Schwarzschild or Kerr as vacuum critical sections.

Two paths could resolve this obstruction. **(a) Recovery via the distortion field equations:** The full GU system couples the section equation to the connection equation D_A*F_A = II_s^H. It is conceivable that the connection equation conspires with the section equation so that Schwarzschild is an on-shell solution of the FULL coupled system even though it is not a critical section of E[s] alone. This requires deriving the full coupled EL system and checking whether Schwarzschild arises as a solution. If it does, R_fail = 0 is recovered and this obstruction closes. **(b) Modified variational principle:** A term added to E[s] — for example, a coupling to the traceless extrinsic curvature |B̂|² or the Yang-Mills curvature of the Sp(64) connection — could shift the critical-section condition so that Schwarzschild is selected. This would require a modification of Weinstein's action; whether a natural such term exists in the GU geometry is open.

This is a genuine structural gap in GU's physical scope, not a bookkeeping issue. It is honest to state it alongside the Freed-Hopkins obstruction as an unresolved constraint the program must face. It is not fatal — the program reproduces cosmological solutions correctly, and the weak-field (linearized) Schwarzschild is recovered at leading order in M/r. But full nonlinear Schwarzschild vacuum recovery is open. *Technical source: `explorations/geometry-curvature-emergence/rfail-non-umbilic-schwarzschild-2026-06-23.md`.*
...
*This is version 7 of this report. Version 6 is archived at [v6](what-geometric-unity-needs-to-do-next-v6.md). Technical sources for results new in v7: generation-count synthesis (`explorations/generation-sector/n5-generation-count-synthesis-2026-06-23.md`), T⁴ vs K3 disambiguation (`explorations/generation-sector/oq3a-t4-vs-k3-disambiguation-2026-06-23.md`), OQ3b tau-correction closure (`explorations/representation-theory-noncompact/oq3b-tau-correction-closure-2026-06-23.md`), cross-term cancellation (`explorations/generation-sector/oq3c-cross-term-cancellation-2026-06-23.md`), split-rank verification (`explorations/representation-theory-noncompact/oq1-split-rank-verification-2026-06-23.md`), Freed-Hopkins non-forgettable observer (`explorations/time-as-finality-crosswalk/freed-hopkins-nonforgettable-observer-2026-06-23.md`), Distler-Garibaldi precision carve-out (`explorations/cycle-gates-and-audits/distler-garibaldi-precision-carveout-2026-06-23.md`), Type II₁ KO-dimension (`explorations/type-ii1-spectral/type-ii1-ko-dimension-2026-06-23.md`), Schwarzschild non-umbilic obstruction (`explorations/geometry-curvature-emergence/rfail-non-umbilic-schwarzschild-2026-06-23.md`). Prior sources listed in v6.*
