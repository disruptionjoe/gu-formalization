...
The repository hosts the 35 persona passes (grouped in `lab/process/persona-passes/` by lens family with `lab/process/persona-passes/INDEX.md` for navigation), 5 + 2 syntheses in `syntheses/`, and 4 deep-research briefs in `lab/literature/`. Contribution guidelines are documented in `CONTRIBUTING.md` at the repository root.
