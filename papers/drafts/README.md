---
title: "Papers"
status: index
doc_type: overview
updated_at: "2026-06-29"
---

# Papers (work in progress)

Drafts, notes, and **previous/superseded versions**. Nothing here is a public claim. Edit freely.

Papers that have graduated for publication live in [`../candidates/`](../candidates/); see the three-stage
lifecycle in [`../README.md`](../README.md).

## Graduated to candidates (no longer here)

- **Located, Not Forced** — the lead generation-count result. Now at
  `../candidates/located-not-forced/` (its `.tex` is the arXiv source). Earlier versions of
  this line stay here as drafts (e.g. `two-primary-no-go-three-primary-boundary-class-2026-06-28.md`).
- **Six-Axis Testability** — the methods/position white paper. Now at
  `../candidates/six-axis-testability/`.

## Still in progress here (selected)

- [`blog-post-draft-v2.md`](blog-post-draft-v2.md) — shorter public exposition draft.
- [`generation-multiplicity-vs-chirality-2026-06-28.md`](generation-multiplicity-vs-chirality-2026-06-28.md)
  — near-duplicate of "Located, Not Forced"; its separable math.RT kernel could become a short companion note
  later (citing the lead paper).
- Superseded predecessors and notes (two-primary-no-go, multiplicity-theorem-note, generation-sector-no-go-note,
  canonical-structures-14d, vz-evasion-preprint, formal-paper-draft-v2, no-go-class-relative-survey) are kept
  for provenance; several are self-flagged historical or superseded. See each file's header.

These are working drafts, not established physics.
