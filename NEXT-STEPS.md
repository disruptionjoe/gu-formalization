...
|---:|---|---|---|
| F1 | **Mixed-covector 14D VZ Schur complement** — extend `explorations/vz1-schur-complement-symbol-2026-06-23.md` and `explorations/vz1-schur-vertical-extension-2026-06-23.md` from horizontal covectors to mixed `xi = xi_H + xi_N`, and decide whether the physical RS projection is horizontal gamma-trace or full 14D gamma-trace. | Full mixed-covector `S_R^14D(xi) = A - B E^{-1} C` computation, or a proof that `E` is singular and a constrained-symbol analysis is required. | VZ1 remains OPEN; horizontal propagation is favorable, but mixed 14D covectors can still reintroduce VZ-type characteristics. |
| F2 | **Choose the `II_s` convention and compute the Hessian on `S^4`** — decide between literal graph immersion and horizontal-normalized pullback/reference subtraction, then linearize `E[s] = integral |II_s|^2`. | Invariant coefficient `C_GU` for `Lambda_GU = C_GU epsilon_sec^2 / t_obs^2`. | The cross-program Lambda comparison remains convention-level rather than geometric. |
| F3 | **Evaluate `K(A,s)` and `R_fail` for the selected section** — use `explorations/codazzi-sp64-bundle-2026-06-23.md` to test whether the normal-flux correction vanishes or equals a recognized stress-energy divergence. | Pass/fail note for 4D Einstein recovery: `K(A,s)=0`/recognized source and `R_fail=0`, or a concrete obstruction. | The 4D reduction cannot claim Einstein recovery from the 14D field equation. |
| ~~F4~~ **CONDITIONALLY_RESOLVED 2026-06-23** | **Replace the N5 analytic target** — the correct invariant is the relative-discrete-series Plancherel multiplicity `m_H(S(6,4))` in `L2(SL(4,R) x_{SO_0(3,1)} S(6,4))`. The Flensted-Jensen equal-rank condition is satisfied (rank 1 = rank 1), so relative discrete series exist for `SL(4,R)/SO_0(3,1)`. The prior pessimistic rank reading (3 != 1) used the wrong rank definition; the correct split-rank is `min(p,q) = min(3,1) = 1`. Generation count is analytically confirmed iff `m_H(S(6,4)) = 24`. Remaining open: branching rules for the 9-dim isotropy embedding of `Spin(3,1)` into `Spin(6,4)` and the explicit value of `m_H`. See `explorations/n5-discrete-series-gl4r-2026-06-23.md`. | Precise invariant defined; Flensted-Jensen equal-rank passes; `m_H(S(6,4)) = 24` is the new concrete open target. | The conditional three-generation result now has a coherent analytic counter — the relative-discrete-series multiplicity. |
| F5 | **Observer-section error model** — determine whether GU section precision `epsilon_sec` and TaF decoherence tolerance `epsilon_dec` can be related by a real observer-section model. | Tolerance bridge, or proof that the Lambda/TaF contact is only shared `t_obs` scaling. | The cross-program coefficient result stays dimensional/proportional, not structurally coupled. |
