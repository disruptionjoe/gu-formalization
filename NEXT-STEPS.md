...
| N4 | **IG dimension matching** — verify whether the inhomogeneous gauge group IG = Sp(64) ⋉ Omega^1(sp(64)) provides the correct algebra-dimension structure for the shiab and tau+ homomorphism. Specifically: dim sp(64) = 8256 is the Lie algebra dimension; Nguyen's argument required 16384 = dim u(128) for the algebra-dimension matching. Determine whether (a) 8256 is in fact the correct dimension for the (9,5) IG construction, (b) the tau+ homomorphism selects a specific larger subgroup G' ⊂ Sp(64) such that dim g' = 16384, or (c) the algebra-dimension argument from Nguyen's §2 simply does not transfer to the quaternionic (9,5) setting. | Clear verdict on the IG dimension matching: resolved, or named as the specific surviving form of Nguyen §2's residual. | This is the residual blocking item from the anomaly dissolution. Without it, Nguyen §2 has a surviving question even after Sp(64) replaces U(128). |
| N5 | **Generation count — two bounded computations** — (a) confirm the L^2 index of D_GU counts H-lines (quaternionic zero modes, dim_H = 3) not R-lines (which would give 4+ generations naively from S^+ = H^{32}). (b) Compute the branching rule RS(3,1) ⊗ S(6,4) under SU(3) × SU(2) × U(1) and verify it decomposes as exactly 16 Weyl fermions (one SM generation). | Clear verdict on the 3-generation claim: confirmed if both computations pass; specific correction if either fails. | Until (a) and (b) are computed, the generation count is conditionally 3 — the structural mechanism is intact but the factor-of-2 from the quaternionic spinor module has not been resolved. |
