...
| ~~HC1~~ **CONDITIONALLY RESOLVED 2026-06-22** | **Three hidden curvature components: representation theory check** — **Verdict: CONDITIONALLY_RESOLVED.** Group is SO(1,3) acting on the curvature 2-form F ∈ Ω²(X⁴, so(1,3)) in Poincaré gauge theory (not conformal SO(2,4), not Spin(9,5) on Y¹⁴). The 3+3=6 count is confirmed under the torsion-sourcing reading: 3 visible (W, S₀, R) + 3 torsion-activated hidden pieces (H^{(1)}, H^{(2)}, H^{(3)}) sourced by the 3 irreducible torsion pieces (T^{(1)}: traceless tensor 16 comp.; T^{(2)}: trace-vector 4 comp.; T^{(3)}: axial-vector 4 comp.) via DT = R∧e. SL(2,C) labels: H^{(1)} ≈ (1,1)_antisym (dim ~9), H^{(2)} ≈ (1/2,1/2)_vector (dim 4), H^{(3)} ≈ (1/2,1/2)_axial (dim 4). The qualifier "large enough" means: use full SO(1,3) to keep T^{(2)} and T^{(3)} as distinct parity-separated irreducibles. **Conditional on:** SL(2,C) labels need explicit Bianchi-map spinor computation to verify; GU's distortion θ (with IG-equivariance) may decompose differently from standard torsion T^{(1)},T^{(2)},T^{(3)} (the primary residual). | See `explorations/hc1-hidden-curvature-components-2026-06-22.md`. | SL(2,C) labels need explicit Bianchi-map verification; θ vs. standard torsion decomposition is main residual. |
