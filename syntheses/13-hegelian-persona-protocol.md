...
**Status.** Public draft artifact. Generated 2026-05-30 as the single-pass output of WRK-381. Public path: `syntheses/13-hegelian-persona-protocol.md`.

**What this is.** A method-note artifact. It describes the dialectical multi-persona protocol that produced the 35 persona passes and 8 syntheses currently in `gu-formalization/passes/` and `gu-formalization/syntheses/`. It is not a math result. Its claim is procedural: the dialectical structure used in WRK-326 is reproducible by other researchers on other open problems, with named preconditions and named failure modes.
...
The proof of generation is on disk: the 35 passes and 8 syntheses listed in `gu-formalization/passes/INDEX.md` and `gu-formalization/syntheses/INDEX.md` are the actual artifacts the protocol produced, and `WRK-375` through `WRK-381` are the bounded follow-on passes the protocol identified.
...
- Source artifacts (all in `gu-formalization/`):
  - `passes/INDEX.md` and the 35 persona passes themselves (01-35 across five lens families).
  - `syntheses/INDEX.md` and the 8 syntheses (00, 00b, 00c, 00d, 00e, 06, 07, 08).
  - `syntheses/00c-hegelian-meta-synthesis.md` — the load-bearing Aufhebung step.
  - `syntheses/00e-problem-shape-distributed-systems-meta-synthesis.md` — the six-axis specification derivation.
  - `syntheses/08-supplementary-15-persona-pathway-ranking.md` — the persona-voted pathway ranking that scoped this card.
...
- **Class B — Forgetful-image evidence base.** `WRK-376` (no-go forgetful-image map) at `syntheses/10-no-go-forgetful-image-map.md`. The per-theorem map is the falsification surface for the Stage 4 Aufhebung.
- **Class C — Construction checklists and pilot analogies.** `WRK-377` (Type II_1 spectral SM checklist) at `specifications/type-ii1-spectral-sm/` and `WRK-378` (Nielsen-Ninomiya protocol analogy pilot) at `syntheses/12-nielsen-protocol-analogy-pilot.md`. These are the highest-ranked surviving substrate-class candidates from the antithesis and problem-shape stages.
