...
**Source basis.** `deep-research/03-assumption-decomposition-no-go-evasion-literature.md`, `syntheses/00b-loophole-synthesis-witten-evasion-test.md`, `syntheses/06-supplementary-insights-novelty-and-tests.md`, `syntheses/08-supplementary-15-persona-pathway-ranking.md`. Cross-built against the six-axis protocol (WRK-375, `specifications/six-axis/six-axis-template.md`).
...
Successful published evasions are uniformly **geometric** class exits. No published evasion uses stochastic, observer-relative, causal-set, or hypergraph language (see Witten evasion table in `syntheses/06`, I12).
...
**Where the analogy may break.** A *non-geometric* substrate (stochastic, observer-relative, causal-set) is not yet shown to admit a smoothing functor producing exactly the Witten image. This is an open derivation problem; the formal opening exists (`syntheses/00b`) but the substantive derivation does not.
...
**Analogy strength.** Medium. For the *adjacent* enrichments (crystalline, mixed, average) the analogy is essentially what the literature is already doing — they are documented extensions inside the same paradigm. For the *original* enrichment (observer/QRF pairing) the analogy is a research proposal, not an established result. Per `deep-research/02` and I7/I8: no published or arXiv-standard literature treats Freed-Hopkins in observer-relative / QRF / QBist / superdeterministic / IIT terms. This is a genuinely open extension proposal.
...
Per `deep-research/03` and I11: in all four families, successful published evasions add **hidden bulk, boundary, singular, bundle, or higher-symmetry structure**. The smooth-bundle shadow loses exactly that added structure.
...
- The map cites the literature in `deep-research/03` but does not independently verify each citation. The map's correctness about the no-go theorems is downstream of that brief's correctness.
