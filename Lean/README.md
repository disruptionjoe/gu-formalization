...
| `GUFormalization/GroupActionMackey.lean` | First set-level Mackey interface for subgroup induction: the `K`-orbit quotient of `Res_K^G Ind_H^G(1)` is explicitly equivalent to `K \\ G / H`, and the stabilizer of `[g,star]` is exactly the transported intersection condition `k*g = g*h`. The result is the trivial-seed orbit/stabilizer layer, not yet the full nontrivial-seed summand decomposition | `VERIFICATION.md` |
