...
| `GUFormalization/GroupActionInduction.lean` | The complementary set-level left adjoint: `Ind_phi B` is the orbit quotient `G ×_H B` with explicit descended left action, and `Hom_G(Ind B,C) ~= Hom_H(B,Res C)`. Quotient orientation and all actions are explicit and non-global | `VERIFICATION.md` |
...
| `GUFormalization/GroupActionMackey.lean` | Set-level Mackey interfaces for subgroup induction: the `K`-orbit quotient of `Res_K^G Ind_H^G(1)` is explicitly equivalent to `K \\ G / H`, with stabilizer `k*g = g*h`; for an arbitrary supplied `H`-set `B`, the transported action `k ↦ g⁻¹kg` gives a `K`-equivariant equivalence from `K ×_(K ∩ gHg⁻¹) {}^gB` to the concrete double-coset summand in `Res_K^G Ind_H^G(B)` | `VERIFICATION.md` |
