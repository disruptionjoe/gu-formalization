...
| `GUFormalization/GroupActionFixedPoints.lean` | Set-level group-action classification: pointwise-invariant valuations are functions into the common fixed-point subtype and have finite cardinality `|Fix(G)|^|A|`; for a transitive acted-on domain, evaluation at a basepoint identifies equivariant maps with the codomain values fixed by that point's stabilizer, giving the exact finite census `|Eqv(A,B)| = |B^Stab(a₀)|`. The regular left `G`-torsor is the free special case with count `|B|`. No physical action or GU verdict is encoded | `VERIFICATION.md` |
