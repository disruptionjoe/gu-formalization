...
| `GUFormalization/GroupActionMackey.lean` | Complete set-level Mackey decomposition for subgroup induction: the `K`-orbit quotient of `Res_K^G Ind_H^G(1)` is `K \\ G / H`; each supplied `H`-set summand is `K ×_(K ∩ gHg⁻¹) {}^gB`; and choosing one representative per double coset yields a `K`-equivariant equivalence from their dependent coproduct to `Res_K^G Ind_H^G(B)`. Representative choice and all actions are explicit | `VERIFICATION.md` |
