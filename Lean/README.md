...
| `GUFormalization/GroupActionFixedPoints.lean` | Set-level group-action classification: pointwise invariance iff valuation-range containment in the common fixed-point set, plus empty-fixed-set and fixed-point-free-element no-invariant corollaries; no physical action or GU verdict is encoded | `VERIFICATION.md` |
