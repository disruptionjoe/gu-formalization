...
| `GUFormalization/GroupActionFixedPoints.lean` | Set-level group-action classification: pointwise-invariant valuations are functions into the common fixed-point subtype and have finite cardinality `|Fix(G)|^|A|`; separately, equivariant maps from the regular left `G`-torsor are equivalent to all codomain values and have cardinality `|B|`. This explicitly separates pointwise invariance from acted-on-domain equivariance. No physical action or GU verdict is encoded | `VERIFICATION.md` |
