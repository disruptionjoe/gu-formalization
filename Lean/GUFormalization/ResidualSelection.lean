...
- **GU instance** (observer-value-selection paper, `papers/candidates/
  observer-value-selection-theorem/`): Lawvere no-closure (I-a) and the
  no-invariant-valuation result (I-b) for a two-element grading with the
...
- The lemma itself is CLASSICAL (Lawvere 1969 / Yanofsky 2003; with
  ...
  discharging both the physics-selection instance and the issuance
  instance.
...
/-- **Residual-Selection Lemma (residual corollary; the fixed-point-count
leg, Lemma C of the paper).** If `α` is fixpoint-free and `A` is inhabited,
no total valuation `p : A → B` is `α`-invariant: every committed valuation
breaks the grading symmetry. -/
...
/-- `Bool` negation is fixpoint-free — the sole load-bearing hypothesis
(A3 of the paper) discharged by a kernel-decided fact about `Bool`. -/
theorem not_fixpoint_free (b : Bool) : (!b) ≠ b := by
  cases b <;> decide

/-! ## Corollary GU: the observer-value-selection theorem
(`B = Bool` two-element grading, `α = not` the swap involution).
These are the paper's (I-a) and (I-b), machine-checked. -/

section GU

variable {A : Type}

/-- **(I-a), concrete residual form.** For every candidate closure
...
/-- **(I-a) No closure.** No weakly point-surjective `T : A × A → Bool`
...
/-- **(I-b) No invariant valuation.** For inhabited `A`, no total
valuation is `α`-invariant: every valuation the observer commits to breaks
the grading symmetry. (Group-theoretic reading: with `G = ⟨α⟩ ≅ Z/2`,
every total valuation is value-type, none is arena-type.) -/
