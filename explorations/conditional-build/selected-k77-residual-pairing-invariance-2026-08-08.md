# Selected K77 residual pairing: conditional local construction

Status: exact local construction; no canon, verdict, or public-posture change.

## Plain-English result

The selected K77 source action says to take a norm square of the residual
`Upsilon`, but the public source does not specify the real K77 pairing that
makes that phrase an action.  This wave constructed the smallest local
candidate compatible with the already-live K77 objects: the degree-thirteen
...
| Object | Meaning here |
|---|---|
| Source “norm square” | A quadratic-action instruction for `Upsilon` |
| `K_loc` | A finite-fibre bilinear on residual values |
| Formal adjoint | An integration-by-parts operator on sections |
...
Let the selected local residual carrier be

`Lambda^13(V*) tensor (Cl^1(V) + Cl^2(V) + Cl^5(V))`,

for `V=R^(7,7)`.  Use the degree-thirteen Hodge form pairing and scalar
Clifford trace.  Exact combinatorics gives:

| Carrier | Dimension or inertia |
|---|---:|
| Degree-thirteen form pairing | inertia `(7,7)` |
| Clifford grade 1 | inertia `(7,7)` |
| Clifford grade 2 | inertia `(49,42)` |
| Clifford grade 5 | inertia `(1001,1001)` |
| Full selected residual carrier | dimension `29,498`; inertia `(14,749,14,749,0)` |

The frozen all-grade source response has 1,470 independent columns and
4,330 coordinate positions.  Restricting `K_loc` to that response gives:

- exact rank `1,470` and nullity `0`;
- exact inertia `(741,729,0)`;
- `7,346` nonzero Gram entries;
- `391` exact congruence blocks, of sizes
  `3x286`, `5x78`, `13x13`, `2x13`, and `27x1`;
- grades 1 and 2 already detect all 1,470 source directions, while the
...
A grade-diagonal pairing begins with weights `(w1,w2,w5)`.  Spin bivectors
preserve Clifford grade, so Spin(7,7) imposes no relation between them.  Two
explicit allowed full-adjoint witnesses give
...
- Representation theory asked whether the proposed symmetry actually mixes
  the live grades.  Exact grade-one and grade-five witnesses do.
- Krein/operator theory required inertia and nullity, not only rank.  The
  response form is nondegenerate and indefinite.
- Variational PDE required a hard fence between fibre algebra and a closed
  operator domain.  The formal adjoint and Green identity remain open.
- Symplectic geometry required that a symmetric bulk Gram form not be called
