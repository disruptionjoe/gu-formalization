---
title: "Woit principles for GU: three exact transfer gates"
...
ready-made unification model. It is a set of three unusually sharp structural
questions:
...
The three companion notes turn those principles into GU gates with executable
...
This sequence is a parallel Woit-derived research path. It does not silently
displace the already frozen `B5-INDEPENDENT-RECONSTRUCTION` lead.

### 1. `GU-OS-THETA-CONSTRUCTOR`
...
### 2. `GU-SOLDERED-VARIATION-EXTRACTION`

Take the committed source action's connection variation and ask whether any
term has the Palatini shape
...
torsion/compatibility map. If the only terms are the H27 algebraic
`theta=0` trap or a divergence equation with kernel, record a stronger
action-level non-transfer result. Do not insert a conventional tetrad after
the fact.

### 3. `GU-TWISTOR-SUBSTRATE-MAP`

Before attempting a Penrose transform, provide:

```text
V_GU, S_GU subset V_GU, Q_GU=V_GU/S_GU,
observer four-plane -> Gr(2,V_GU),
incidence lift, real structure/Phi, and reconstructed solder form.
```

Only after those types close should a twistor symbol be tested inside the
frozen B5 `Hom_H(V tensor W_i,W_j)` enumeration. A named twistor operator is a
seed, not completeness.
