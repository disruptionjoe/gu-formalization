...
method: "5 specialist seats inline (pre-flight) -> execution -> the same 5 seats inline (hostile review),
  per house convention that personas run inline. Directed by Joe, direct chat 2026-08-09."
...
`--break-system-packages` was **not** used. The run was executed in an isolated venv
...
3. **Layer-0 semanticist.** Four objects wear the letter `C`. And the governing hypothesis note already
   ...
   used as an argument.** The run must test a mechanism.
4. **Verification engineer.** Reproduce published anchors before trusting any new number. Do not
   `--break-system-packages`.
5. **Adversary.** A null here is uninformative (seat 1). The run is only worth doing if it contains a
...
and, decisively for the question this run was commissioned to answer:
