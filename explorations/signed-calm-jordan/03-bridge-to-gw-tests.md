...
**Predecessor inputs.** `01-jordan-calm-formalization.md`; `02-option-b-metric-cointegration.md`; WRK-387 `falsification-result.md` (the original GW-bridge failure); 21-persona §1 Lattice QFT theorist + §3 Anomaly theorist + §4 Numerical lattice physicist (GW-side authority).
...
`[speculation, this draft's read]` — the WRK-396 exploration *confirms* the 21-persona meta-verdict's "Option II-Retreat" framing rather than overturning it. The signed-CALM repair gives a tighter, more publishable version of the propagation-layer survival claim (via JD-CALM or AC-CALM as the technical machinery), but does not rescue the integer-index decision-layer claim.
