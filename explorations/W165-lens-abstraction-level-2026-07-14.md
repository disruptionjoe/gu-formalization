...
external_refs:
  - "Weyl, The Classical Groups (1939) -- First Fundamental Theorem for O(n): the invariant ring of a symmetric matrix is generated by the power traces (degree-2 part = span{(tr S)^2, tr(S^2)})"
  - "Willmore, Total Curvature in Riemannian Geometry (1982); Graham-Jenne-Mason-Sparling, J. LMS 46 (1992) 557 -- the trace-free conformal (Willmore/GJMS) energy of the second fundamental form"
  - "Kempf-Ness, LNM 732 (1979) 233 -- the moment-map norm-square as the canonical invariant energy"
  - "Chamseddine-Connes, CMP 186 (1997) 731 -- the spectral action principle (heat-kernel a_2 coefficient)"
  - "Stelle, PRD 16 (1977) 953 -- fourth-order gravity spectrum (the R^2 = spin-0 scalaron whose sign c_R sets tachyonicity)"
---

# W165 -- the wrong abstraction level: does a universal property force the action, and which SIGN?

## 0. The charge, and the one-line answer

The full-roster two-track sweep (2026-07-11, slice B synthesis) diagnosed a recurring failure mode:
"the program is one abstraction level too low -- declaring an object where a UNIVERSAL PROPERTY
(variational uniqueness, index, adjoint) would force it, and treating the signature that sets the
grading as fixed rather than as the lever to force," plus "coherent conflated with forced." That
slice NAMED concrete forcing candidates: the source action as the UNIQUE conformally-invariant
Willmore/GJMS functional of the soldering distortion (analyst + twistor), as the Kempf-Ness
moment-map norm-square (symplectic geometer), as the Connes spectral action (NCG), as the value of
the section-functor adjoint (category theorist). The hope was that a universal property would both
FORCE the source action and, with it, settle the scalaron SIGN (debit-1, the tachyon).

This wave takes the hope literally and EVALUATES it against the actual induced-action data
(W126 `|II|^2`, W130 `c_R`, W159 `|H|^2` and the shape family). The hope INVERTS. The named
universal properties do force a point when you add the natural conformal/positivity rigidifier --
and the point they force is the TACHYONIC one. The reason is a degeneracy nobody had exploited: the
covariant scalaron coupling is IDENTICAL on both isotypic pieces (`c_R_II = c_R_H = -4/9`), so it is
BLIND to the shape freedom the sweep hoped to exploit; the sign is a one-bit function of the total
weight, which every norm-square puts on the tachyonic side. Health lives off the norm-square cone,
...
|---|---|---|
| Source-action candidate | a NORM-SQUARE of the second fundamental form `II` w.r.t. an invariant inner product | This is the common denominator of ALL four named forcing candidates (Willmore/GJMS, Kempf-Ness, spectral action, adjoint) at quadratic order. |
| Isotypic split | `II = H (trace) (+) II_0 (trace-free)`, two inequivalent real irreducibles | The canonical structure-group decomposition; the source of the shape family. |
| Scalaron coupling | the covariant `c_R = a2s + a3s/3` on each piece (W130) | W157/W159 settled that the physical scalaron sign rides `c_R`, not the slice `a2_MSS`. |
...
itself a universal property -- the invariants are the terminal cone over the group action -- and by
Schur's lemma it fixes ONE scale per inequivalent irreducible summand. The second fundamental form
of an immersion splits canonically under the structure group as the trace part `H` (the mean
curvature vector) and the trace-free part `II_0`, two INEQUIVALENT real irreducibles. So the
invariant quadratic forms on `II` are a 2-DIMENSIONAL space.

This is EXHIBITED, not named (test U1a). On the single-codimension model (base dimension 3), the
exact `so(3)`-invariance condition `L_X^T Q + Q L_X = 0` for all three generators (where `L_X` is
the coordinate matrix of `S -> [X, S]`) has a solution space of dimension exactly 2, spanned by
`(tr S)^2` and `tr(S^2)` -- equivalently `|H|^2` and `|II|^2`. `SO(3)`-invariance equals
`O(3)`-invariance at quadratic order (the only other `O(n)` relative invariant, `det S`, is cubic),
and completeness of the two power-trace invariants is the classical First Fundamental Theorem
...
polarization), a conformal weight (Willmore/GJMS selects the trace-free combination), or a preferred
...
```
|II|^2 :  c_R_II = a2s + a3s/3 = 8/9 + (-4)/3 = -4/9
|H|^2  :  c_R_H  = a2s + a3s/3 = -4/9 + 0    = -4/9
```

They are IDENTICAL. Therefore on the shape family `alpha|II|^2 + beta|H|^2` the covariant scalaron
coupling COLLAPSES (test U2):

```
c_R(alpha, beta) = alpha (-4/9) + beta (-4/9) = -(4/9)(alpha + beta) .
```

`c_R` depends ONLY on the total weight `alpha + beta` -- it is SHAPE-BLIND. Meanwhile the Einstein
coefficient does vary with the shape: `a1(alpha, beta) = (alpha + 4 beta)/3` (test U3). So W159's
`det = 4/9` "independence of `(a1, c_R)`" is carried ENTIRELY by `a1`; `c_R` contributes nothing to
the shape dependence. The consequence is a one-bit law:
...
The negative control confirms the degeneracy is GU-SPECIFIC, not a triviality (test NC1): a
...
**The conformally-natural rigidifier forces the trace-free point, which is TACHYONIC (test U4).**
The Willmore/GJMS universal property selects the trace-free combination
`|II_0|^2 = |II|^2 - c|H|^2` with the trace-removal coefficient `c in (0,1)` (standard `c = 1/n < 1`).
Its coupling is `c_R = -(4/9)(1 - c) < 0` for EVERY `c < 1`. The pure `|II|^2` point (`c = 0`) is
`c_R = -4/9 < 0` too. So the single most-cited forcing candidate -- "the source action is the unique
conformally invariant functional of the soldering distortion" -- when actually evaluated, forces a
TACHYONIC point.

**Any positive-total-weight norm-square forces the tachyon (test U5).** A positive-definite
norm-square lives on the cone `alpha + beta > 0`: the Kempf-Ness moment-map norm-square, the spectral
action's `a_2` heat-kernel coefficient with a positive cutoff function, the positive-definite
...
components carry OPPOSITE signatures. The `(9,5)` ambient inner product is INDEFINITE (W159:
`w2 = <II_1, II_1> = -64 < 0`), so a KREIN norm-square genuinely can have `alpha < 0` -- W159's
healthy member `(alpha, beta) = (-2, 1)` is exactly such a point (test U6). The sign-deciding object
is therefore the RELATIVE Krein signature of the trace component `H` (the record-count / BLMS number
mode -- the mode W156 identified as where the tachyon and `Lambda` both live) versus the trace-free
component `II_0` (the geometric mode).

That relative signature is an assignment on the `(9,5)` `q=5` indefiniteness -- which W150 DERIVED as
the image of the FINALITY FRONTIER. And by the standing tri-repo division of labor (Joe 2026-07-02),
the CAPABILITY MEASURE and the finality/irreversibility structure on that frontier are owned by
time-as-finality (TaF), not by GU.

**The bridge, as a GATED CONJECTURE (stated, not asserted; test U7).** The completion NEEDS, from
across the gate, TaF's capability/finality signature on the `q=5` frontier assigning the record-count
(trace) mode its Krein sign RELATIVE to the geometric (trace-free) mode. Two branches, both making
the sign a THEOREM rather than a free coefficient:

- IF that relative sign is OPPOSITE, a Krein norm-square lands at `alpha + beta < 0` and the
  universal property FORCES HEALTH -- debit-1 dissolves as a signature effect.
- IF that relative sign is the SAME, the tachyon is FORCED, and it is REINTERPRETED as the
...
respects the gate on every count: no cross-repo identity is asserted anywhere in the test; the
identity is gated behind at least two adapter contracts (the single adapter actually swung, TaF's
E3/T421, logged a DISANALOGY); single-process caution stands; and TaF's capability results are
finite-witness (T110, the only general theorem, is NEGATIVE). It does NOT convert the debit within
the gate; it relocates the sign question precisely.

## 6. Synthesis -- the return data

| Question | Verdict | Basis |
|---|---|---|
| Does a universal property FORCE the action to a point? | Only up to a 1-param PROJECTIVE family | U1a: `O(3)`-invariant quadratic forms of `II` = 2-dim (EXHIBITED) |
| Does forcing the action force the SIGN? | NO -- the sign is orthogonal to the shape | U2/U3: `c_R = -(4/9)(alpha+beta)` SHAPE-BLIND; `sign(c_R) = -sign(alpha+beta)` |
| Do the NAMED universal properties force HEALTH? | NO -- they force the TACHYON | U4/U5: trace-free Willmore `c_R = -(4/9)(1-c) < 0`; positive-cone forces `c_R < 0` |
| Where does health live? | off the norm-square cone; at OPPOSITE isotypic Krein signs | U6: `(9,5)` indefinite (`w2 = -64 < 0`); the relative `q=5` sign decides |
| Who owns the deciding object? | TaF's capability measure on the `q=5` finality frontier | GATED CONJECTURE (U7); tri-repo, identity NOT asserted |

**UNIVERSAL-PROPERTY VERDICT: NO-FORCING-PROPERTY-FOUND (within GU)** -- and sharper than a null: the
conformally-natural norm-square universal properties are EXHIBITED to force the TACHYONIC point, not
health. The two-track "one abstraction level too low" hope is not merely unfulfilled; it inverts. The
sign-deciding lever is the isotypic Krein signature on the `q=5` frontier, which is gated to TaF.

**CROSS-REPO BRIDGE (gated conjecture):** the completion needs, from time-as-finality, the
capability/finality Krein-signature assignment to the record-count (trace) mode relative to the
geometric (trace-free) mode on the `(9,5)` `q=5` finality frontier. Opposite sign forces health; same
sign forces the tachyon-as-irreversible-finality-mode (T110 + autopoiesis). Either way the scalaron
SIGN becomes a theorem of the finality structure -- but the tri-repo identity stays UNTESTED
(temporal-issuance owns issuance; TaF owns the capability measure; adapter contracts un-swung;
single-process caution).

**COMPLETION VERDICT + effect on bar (b): CONDITIONAL RELOCATION, NOT CONVERSION. bar (b) UNCHANGED.**
The debit does not convert within the gate. What this wave adds is a precise localization: debit-1 is
not an unexplained coefficient sickness and not (W159) a forcible feature of attraction; it is the
one-bit sign of the record-count mode's Krein signature on the finality frontier -- exactly the
`q=5` object W150 named and the E2 lever W159 named, now shown to be the SAME object and shown to be
what a universal property would have to import to force either way. Consistent with W159 and W160:
the flaw count does not drop; the standing debits `{1, 3}` are unchanged.

## 7. Gates and honest limits

- Exploration grade; conditional register throughout. Nothing asserts GU, asserts a vacuum, or
  changes any verdict. The results are computed statements about what a norm-square universal property
  does to the W126/W130 coefficients, with W126/W130/W159 positive controls, not assertions about GU.
- W138 battery honored: every verdict binds to GU-specific objects (the W130 `c_R` degeneracy, the
  W159 `|H|^2` decomposition and `w2 = -64`, the `(9,5)` `q=5` frontier). The tempting stories the
  structure FORBIDS are recorded: "a universal property forces the healthy action" (forbidden -- it
  forces the tachyon, U4/U5); "the shape freedom lets us tune the sign" (forbidden -- `c_R` is
  shape-blind, U2); "the collapse is automatic" (forbidden -- NC1, it is GU-specific).
- Tri-repo gating STRICT: the finality/capability bridge is a GATED CONJECTURE; no cross-repo
  identity is asserted anywhere. "record / finality / capability" stay pointers to
  temporal-issuance / time-as-finality / TaF; GU owns the induced-action and Krein math only.
- The universal-property analysis is at quadratic order (the induced potential sector is exactly
  quadratic, W126). It does not claim the four candidates are literally identical -- only that at the
  order that sets the scalaron sign they reduce to a norm-square on the same reducible `II` module,
  where Schur leaves the sign-deciding relative scale free.
- No canon / RESEARCH-STATUS / claim-status / verdict / posture change; the debit count stays `{1,3}`;
  H41 unbuilt; H59/H61a OPEN; the E2 AF/AS fork carried, not closed. No forbidden target assumed or
