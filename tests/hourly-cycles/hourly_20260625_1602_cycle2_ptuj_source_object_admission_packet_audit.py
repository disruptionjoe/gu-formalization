"""Audit PTUJ_SourceObjectAdmissionPacket_1602_V1."""
...
DOC = ROOT / "explorations" / "hourly-20260625-1602-cycle2-ptuj-source-object-admission-packet.md"

EXPECTED_ARTIFACT_ID = "PTUJ_SourceObjectAdmissionPacket_1602_V1"
EXPECTED_RUN_ID = "hourly-20260625-1602"
EXPECTED_OWNED_PATH = "explorations/hourly-20260625-1602-cycle2-ptuj-source-object-admission-packet.md"
EXPECTED_AUDIT = "tests/hourly_20260625_1602_cycle2_ptuj_source_object_admission_packet_audit.py"
...
def load_text() -> str:
    try:
        ...
        raise AssertionError(f"missing PTUJ source-object packet: {DOC}") from exc
...
class PTUJSourceObjectAdmissionPacketAudit(unittest.TestCase):
    @classmethod
    ...
    def test_global_receipt_restart_and_target_import_firewall(self) -> None:
    ...
    def test_both_branch_rows_are_rejected_or_blocked_with_missing_fields(self) -> None:
        ...
        self.assertGreater(len(official["required_fields_missing"]), 0)
        ...
        self.assertGreater(len(local["required_fields_missing"]), 0)
        self.assertIn("target_import_guard", local["required_fields_present"])

    def test_admission_rule_requires_nonempty_branch_receipt(self) -> None:
    ...
    def test_formula_keating_identity_and_proof_restart_promotion_are_false(self) -> None:
        ...
        self.assertFalse(firewall["ptuj_formula_promotion"])
        self.assertFalse(firewall["keating_identity_promotion"])
        self.assertFalse(firewall["ig_selector_promotion"])
        self.assertFalse(firewall["proof_restart"])
        self.assertFalse(firewall["metadata_as_formula_receipt"])
        self.assertFalse(firewall["locator_as_source_object"])
        self.assertFalse(firewall["target_physics_as_source_selector"])
    ...
    def test_first_obstruction_and_next_object_are_explicit(self) -> None:
        self.assertIn("official_custodian_source_asset_manifest", self.summary["first_obstruction"])
        self.assertIn("lawful_local_byte_toolchain_output_manifest", self.summary["first_obstruction"])
        ...
        self.assertEqual(self.summary["next_object"], "PTUJ_SourceObjectAdmissionPacket_1602_V1.accepted_branch_receipt")
        self.assertTrue(self.summary["next_object_explicit"])
        self.assertIn("construct_one_accepted_PTUJ_source_object_branch", self.summary["next_meaningful_proof_or_source_computation"])
