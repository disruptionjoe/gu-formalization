...
`selected_action_stationary_spin_lc_hessian_probe.py` and
`conditional_physics_ledger_v030_probe.py`. They separate the rank-ten
coordinate Christoffel map from the rank-nine action spin-Levi-Civita map,
compute the exact stationary selected metric Hessian on each causal orbit,
measure its rank-three diffeomorphism cross residual, and freeze five
distance-only migrations. Direct curvature/full-II/defect/observation Ward
