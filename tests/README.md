...
`k77_wave2_full_adjoint_shiab_bianchi_two_connection_target_probe.py` and
`k77_wave2_bianchi_two_connection_target_independent.sage`. They prove the
full displayed eight-map span is exactly five, construct the eddy-completed
first-moment Bianchi syzygy, and independently reconstruct the same pre-Shiab
path target from the shifted two-connection blocks. Their immediate
predecessor is `k77_wave2_action_polarization_common_observation_domain_probe.py`
with its independent Sage product-rank verifier; it computes the complete
grade-one span, all eight action polarizations, selection rank zero, and the
conditional common `H10 -> H9` observation scale. The earlier predecessor is
`k77_wave2_moving_shiab_epsilon_ward_green_domain_probe.py`, the exact
moving-Shiab census, primitive epsilon/even-Ward and compact-Green gate. The
earlier conormal predecessor is `k77_wave2_i1b_conormal_symbol_weld_domain_probe.py`, the exact
selector-independent I1B principal-symbol, fixed-section `85/91` annihilator,
all-section factorization, normal-density weld and trace-domain gate.
Its immediate receiver/localization predecessors remain
`k77_wave2_actual_y14_receiver_ordering_probe.py`,
`k77_wave2_augmented_torsion_defect_euler_receiver_probe.py`, and
`k77_wave2_full_source_action_defect_localization_probe.py`.
