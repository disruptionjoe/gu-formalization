...
| `channel-swings/` (588 Python + 92 Sage) | active channel and Lane-1 swing probes; the current count includes `selected_k77_i2b_real_shiab_displasion_image_probe.py`, which excludes the displasion target from the complete fixed-`H_q` real grade-one Shiab image while retaining an unrestricted complex positive control, and `conditional_physics_ledger_v0202_probe.py`, plus `selected_k77_source_i2b_hq_stationarity_probe.py`, which proves SC-ACT-04 owns the restricted potential while exposing fourteen transverse connection derivatives, and all predecessors. The detailed historical inventory remains represented by the files and per-wave entries above. |
