...
| `channel-swings/` (586 Python + 92 Sage) | active channel and Lane-1 swing probes; the current count includes `selected_k77_source_i2b_hq_stationarity_probe.py`, which proves SC-ACT-04 owns the restricted potential while exposing fourteen transverse connection derivatives, and `conditional_physics_ledger_v0201_probe.py`, plus `selected_k77_hq_action_owner_potential_probe.py`, which separates the released first action from the conditional eddy-square Higgs-potential route, plus `selected_k77_split_layer_commutant_action_parent_probe.py`, which computes the exact split-subgroup commutant and distinguishes the full, block and finer split parents, plus `selected_k77_polarized_radical_bfv_ownership_gate_probe.py`, `selected_k77_polarized_green_dual_gate_probe.py`, `selected_k77_natural_trace_constraint_gate_probe.py`, `selected_k77_moving_antidualizer_darboux_probe.py`, `selected_k77_coupled_green_domain_probe.py`, `selected_k77_coupled_gauge_noether_bv_probe.py`, `selected_k77_high_conviction_receiver_completion_probe.py`, `selected_k77_gamma_trace_graph_dynamics_probe.py`, `selected_k77_moving_varpi_stationary_intersection_probe.py`, `selected_k77_lorentzian_chiral_class_pairing_probe.py`, `selected_k77_p3_replacement_surplus_probe.py`, `selected_k77_p3_normal_tangential_support_probe.py`, the P3 chiral-spin bundle diagonal, P3 native-characteristic pairing kill, P3 characteristic-amplitude conditional, zero-fermion VEV selector-exhaustion composition, zero-fermion coupled-Hessian/current-order gate, induced K77 fermion principal discriminator, complete nonzero-branch parent Hessian, bosonic parent action ownership, the moving-parent bundle/observation reduction, grade-five/unitary-parent Euler closure, complete Euler-jet tangent closure, observation-stabilizer subbundle and nonrecursive tangent bank, minimal-Hessian tangent closure, moving-metric first-action completion, lower-order source-block reconciliation, two-branch action-block port, branch-Hessian discriminator, bulk-operator mixed-order admission gate, common graded trace/boundary-triple skeleton, relative edge-bitorsor topology theorem, branch symplectic/BFV no-selector, branch boundary-amplitude classification, full-parent branch-stationarity gate, source-tangent branch predecessor, nonconstant affine atlas/Xi-prolongation gate, source-Euler two-to-one family and local connection-jet gate, source-shaped scalar curvature-jet trace predecessor, common connection-critical branch and moving-epsilon Hessian cross, primitive-epsilon common principal bank, partial stationary Gram causal strata and trace quotients, compact-boundary strong Sobolev edge reduction/current algebra, full nonzero-`A0` tilted algebraic edge descent, the physical-boundary disposition selector, local selected-action Noether/preboundary composition, common 34-field physical equation-dual/Green composition, source-native matched-q physical Ward closure, common metric residual-coordinate bank, common-field formal-adjoint/Green ownership gate, fixed-varpi normal Frechet closure, transverse comoving coefficient closure, operative pairing-symmetry closure, conditional K77 residual pairing, action/Frechet/Ward object separation, signature-generic Cartan/Ward composition, signature-rationale retype, physical-diffeomorphism split, independent Sage routes, and all predecessors. The detailed historical inventory remains represented by the files and per-wave entries above. |
