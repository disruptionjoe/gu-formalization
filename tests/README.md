...
| `channel-swings/` (434 Python + 41 Sage) | active channel and Lane-1 swing probes; the current count includes the selected K77 action boundary coefficient bank, its independent Sage reconstruction, and ledger-v0.76 integrity gate. The detailed historical inventory remains represented by the files and per-wave entries above. |
