...
`selected_branch_bv_tt_curvature_vev_flrw_probe.py` with an independent Sage
control and v0.14 ledger probe. It constructs the rank-ten metric-to-gauge-
rotated-Levi-Civita derivative modulo gauge, proves at least two non-exact
even-BV TT classes for the massive partner, builds its finite positive Krein
majorant and derives the local curvature/VEV horn plus exact non-screening
susceptibility. Planted controls forbid gauge-to-nonlinear promotion,
even-BV-to-odd promotion, finite-to-UV positivity, spatial-flatness-to-`R=0`
collapse and tracking-to-screening promotion.
