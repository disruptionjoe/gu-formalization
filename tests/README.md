...
`selected_action_second_soldering_observation_jets_probe.py` and
`conditional_physics_ledger_v029_probe.py`. They construct the exact nonzero
symmetric-frame spin Levi-Civita second jet, type the affine observation
Hessian and spatial second section jet, verify the nonlinear formal-adjoint
Euler/preboundary owner, and freeze five distance-only migrations. Direct
