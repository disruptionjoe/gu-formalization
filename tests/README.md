...
`trace_omega_higgs_chirality_compose_probe.py` and
`conditional_physics_ledger_v019_probe.py`. They enforce the four append-only
distance migrations, shared no-new-datum `T_omega -> sigma_epsilon -> h_omega`
chain, preserved `RA-D2` genuine-falsification verdict, mandatory symplectic
boundary, unchanged residue/P1/P2/P3 and the full-moving selected-cubic Build
gate at rank one.
