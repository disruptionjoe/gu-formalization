...
`pre_shiab_gauss_defect_action_bv_symbol_probe.py` with its independent Sage
control and v0.5 ledger probe. It rules out current `I1B` ownership at the
homogeneous `T=0` selected-Shiab value locus, constructs a conditional
restriction-first fixed-slot repair, preserves the trace-reversed `(6,4)`
pairing, and proves the exact non-null `4 -> 20 -> 20 -> 4` even-BV symbol
complex. Its zero-gain and null controls block promotion to a global or
physical quotient; the global `epsilon_IG` soldering map, nonlinear weld and
null/Green domain are the recorded successor gate.
