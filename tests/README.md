...
`selected_action_coupled_diffeomorphism_ward_retype_probe.py` and
`conditional_physics_ledger_v032_probe.py`. They retain the metric-only
rank-34/affine-21 theorem at its scoped grade, restore the rank-four
connection-one-form Lie symbol, compute the coupled rank-98/affine-198 Ward
target, and freeze five corrected distances. Actual same-`I1B` cross and
connection blocks, BV, global domain and BFV remain open.
