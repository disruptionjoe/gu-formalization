...
`k77_global_even_bv_null_green_domain_probe.py` with an independent Sage/QQ
control and v0.8 ledger probe. It composes the global K77 owner chain, checks
formal minimal homogeneous-gauge CME hypotheses, proves the null
`10 -> 6 -> 2` constraint/gauge quotient with explicit plus/cross modes, and
types the conditional flat-defect Green complex. Planted controls block
promotion to diffeomorphism/odd BV, positivity, curved/global `Y14` analysis,
observation or cosmology. The global coupled `Y14` observation/domain gate is
