...
| `recovery-contract/` (8) | recovery-certification branch-local computational checkpoints, including the GR forced-coefficient residual test, GR no-go history/scope defense gates through Swing 2, cosmological field-type/scalar-truncation gate, cosmology no-go history/scope defense gate, Standard Model selector screen, SM selector no-go history/scope defense gate, and QM physical-sector conditional sufficiency gate under the frozen action fingerprint |
