...
| `channel-swings/` (446 Python + 47 Sage) | active channel and Lane-1 swing probes; the current count includes the stationary two-layer Hessian factorization probe, independent Sage route, and ledger-v0.82 integrity gate. The detailed historical inventory remains represented by the files and per-wave entries above. |
