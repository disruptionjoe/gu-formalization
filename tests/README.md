...
`selected_branch_linearized_totalization_domain_probe.py` with an independent
Sage control and v0.13 ledger probe. It separates the radial Clifford-one
Hessian from the actual gravitational Gauss Hessian, derives the exact
trace/traceless coefficients, types the one-action current/stress chain,
constructs a common observed defect Krein/Green domain and classifies the
massless/partner residues. Planted controls forbid carrier conflation, current-
equals-stress, Green-to-positive-cohomology promotion and direct-source-to-two-
field-cosmology promotion.
