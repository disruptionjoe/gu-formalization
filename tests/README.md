...
`first_interaction_krein_global_zero_mode_probe.py` with an independent Sage
control and v0.15 ledger probe. It proves the first owned cubic defeats every
multiplicative scalar-sign extension of the free spectral grading, closes the
finite-local constant-mode class against screening and constructs the exact
conditional normalized global projector. Planted controls forbid promotion to
no interacting `C`, a GU-derived averaging functional or P2 identification.
