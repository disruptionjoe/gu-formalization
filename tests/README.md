...
`selected_cubic_two_connection_principal_ward_descent_probe.py`,
`selected_cubic_two_connection_principal_ward_descent_independent.sage` and
`conditional_physics_ledger_v022_probe.py`. They prove by independent exact
routes that the source-owned `24+24 -> 24` connection difference has rank 24
and diagonal kernel 24, turns the isolated rank-five principal gauge block
into rank zero without erasing the physical LC kernel, enforce the mandatory
symplectic fence against full Ward/BV/preboundary promotion, and freeze
exactly five v0.22 distance migrations with unchanged verdicts, residue,
