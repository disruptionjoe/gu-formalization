...
`selected_action_curvature_graph_six_versus_four_probe.py` and
`conditional_physics_ledger_v034_probe.py`. They compute the exact selected
graph curvature gain `-1/26`, lift all six zero-jet nongauge nulls off cone,
retain exactly two tensor characteristics on cone and freeze five corrected
distances. The off-graph `d_B T` block, observation receiver, common domain,
odd BV and BFV remain open.
