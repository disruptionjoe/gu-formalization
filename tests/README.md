...
`k77_wave2_eddy_augmented_torsion_euler_prolongation_probe.py` with its
independent Sage control. It reconstructs the eddy path average from the
two-connection data, replays the K77-B3 kill of the printed unit-weight
endpoint, and builds the action's actual Fréchet-adjoint Euler formula. The
rank-13 generic symbol is retained only for the printed degree-14 rival; Sage
rebuilds that rival and the identity-Shiab transgression control, not the
action Euler. Immediate predecessors are the principal-Bianchi selector, the
full-adjoint/path-moment target, the shifted two-connection action owner and
the action-first current/Ward architecture. Receiver/localization and common
domain predecessors remain separately inventoried below.
