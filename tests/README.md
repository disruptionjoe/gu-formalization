...
| `channel-swings/` (600 Python + 92 Sage) | active channel and Lane-1 swing probes; the current count includes `selected_k77_i2b_full_trace_orbit_derivative_probe.py`, which exhausts the 13-dimensional normalized trace orbit and its moving primalizer derivatives, `conditional_physics_ledger_v0208_probe.py`, and all predecessors. The detailed historical inventory remains represented by the files and per-wave entries above. |
