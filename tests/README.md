...
`two_layer_action_selected_cubic_owner_retype_probe.py` and
`conditional_physics_ledger_v024_probe.py`. They separate first-order,
residual-square and observer-full-II owners, prove generic cubic independence
with an accidental one-dimensional plant, preserve one-way solution
redundancy, split the queue before Q1 and freeze exactly one LT-GR3
distance-only migration with unchanged verdicts, residue, quotients and
P1/P2/P3. The predecessor homogeneous Ward Python/Sage pair remains the
latest construction result.
