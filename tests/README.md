...
`selected_moving_k77_vacuum_p2_norm_probe.py` with an independent Sage control
and v0.12 ledger probe. It builds the rank-100 horizontal Gauss receiver,
derives full `|II|^2` from the written action against a rank-ten trace-first
control, and computes the selected non-cyclic K77 algebraic branch
`t=-kappa_1/312`. Planted controls forbid receiver homonyms, `P2_norm`/
`P2_datum` collapse, algebraic-to-physical-vacuum promotion, special-branch
endpoint extrapolation and unbooked physical residue reduction.
