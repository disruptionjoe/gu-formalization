...
`observed_upback_stress_normal_constraint_vacuum_probe.py` with an independent
Sage/QQ control and v0.10 ledger probe. It reconstructs the action-owned
Hilbert stress by radial transgression, includes the active Krein pairing in
an exact massless-Dirac control, retains the observed `10 -> 6 -> 2`
plus/cross quotient, exposes the repaired gravity double pole and scopes the
zero/indefinite vacuum result to the quadratic horn. Planted controls forbid
literal `VU`-as-stress, current/stress collapse, quotient-to-single-pole
promotion, false stability/screening and P1/P2/P3 movement.
