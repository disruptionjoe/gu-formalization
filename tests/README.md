...
`selected_action_source_variable_hessian_probe.py` and
`conditional_physics_ledger_v033_probe.py`. They replace the frozen-slot
one-form surrogate with source variables `(g,varpi)`, compute the exact
rank-24 selected zero-jet Hessian with four gauge plus six nongauge null
directions, close both Ward block equations and freeze five corrected
distances. The full first-order `I1B` six-versus-four test, BV, global domain
and BFV remain open.
