...
| `channel-swings/` (128) | active channel and Lane-1 swing probes, including the native operator/domain/`w1` bridge audit, the Pin+ degree-14 Smith/table gate, the Mannheim--Callias end-admission gate, the quaternionic Fredholm/end-clutching gate, the vertical--Krein source-action/B5 one-bit-weld probe, the three-route full-carrier Bott/vertical/actual-fibre starts, the W177 stationarity plus W131-to-B5 normalized-transport gates, and the full-20 coarse observer/BV first-write with exact maximal-envelope coloring, whole-envelope integer-degree, realification, and four-stage-carrier Layer-0 controls |
