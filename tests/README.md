...
`k77_moving_observation_y14_domain_obstruction_probe.py` with an independent
Sage/QQ control and v0.9 ledger probe. It constructs the determinant-one
complete first-jet section observation and inverse-transpose equation dual,
kills value-only and finite-jet global-shell promotion, computes all three
K77 hypersurface inertias and types the conditional observed
curvature/distortion equation. Planted controls preserve constrained
ultrahyperbolic and observation-first horns, plus/cross modes, the open
up-and-back stress map and nonzero-vacuum selection.
