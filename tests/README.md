...
`selected_action_ward_completion_identifiability_probe.py` and
`conditional_physics_ledger_v031_probe.py`. They solve the symmetric Ward
target for the stationary spin-LC residual, prove rank 34 with a
21-dimensional quotient-form ambiguity, reject separately invariant blocks
and observation congruence as cancellations, and freeze five distance-only
migrations. Same-`I1B` direct coefficient assembly, BV, global domain and BFV
remain open.
