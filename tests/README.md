...
`k77_epsilon_gravitational_soldering_weld_probe.py` with its independent Sage
control and v0.6 ledger probe. It builds the K77 receiver
`pr_V pi1_epsilon(v_T(g/2))`, proves rank ten plus an adjoint isometric right
inverse and orthogonal projector, and proves an exact same-stratum action
sector replacement. Its planted controls block K95 import, Euclidean pairing,
equivariance-as-uniqueness, double counting and promotion to a bulk/defect or
nonlinear-BV theorem. Global full-reduction existence, support normalization
and the null/Green domain are the recorded successor gate.
