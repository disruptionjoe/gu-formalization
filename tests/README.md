...
| `recovery-contract/` (6) | recovery-certification branch-local computational checkpoints, including the GR forced-coefficient residual test, GR no-go history/scope defense gate, cosmological field-type/scalar-truncation gate, cosmology no-go history/scope defense gate, Standard Model selector screen, and SM selector no-go history/scope defense gate under the frozen action fingerprint |
