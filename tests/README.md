...
`k77_wave2_action_polarization_common_observation_domain_probe.py` and the
independent Sage verifier
`k77_wave2_action_polarization_channel_rank_independent.sage`. They compute
the complete grade-one eight-channel span, three exact restricted relations,
all eight action polarizations, selection rank zero, and the conditional
common `H10 -> H9` observation scale. Their immediate predecessor is
