...
| `recovery-contract/` (19) | recovery-certification branch-local computational checkpoints, including the construction-space GR R0, SM R0, P3 retro-verification, P4 QM checklist, P5 source-object specification, and P6 conditional-interior gates, the GR forced-coefficient residual test, GR no-go history/scope defense gates through Swing 3, cosmological field-type/scalar-truncation gate, cosmology no-go history/scope defense gates through Swing 3, Standard Model selector screen, SM selector no-go history/scope defense gates through Swing 3, and QM physical-sector conditional sufficiency gate under the frozen action fingerprint |
...
These 244 tracked direct root scripts are referenced as provenance from `canon/*-RESULTS.md`, so they are
