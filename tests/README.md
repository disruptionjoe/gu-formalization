...
`selected_cubic_qft_threshold_numerator_probe.py` with an independent Sage
control and v0.17 ledger probe. It proves that scalar fluctuation adds no new
zero-field Hessian block, carries every selected cubic monomial through both
scalar parities, derives the exact positive-branch mass ratio and continuum
odd-channel shells, and requires a selected nonzero on-shell numerator before
booking a Q1 pole. Planted controls forbid importing the W169 discrete result,
dropping monomials, identifying the scalaron with the massive spin-two partner,
or promoting kinematics to a physical-sheet/Fock/domain result.
