...
A partial ladder (4 of 7 rungs structural, 3 blocked) is the honest result.

CORRECTION 2026-08-03 (eleven-lens audit HB-05; register M-M13): rung 5 (Rokhlin)
RETYPED.  Rokhlin's theorem lives in Omega^Spin_4 = Z (generated by K3, sigma = -16;
detected by sigma/16), NOT in Omega^{Pin+}_4 = Z/16 -- that is a true group fact
(Kirby-Taylor) but a DIFFERENT group carrying a DIFFERENT theorem (the two 16s are
related via Guillou-Marin characteristic-surface arithmetic, not equal).  Under this
ladder's own typing rule the rung is a free-integer-index rung (a 2-power
DIVISIBILITY of the Z-valued signature), so the honest tally is 4 structural + 3
blocked (was 5 + 2).  The 2-primarity of the Rokhlin statement itself (mod 2^4) is
UNAFFECTED, and the blindness conclusion is unchanged (Hom(Z/24, Z) = 0 kills the
order-3 carrier on free rungs too).
...
ladder = [
    ...
    (5, "Rokhlin mod 16",
     ...
     "spin-4-manifold signature: 2-power DIVISIBILITY of a free integer index",
     "Omega^Spin_4 = Z  (free; sigma/16 is an iso to Z)", "Z  (free part)", "FREE", None, BLOCKED,
     "HB-05 RETYPE (2026-08-03): Rokhlin lives in Omega^Spin_4 = Z (gen K3, sigma=-16; "
     "detected by sigma/16), NOT Omega^{Pin+}_4 = Z/16 -- a true group fact (Kirby-Taylor) "
     "but a different group/theorem (the two 16s relate via Guillou-Marin characteristic-"
     "surface arithmetic, not equality). sigma is Z-valued, so by this ladder's own typing "
     "rule the rung is FREE; the mod-2^4 congruence content (2-primarity of the Rokhlin "
     "statement) is UNAFFECTED, and Hom(Z/24,Z)=0 still kills the order-3 carrier."),
...
check(sorted(proven_rungs) == [1, 2, 6, 7],
      "PROVEN-2-PRIMARY-BY-TYPE rungs: {1,2,6,7}  (4 of 7; HB-05 retyped rung 5)")
check(sorted(blocked_rungs) == [3, 4, 5],
      "BLOCKED (free-part, contingent) rungs: {3,4,5}  (3 of 7)")
...
print("         odd part is 1 -- rungs 1,2,6,7 [PROVEN-2-PRIMARY-BY-TYPE, cited], or")
print("    (ii) an integer (free, Z-valued) index/signature -- rungs 3,4,5 [BLOCKED]:")
...
print("         (Rung 5 retyped 2026-08-03, HB-05: Rokhlin is a 2-power divisibility")
print("         of the free signature index on Omega^Spin_4 = Z; its mod-2^4 content")
print("         is unaffected.)")
...
print()
print("  Ladder: 4 of 7 rungs proven-2-primary-by-type; 3 of 7 blocked (free-part,")
print("  contingent).  No rung rides an UNVERIFIED group fact -- the three blocked")
print("  rungs are blocked by the interpretive contingency of the integer-index")
print("  reading, not by a missing citation.")
