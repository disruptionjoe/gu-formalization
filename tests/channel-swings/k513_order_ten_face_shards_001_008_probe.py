#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K513."""
...
spec = importlib.util.spec_from_file_location(
    "k513_probe_target", HERE / "k513_order_ten_face_shards_001_008.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K513")
K513 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K513)
...
def checks(payload):
    ...
    return [
        payload["result_id"] == "K513-ORDER-TEN-FACE-SHARDS-001-008",
        fixed["first_shard_id"] == "order10-face-001",
        fixed["last_shard_id"] == "order10-face-008",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"] == K513.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 36,
        decision["unexecuted_remaining_face_programs"] == 880,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 879),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K513.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K513 controls: {sum(controls)}/{len(controls)}; "
