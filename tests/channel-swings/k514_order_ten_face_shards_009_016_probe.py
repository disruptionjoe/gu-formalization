#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K514."""
...
spec = importlib.util.spec_from_file_location(
    "k514_probe_target", HERE / "k514_order_ten_face_shards_009_016.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K514")
K514 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K514)
...
def checks(payload):
    ...
    return [
        payload["result_id"] == "K514-ORDER-TEN-FACE-SHARDS-009-016",
        fixed["first_shard_id"] == "order10-face-009",
        fixed["last_shard_id"] == "order10-face-016",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"] == K514.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 68,
        decision["unexecuted_remaining_face_programs"] == 848,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 847),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K514.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K514 controls: {sum(controls)}/{len(controls)}; "
