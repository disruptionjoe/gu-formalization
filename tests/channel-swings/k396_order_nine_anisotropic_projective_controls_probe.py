#!/usr/bin/env python3
"""Independent regeneration and hostile probe for K396."""
...
PRODUCER = ROOT / "tests/channel-swings/k396_order_nine_anisotropic_projective_controls.py"
STORED = ROOT / "lab/process/k396-order-nine-anisotropic-projective-controls.json"
spec = importlib.util.spec_from_file_location("k396_probe_target", PRODUCER)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K396 producer")
...
def main() -> int:
    ...
    checks = [
        stored == rebuilt,
        len(stored["anisotropic_control_bank"]) == 14,
        stored["fixed_control"]["ordered_descriptor_control_evaluations"] == 62_720,
        ...
        all(row["coherent_group_count"] == 20 for row in stored["anisotropic_control_bank"]),
    ...
    if not all(checks):
        raise AssertionError("K396 independent control failed")
    mutations = [
        lambda p: p["fixed_control"].__setitem__("selected_controls", 13),
        lambda p: p["fixed_control"].__setitem__("ordered_descriptor_control_evaluations", 62_719),
        ...
        lambda p: p["anisotropic_control_bank"][0].__setitem__("coherent_group_count", 19),
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K396 hostile rejection failed: {rejected}/{len(mutations)}")
    print(f"K396 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
