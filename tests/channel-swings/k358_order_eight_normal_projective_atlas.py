#!/usr/bin/env python3
"""Compile the exact maximum-coordinate projective atlas for K354 faces."""
...
OUTPUT = ROOT / "lab/process/k358-order-eight-normal-projective-atlas.json"
...
def jacobian_control(codimension: int) -> dict[str, Any]:
    ...
    return {
        ...
        "expected_denominator_power": q(expected),
...
def build() -> dict[str, Any]:
    ...
    if len(programs) != 517 or len(k356["face_cell_bank"]) != 517:
        raise AssertionError("K354/K356 face census changed")
    ...
    for mask in masks:
        ...
        for anchor in mask:
            ...
            charts.append({
                ...
                "coordinate_map": {
                    anchor: "1/(1+sum(r))",
                    **{axis: f"r_{axis}/(1+sum(r))" for axis in ratio_axes},
                },
    ...
    program_map = [{
        ...
        "axis": row["axis"],
        "mask_id": mask_ids[tuple(row["zeroed_axes"])],
        "codimension": int(row["codimension"]),
    ...
    return {
        "schema_version": "1.0",
        "result_id": "K358-ORDER-EIGHT-NORMAL-PROJECTIVE-ATLAS",
        "created": "2026-09-23",
        "classification": "INTERNAL_STRUCTURAL_ONLY",
        "direction": "observed_to_native",
        "fixed_control": {
            "predecessor_manifests": [str(K354.relative_to(ROOT)), str(K356.relative_to(ROOT))],
            "raw_time_axes": 18,
        ...
        "atlas_summary": {
            "all_81_unique_masks_compiled": len(atlas) == 81,
            "all_517_programs_mapped": len(program_map) == 517,
            ...
            "program_map_sha256": digest(program_map),
        },
        "decision": {
        ...
        "release_test": {
            "exactly_81_unique_masks": len(atlas) == 81,
            "exactly_578_unique_charts": sum(row["chart_count"] for row in atlas) == 578,
            "exactly_2998_program_chart_uses": sum(len(row["chart_ids"]) for row in program_map) == 2998,
        ...
        "claim_ceiling": "Exact finite maximum-coordinate projective charts for all 81 unique zero masks and all 517 K354 face programs, with exact inverse, tie ownership, radial/projective measure split and Jacobian formula across every reachable codimension. This is a coordinate cover, not a positive-width interval evaluation of the charts, a zero-inclusive boundary majorant, a disjoint global face-neighborhood cover, a K348 hybrid integral, a complete order-eight remainder/integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public or physical claim.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["face_programs"], fixed["unique_zero_masks"], fixed["unique_maximum_charts"], fixed["program_chart_uses"]) != (517, 81, 578, 2998):
        raise AssertionError("K358 census changed")
    atlas = payload["mask_atlas"]
    programs = payload["program_chart_map"]
    if len(atlas) != 81 or len(programs) != 517:
        raise AssertionError("K358 atlas identity changed")
    if any(row["chart_count"] != row["codimension"] or len(row["charts"]) != row["codimension"] for row in atlas):
        raise AssertionError("K358 maximum-chart count changed")
    if not all(row["agrees"] for row in payload["jacobian_controls"]):
        raise AssertionError("K358 Jacobian control changed")
    if not all(row["reconstructed_exactly"] and row["sum_is_one"] and row["anchor_is_maximal"] and row["equal_normal_corner_is_uniform"] for row in payload["exact_controls"]):
        raise AssertionError("K358 inverse control changed")
    ...
    if not contract["closed_chart_union_covers_complete_projective_simplex"] or not contract["chart_interiors_are_disjoint_under_tie_rule"]:
        raise AssertionError("K358 cover contract changed")
    ...
    if not decision["exact_normal_projective_coordinate_cover_complete"] or any(decision[key] for key in ("exact_angular_measure_compiled", "positive_width_projective_cells_evaluated", "recursive_positive_interior_cover_complete", "complete_hybrid_integrals_emitted")):
        raise AssertionError("K358 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K358 release test failed")
