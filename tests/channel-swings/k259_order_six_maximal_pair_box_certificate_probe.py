#!/usr/bin/env python3
"""Independent reverse-order raw-allocation replay of K259."""
...
RECORD = ROOT / "lab/process/k259-order-six-maximal-pair-box-certificate.json"
...
def raw_terms(source: dict):
    ...
    for entry in reversed(source["complete_face_hypergraph"]["entries"]):
        outer = entry["coefficient_product"] * (
...
def groups(items, pair):
    active_axes = (1,) + pair
    inactive_axes = tuple(
        axis for axis in range(7, -1, -1) if axis not in active_axes
    )
...
def main() -> None:
    ...
    bounds = {pair: [] for pair in PAIRS}
    floats = {pair: [] for pair in PAIRS}
    cell_measures = []
        ...
        for j in range(15, -1, -1):
            for k in range(15, -1, -1):
                ...
                for pair in PAIRS:
                    ...
                    floats[pair].append(float(value.upper()))
    ...
    for size in range(15, 8, -1):
        subsets = list(combinations(tuple(reversed(PAIRS)), size))
        for subset in reversed(subsets):
            sums = [
                sum(floats[pair][cell] for pair in subset)
                for cell in range(4096)
            ]
            cell = max(range(4096), key=sums.__getitem__)
            upper = sum((bounds[pair][cell].upper() for pair in subset), arb(0))
            if upper < 0:
                exact = [
                    sum((bounds[pair][n].upper() for pair in subset), arb(0))
                    for n in range(4096)
                ]
                if all(value < 0 for value in exact):
                    assert size == 9
                    winners.append(tuple(sorted(subset)))
                    continue
                cell = max(range(4096), key=lambda n: exact[n])
                upper = exact[cell]
            assert upper >= 0
            failures[str(size)] += 1
            # Reverse cell traversal means n maps to producer cell 4095-n.
            witness_rows.append(f"{size}:{pair_text(subset)}:{4095 - cell}")

    cert = record["exhaustive_subset_certificate"]
    recorded_winners = {
        tuple(map(tuple, row["exceptional_pairs"]))
        for row in cert["maximal_subsets"]
    }
    assert set(winners) == recorded_winners and len(winners) == 4
    assert dict(sorted(failures.items(), key=lambda row: int(row[0]))) == cert["failing_subsets_by_size"]
    digest = hashlib.sha256(("\n".join(sorted(witness_rows)) + "\n").encode()).hexdigest()
    assert digest == cert["nonnegative_upper_witness_sha256"]

    selected = tuple(
        map(tuple, record["selected_nine_box_bundle"]["exceptional_pairs"])
    )
    total_mass = arb(0)
    negative = 0
    for cell, measure in enumerate(cell_measures):
        bundle = sum((bounds[pair][cell] for pair in selected), arb(0))
        assert bundle.upper() < 0
        negative += 1
        total_mass += (-bundle.upper()) * A(measure)
    ...
    assert negative == 4096 and normalized > A(floor)
    ...
    for row in cert["k258_one_pair_extensions"]:
        pair = tuple(row["added_pair"])
        k258 = ((2, 4), (2, 5), (2, 6), (2, 7), (3, 5), (4, 5), (5, 6), (5, 7))
        subset = tuple(sorted((*k258, pair)))
        # Convert producer cell to reverse traversal.
        cell = 4095 - row["witness_cell_linear_index"]
        upper = sum((bounds[item][cell].upper() for item in subset), arb(0))
        assert upper >= 0
    ...
    print("[PASS] K259 independent maximal subsets", len(winners))
    print("[PASS] K259 reverse witness digest", digest)
    print("[PASS] K259 exact center, mass, disjointness and K247 controls")
