#!/usr/bin/env python3
"""Execute K511 shards 009--016 through K508's true interval backend."""
...
OUTPUT = ROOT / "lab/process/k514-order-ten-face-shards-009-016.json"
...
K508 = load("k508_for_k514", "k508_order_ten_selected_face_cell_bank.py")
...
def execute_program(program, singular_templates, confluent_templates):
    ...
    for cell_index, (left, right) in enumerate(K508.NORMAL_CELLS):
        ...
        if len(groups) != 28:
            raise AssertionError("K514 coherent-group census changed")
        operation_uses += uses
        if not math.isfinite(float(abs(second).upper())):
            raise AssertionError("K514 nonfinite cell")
        ...
        if minimum <= 0:
            raise AssertionError("K514 zero cumulative argument")
        ...
        if cell_index == 0:
            ...
            if not direct_overlap:
                raise AssertionError("K514 direct overlap failed")
    return (
        {
...
def build() -> dict:
    ...
    if [row["shard_id"] for row in selected_shards] != expected_ids:
        raise AssertionError("K514 shard identity changed")
    if any(row["program_count"] != 4 for row in selected_shards):
        raise AssertionError("K514 shard size changed")
    ...
    return {
        "schema_version": "1.0",
        "result_id": "K514-ORDER-TEN-FACE-SHARDS-009-016",
        "created": "2026-09-25",
        ...
        "decision": {
            "shards_009_through_016_released": True,
            "cumulative_K511_shards_executed_including_K512": 17,
            "cumulative_remaining_face_programs_executed": 68,
            "unexecuted_remaining_face_programs": 848,
            ...
            "next_exact_input": "Continue K511 at shard 017 under serialized resource control, then construct the normal-projective partition before interior and tail composition.",
        },
        "source_and_ledger_effect": "none",
        "claim_ceiling": "Rigorous 180-digit K508-backend evidence for the 32 exact programs in K511 shards 009 through 016, across 224 positive-width normal cells. Together with K512 and K513 this completes 68 of the 916 K508-omitted programs. It is not the other 848 programs, a complete face bank, projective/interior/tail cover, hybrid integral, K500 leakage, K473 floor, K457 value, K152 interval, or source/physical result.",
    }
...
def validate(payload: dict) -> None:
    ...
    if actual != expected:
        raise AssertionError("K514 fixed census changed")
    required = (
    ...
    if not all(bank[key] for key in required):
        raise AssertionError("K514 backend evidence failed")
    k511 = json.loads(K511_JSON.read_text())
    ...
    if actual_program_ids != expected_program_ids:
        raise AssertionError("K514 stored program identity changed")
    if bank["bank_sha256"] != K508.digest(rows):
        raise AssertionError("K514 stored bank digest changed")
    stored_uses = sum(
    ...
    if bank["divided_difference_operation_uses"] != stored_uses:
        raise AssertionError("K514 operation census changed")
    direct_ids = [row["program_id"] for row in payload["direct_overlap_controls"]]
    if direct_ids != expected_program_ids:
        raise AssertionError("K514 direct-control identity changed")
    for stored, expected_shard in zip(
        ...
        if (
            ...
            raise AssertionError("K514 stored shard provenance changed")
        shard_rows = [row for row in rows if row["shard_id"] == stored["shard_id"]]
        if stored["program_rows_sha256"] != K508.digest(shard_rows):
            raise AssertionError("K514 stored shard digest changed")
    if not decision["shards_009_through_016_released"]:
        raise AssertionError("K514 release boundary changed")
    if decision["unexecuted_remaining_face_programs"] != 848:
        raise AssertionError("K514 remainder census changed")
    forbidden = (
    ...
    if any(decision[key] for key in forbidden):
        raise AssertionError("K514 overclaimed the order-ten cover")
