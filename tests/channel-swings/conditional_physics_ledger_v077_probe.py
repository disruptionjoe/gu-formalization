#!/usr/bin/env python3
"""Integrity checks for conditional physics ledger v0.77."""
...
old = strict("lab/process/conditional-physics-ledger-v0.76.json")
new = strict("lab/process/conditional-physics-ledger-v0.77.json")
registry = strict("lab/process/selected-k77-full-u6464-action-bank.json")

check("schema", "ledger advances once", old["schema_version"] == "0.76" and new["schema_version"] == "0.77")
check("schema", "predecessor is v0.76", new["predecessor"].endswith("v0.76.json"))
...
for rid in sorted(expected):
    ...
    check("rows", f"{rid} points to v0.77 evidence",
          new_rows[rid]["evidence"] == "selected-k77-full-u6464-action-bank-2026-08-08.md")
    check("rows", f"{rid} preserves arbitrary-K rejection",
          "P_EQUALS_KT_ACTION_OWNER_REJECTED" in new_rows[rid]["mapping_grade"])
    check("rows", f"{rid} records the full pointwise action bank",
          "FULL_U6464_POINTWISE_ACTION_BANK_RANK14_NORMAL10_EXACT" in new_rows[rid]["mapping_grade"])
    check("rows", f"{rid} records the grade-five geometry correction",
          "GRADE5_SUPPORT_EXACT" in new_rows[rid]["mapping_grade"]
          and "FULL_SUPPORT_INERTIA_4_6" in new_rows[rid]["mapping_grade"])
...
migrations = [item for item in new["migrations"]
              if item.get("from_version") == "0.76" and item.get("to_version") == "0.77"]
...
check("registry", "the full real comparator and grade union are exact",
      registry["exact_results"]["full_real_dimension"] == 16384
      and registry["exact_results"]["seed_grade_union"] == {"1": 14, "2": 59, "5": 476})
...
check("registry", "raw and observed images are nondegenerate and indefinite",
      ...
      and registry["exact_results"]["observed_normal_gram_inertia"] == [4, 6, 0])
check("registry", "full-support row fingerprint is frozen",
      registry["exact_results"]["seed_row_supports"] == [42, 60, 46, 64, 47, 62, 46, 66, 47, 66, 47, 62, 53, 58])
check("registry", "endpoint orientation remains explicit",
      registry["exact_results"]["endpoint_orientation_opposite"] is True)
check("queue", "rank one is global bundle and physical observation",
      "global adjoint-bundle" in new["next_work_queue"][0]["why"]
      and "physical observation" in new["next_work_queue"][0]["why"])
check("scope", "no constraint-accounting movement",
      set(registry["constraint_fence"].values()) <= {0, "UNUSED"})
check("scope", "global bundle, BFV and common domain remain fenced",
      any("adjoint-bundle" in item for item in registry["boundary"])
