#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K516."""
...
spec = importlib.util.spec_from_file_location(
    "k516_probe_target", HERE / "k516_order_ten_face_shards_025_032.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K516")
K516 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K516)
...
def checks(payload):
    ...
    return [
        ...
        fixed["first_shard_id"] == "order10-face-025",
        fixed["last_shard_id"] == "order10-face-032",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"] == K516.K515.K514.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 132,
        decision["unexecuted_remaining_face_programs"] == 784,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 783),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K516.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K516 controls: {sum(controls)}/{len(controls)}; "
