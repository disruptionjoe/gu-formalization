#!/usr/bin/env python3
"""Execute K511 shards 057--064 through the certified K519/K508 backend."""
...
OUTPUT = ROOT / "lab/process/k520-order-ten-face-shards-057-064.json"
...
K519 = load("k519_for_k520", "k519_order_ten_face_shards_049_056.py")
K519.K518.K517.K516.K515.K514.SHARD_START = SHARD_START
K519.K518.K517.K516.K515.K514.SHARD_STOP = SHARD_STOP


def build() -> dict:
    payload = K519.build()
    payload["result_id"] = "K520-ORDER-TEN-FACE-SHARDS-057-064"
    payload["created"] = "2026-09-26"
    payload["decision"] = {
        "shards_057_through_064_released": True,
        "cumulative_K511_shards_executed_including_K512": 65,
        "cumulative_remaining_face_programs_executed": 260,
        "unexecuted_remaining_face_programs": 656,
        ...
        "next_exact_input": (
            "Continue K511 at shard 065 under serialized resource control, then "
    ...
    payload["claim_ceiling"] = (
        ...
        "K511 shards 057 through 064, across 224 positive-width normal cells. "
        "Together with K512--K519 this completes 260 of the 916 K508-omitted "
        "programs. It is not the other 656 programs, a complete face bank, "
...
def validate(payload: dict) -> None:
    ...
    if actual != expected:
        raise AssertionError("K520 fixed census changed")
    required = (
    ...
    if not all(bank[key] for key in required):
        raise AssertionError("K520 backend evidence failed")
    ...
    if actual_program_ids != expected_program_ids:
        raise AssertionError("K520 stored program identity changed")
    if bank["bank_sha256"] != K519.K518.K517.K516.K515.K514.K508.digest(rows):
        raise AssertionError("K520 stored bank digest changed")
    stored_uses = sum(
    ...
    if bank["divided_difference_operation_uses"] != stored_uses:
        raise AssertionError("K520 operation census changed")
    direct_ids = [row["program_id"] for row in payload["direct_overlap_controls"]]
    if direct_ids != expected_program_ids:
        raise AssertionError("K520 direct-control identity changed")
    for stored, expected_shard in zip(
        ...
        if (
            ...
            raise AssertionError("K520 stored shard provenance changed")
        shard_rows = [row for row in rows if row["shard_id"] == stored["shard_id"]]
        if stored["program_rows_sha256"] != K519.K518.K517.K516.K515.K514.K508.digest(shard_rows):
            raise AssertionError("K520 stored shard digest changed")
    if not decision["shards_057_through_064_released"]:
        raise AssertionError("K520 release boundary changed")
    if decision["cumulative_remaining_face_programs_executed"] != 260:
        raise AssertionError("K520 cumulative census changed")
    if decision["unexecuted_remaining_face_programs"] != 656:
        raise AssertionError("K520 remainder census changed")
    forbidden = (
    ...
    if any(decision[key] for key in forbidden):
        raise AssertionError("K520 overclaimed the order-ten cover")
    if payload["source_and_ledger_effect"] != "none":
        raise AssertionError("K520 moved source or ledger state")
    if not all(
        math.isfinite(float(cell["complete_second_derivative_abs_upper"]))
        for row in rows
        for cell in row["cells"]
    ):
        raise AssertionError("K520 stored nonfinite cell")
