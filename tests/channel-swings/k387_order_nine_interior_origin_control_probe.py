#!/usr/bin/env python3
"""Deterministic replay, finite-difference check and hostile controls for K387."""
...
PRODUCER = Path(__file__).with_name("k387_order_nine_interior_origin_control.py")
STORED = ROOT / "lab/process/k387-order-nine-interior-origin-control.json"


def load():
    spec = importlib.util.spec_from_file_location("k387_probe_target", PRODUCER)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load K387 producer")
...
def main() -> int:
    ...
    if stored != module.build():
        raise AssertionError("deterministic K387 replay differs")
    ...
    checks = [
        ...
        all(row["all_20_group_intervals_sha256"].startswith("sha256:") for row in rows),
        ...
        stored["control_summary"]["K383_s1_node_second_derivative_replayed"],
        finite_difference_relative_error < 2e-8,
        not stored["control_summary"]["complete_hybrid_integrals_emitted"],
    ]
    if not all(checks):
        raise AssertionError(f"K387 control failed; finite-difference relative error={finite_difference_relative_error}")
    mutations = [
        lambda p: p["fixed_control"].__setitem__("hybrid_terms", 19),
        lambda p: p["fixed_control"].__setitem__("ordered_directional_entries_per_axis", 2368),
        lambda p: p["hybrid_interior_reference_bank"].pop(),
        lambda p: p["hybrid_interior_reference_bank"][0].__setitem__("moving_dimension", 19),
        ...
        lambda p: p["control_summary"].__setitem__("all_20_hybrid_reference_intervals_finite", False),
        ...
        lambda p: p["decision"].__setitem__("complete_order_nine_remainder_emitted", True),
    ]
    rejected = 0
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K387 hostile rejection changed: {rejected}/{len(mutations)}")
    print(f"K387 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
