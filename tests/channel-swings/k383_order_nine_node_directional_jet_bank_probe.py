#!/usr/bin/env python3
"""Deterministic replay and independent finite-difference controls for K383."""
...
PRODUCER = Path(__file__).with_name("k383_order_nine_node_directional_jet_bank.py")
STORED = ROOT / "lab/process/k383-order-nine-node-directional-jet-bank.json"


def load():
    spec = importlib.util.spec_from_file_location("k383_probe_target", PRODUCER)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load K383 producer")
...
def raw_complete_value(module, raw: list[arb]) -> arb:
    cumulative_s = {position: sum(raw[position - 1 : 10], arb(0)) for position in range(1, 11)}
    cumulative_v = {position: sum(raw[10 + position - 1 :], arb(0)) for position in range(1, 11)}
    ...
    def gram(left, right):
        ...
        left_occurrences = module.K382_MODULE.occurrences(left)
        right_occurrences = module.K382_MODULE.occurrences(right)
...
def main() -> int:
    ...
    if stored != module.build():
        raise AssertionError("deterministic K383 replay differs")
    rows = {row["axis"]: row for row in stored["axis_node_jets"]}
    raw = [arb(1) / 256 for _ in range(20)]
    h = arb("1e-8")
    independent_checks = []
    independent_rows = []
    for axis in ["s1", "s5", "s10", "v1", "v5", "v10"]:
    ...
    checks = [
        len(rows) == 20,
        stored["fixed_control"]["axis_entry_evaluations"] == 89600,
        stored["bank_summary"]["all_axis_jets_finite"],
        stored["bank_summary"]["complete_value_coefficient_replays_K381"],
        ...
        not stored["bank_summary"]["complete_order_nine_remainder_emitted"],
        all(independent_checks),
    ]
    if not all(checks):
        raise AssertionError(f"K383 control failed; independent relative errors={independent_rows}")
    mutations = [
        ...
        lambda p: p["fixed_control"].__setitem__("symmetric_node_upper_triangle_entries", 2367),
        lambda p: p["fixed_control"].__setitem__("ordered_directional_entries_per_axis", 4479),
        lambda p: p["fixed_control"].__setitem__("axis_entry_evaluations", 89599),
        lambda p: p["axis_node_jets"].pop(),
        lambda p: p["bank_summary"].__setitem__("all_axis_jets_finite", False),
        lambda p: p["bank_summary"].__setitem__("complete_value_coefficient_replays_K381", False),
        ...
        lambda p: p["bank_summary"].__setitem__("complete_order_nine_remainder_emitted", True),
        lambda p: p["release_test"].__setitem__("all_20_axes_evaluated", False),
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K383 hostile rejection changed: {rejected}/{len(mutations)}")
    print(f"K383 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
