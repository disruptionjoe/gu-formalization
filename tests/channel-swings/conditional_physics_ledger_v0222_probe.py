#!/usr/bin/env python3
"""Append-only and accounting checks for conditional physics ledger v0.222."""
...
OLD_PATH = ROOT / "lab/process/conditional-physics-ledger-v0.221.json"
NEW_PATH = ROOT / "lab/process/conditional-physics-ledger-v0.222.json"
...
check("schema", new["schema_version"] == "0.222")
check("predecessor", new["predecessor"].endswith("v0.221.json"))
check("status", new["status"] == "CURRENT_APPEND_ONLY_LEDGER_V0_222")
check("run_owner", new["updated_by"] == "RUN-20260812-215541-gu-i2b-contact-euler-hodge-adapter")
...
check("current_evidence", all(new_rows[row]["evidence"] == "selected-k77-i2b-contact-euler-hodge-adapter-2026-08-12.md" for row in expected_rows))
...
check("migration_versions", all((entry["from_version"], entry["to_version"]) == ("0.221", "0.222") for entry in new_migrations))

observer = new["residue"]["conditional_observer_time_reduction"]
check("hodge_principal_successor", "E3_RADIAL_EULER_ROW" in new_rows["RA-E1"]["mapping_grade"])
check("radial_e3_open", "MOVING_QB_METRIC_SECTION_SHIAB_GAUGE_CORRECTIONS" in observer["open"])
check("source_return", "HODGE_PRINCIPAL_ACTIVE_INTERSECTION" in new["source_return"])
check("frontier", new["frontier_delta"] == {"headline_delta": "NONE", "conditions_closed": 4, "conditions_opened": 1, "remaining_named_conditions": 1})
...
check("no_nonzero_stationary_e3", "NONZERO_STATIONARY_E3_SELECTED" not in json.dumps(new), planted=True)
check("no_global_hodge_bundle", "GLOBAL_HODGE_CONTACT_SUBBUNDLE_PROVED" not in json.dumps(new), planted=True)
