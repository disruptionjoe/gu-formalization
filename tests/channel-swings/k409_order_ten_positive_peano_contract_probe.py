#!/usr/bin/env python3
"""Deterministic replay and hostile controls for K409."""
...
PRODUCER = Path(__file__).with_name("k409_order_ten_positive_peano_contract.py")
STORED = ROOT / "lab/process/k409-order-ten-positive-peano-contract.json"


def load():
    spec = importlib.util.spec_from_file_location("k409_probe_target", PRODUCER)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load K409 producer")
...
def main() -> int:
    ...
    if stored != module.build():
        raise AssertionError("deterministic K409 replay differs")
    ...
    checks = [
        ...
        len(rows) == 22,
        all(row["preceding_axes_fixed_at_node"] + row["following_axes_integrated_exactly"] == 21 for row in rows),
        not stored["tensor_telescoping"]["mixed_derivatives_required"],
        stored["tensor_telescoping"]["pure_second_directional_terms"] == 22,
        stored["tensor_telescoping"]["node_jet_substitution_for_global_integral_forbidden"],
        not stored["decision"]["complete_twenty_two_axis_remainder_numerically_bounded"],
        not stored["decision"]["complete_order_ten_integral_emitted"],
        not stored["decision"]["rank_five_order_ten_global_cover_released"],
    ]
    if not all(checks):
        raise AssertionError("K409 control failed")
    mutations = [
        ...
        lambda p: p["tensor_telescoping"].__setitem__("pure_second_directional_terms", 21),
        ...
        lambda p: p["decision"].__setitem__("complete_twenty_two_axis_remainder_formula_emitted", False),
        lambda p: p["decision"].__setitem__("complete_twenty_two_axis_remainder_numerically_bounded", True),
        lambda p: p["decision"].__setitem__("complete_order_ten_integral_emitted", True),
        lambda p: p["decision"].__setitem__("rank_five_order_ten_global_cover_released", True),
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K409 hostile rejection changed: {rejected}/{len(mutations)}")
    print(f"K409 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
