...
ledger = json.loads((ROOT / "lab/process/conditional-physics-ledger-v0.199.json").read_text())

assert ledger["schema_version"] == "0.199"
assert ledger["predecessor"].endswith("v0.198.json")
...
assert len([m for m in ledger["migrations"] if m["to_version"] == "0.199"]) == 3
assert len(ledger["wave_row_dispositions"]) == 3
rows = {row["id"]: row for row in ledger["rows"]}
for row in ("RA-E1", "RA-E3", "LT-SM6"):
    assert "selected-k77-moving-hq-eddy-quartic" in rows[row]["evidence"]
assert "phase-corrected" in rows["RA-E1"]["distance"]
assert "512(sum h_a^2)^2" in rows["LT-SM6"]["distance"]
...
print("PASS: ledger v0.199 preserves headline accounting and migrates exactly three distances to the selected physical action/vacuum gate.")
