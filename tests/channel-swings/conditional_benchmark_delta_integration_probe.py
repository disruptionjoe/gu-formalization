...
def main() -> int:
    ...
    cases = [
        ...
        ("predecessor-verdict-drift", lambda d: (lambda row: row.update(verdict="SAME" if row.get("verdict") != "SAME" else "NEEDS"))(next((row for row in d["rows"] if row["id"] == "LT-GR1"), {}))),
        ("sm-grant-loss", lambda d: next((row for row in d["rows"] if row["id"] == "LT-SM8"), {}).get("context", {}).update(grant="G5")),
        ("owner-c-loss", lambda d: next((row for row in d["rows"] if row["id"] == "LT-GR2"), {}).get("context", {}).update(note="none")),
