#!/usr/bin/env python3
"""Execute K511 shards 033--040 through the certified K516/K508 backend."""
...
K516 = load("k516_for_k517", "k516_order_ten_face_shards_025_032.py")
K516.K515.K514.SHARD_START = SHARD_START
K516.K515.K514.SHARD_STOP = SHARD_STOP
...
def build() -> dict:
    ...
    payload["decision"] = {
        "shards_033_through_040_released": True,
        "cumulative_K511_shards_executed_including_K512": 41,
        "cumulative_remaining_face_programs_executed": 164,
        "unexecuted_remaining_face_programs": 752,
        ...
        "next_exact_input": (
            "Continue K511 at shard 041 under serialized resource control, then "
    ...
    payload["claim_ceiling"] = (
        ...
        "K511 shards 033 through 040, across 224 positive-width normal cells. "
        "Together with K512--K516 this completes 164 of the 916 K508-omitted "
        "programs. It is not the other 752 programs, a complete face bank, "
...
def validate(payload: dict) -> None:
    ...
    if actual != expected:
        raise AssertionError("K517 fixed census changed")
    required = (
    ...
    if not all(bank[key] for key in required):
        raise AssertionError("K517 backend evidence failed")
    ...
    if actual_program_ids != expected_program_ids:
        raise AssertionError("K517 stored program identity changed")
    if bank["bank_sha256"] != K516.K515.K514.K508.digest(rows):
        raise AssertionError("K517 stored bank digest changed")
    stored_uses = sum(
    ...
    if bank["divided_difference_operation_uses"] != stored_uses:
        raise AssertionError("K517 operation census changed")
    direct_ids = [row["program_id"] for row in payload["direct_overlap_controls"]]
    if direct_ids != expected_program_ids:
        raise AssertionError("K517 direct-control identity changed")
    for stored, expected_shard in zip(
        ...
        if (
            ...
            raise AssertionError("K517 stored shard provenance changed")
        shard_rows = [row for row in rows if row["shard_id"] == stored["shard_id"]]
        if stored["program_rows_sha256"] != K516.K515.K514.K508.digest(shard_rows):
            raise AssertionError("K517 stored shard digest changed")
    if not decision["shards_033_through_040_released"]:
        raise AssertionError("K517 release boundary changed")
    if decision["cumulative_remaining_face_programs_executed"] != 164:
        raise AssertionError("K517 cumulative census changed")
    if decision["unexecuted_remaining_face_programs"] != 752:
        raise AssertionError("K517 remainder census changed")
    forbidden = (
    ...
    if any(decision[key] for key in forbidden):
        raise AssertionError("K517 overclaimed the order-ten cover")
    if payload["source_and_ledger_effect"] != "none":
        raise AssertionError("K517 moved source or ledger state")
    if not all(
        math.isfinite(float(cell["complete_second_derivative_abs_upper"]))
        for row in rows
        for cell in row["cells"]
    ):
        raise AssertionError("K517 stored nonfinite cell")
