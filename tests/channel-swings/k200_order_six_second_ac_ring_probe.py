#!/usr/bin/env python3
"""Probe and hostile self-test for the K200 second enlarged a/c square."""
...
SOLVER_PATH = ROOT / "tests/channel-swings/k200_order_six_second_ac_ring.py"
MANIFEST_PATH = ROOT / "lab/process/k200-order-six-second-ac-ring-wave.json"
...
SOLVER = load_module("k200_solver_for_probe", SOLVER_PATH)


def checks(data: dict[str, Any]) -> list[tuple[str, bool]]:
    fixed = data["fixed_control"]
    square = data["certified_second_enlarged_ac_square"]
    ring = square["size_three_row_chart_second_ring_cover"]
    ...
    return [
        ...
        ("predecessor", fixed["predecessor_manifest"].endswith("k199-order-six-first-ac-corners-wave.json")),
        ("inner", fixed["k199_inner_a_c_range"] == ["63/128", "65/128"]),
        ("outer", fixed["second_expanded_a_c_range"] == ["503/1024", "521/1024"]),
        ...
        ("size", ring["size"] == 3),
        ("ring ranges", ring["inner_a_c_range"] == ["63/128", "65/128"] and ring["outer_a_c_range"] == ["503/1024", "521/1024"]),
        ("six ranges", set(packet_ranges) == expected_packet_ids),
        ("lower side", packet_ranges["lower_side"]["a"] == ["503/1024", "63/128"] and packet_ranges["lower_side"]["c"] == ["63/128", "65/128"]),
        ("upper side", packet_ranges["upper_side"]["a"] == ["65/128", "521/1024"] and packet_ranges["upper_side"]["c"] == ["63/128", "65/128"]),
        ("lower lower", packet_ranges["lower_lower"]["a"] == ["503/1024", "63/128"] and packet_ranges["lower_lower"]["c"] == ["503/1024", "63/128"]),
        ("upper upper", packet_ranges["upper_upper"]["a"] == ["65/128", "521/1024"] and packet_ranges["upper_upper"]["c"] == ["65/128", "521/1024"]),
        ...
        ("K199 inherited", square["k199_first_enlarged_square_inherited"]),
        ("six packets", square["two_side_strips_and_four_corners_covered"]),
        ("join", square["exact_join_to_k199_square"]),
        ("square complete", square["second_enlarged_a_c_square_complete"]),
        ...
        ("decision sides", decision["two_one_step_side_strips_certified"]),
        ("decision corners", decision["four_one_step_corner_boxes_certified"]),
        ("decision join", decision["k199_square_join_preserved"]),
        ("decision square", decision["second_enlarged_a_c_square_certified"]),
        ("decision complement", not decision["farther_a_c_complement_covered"]),
        ("square release", release["second_enlarged_a_c_square_outwardly_certified"]),
        ...
        ("downstream withheld", not any(value for key, value in release.items() if key not in {
            ...
            "second_enlarged_a_c_square_outwardly_certified",
...
MUTATIONS = [
    mutate(("fixed_control", "second_expanded_a_c_range"), ["63/128", "65/128"]),
    mutate(("fixed_control", "q_subdivisions"), 16),
    mutate(("certified_second_enlarged_ac_square", "size_three_row_chart_second_ring_cover", "accepted_q_tiles"), 31),
    mutate(("certified_second_enlarged_ac_square", "size_three_row_chart_second_ring_cover", "accepted_ring_tiles"), 191),
    mutate(("certified_second_enlarged_ac_square", "size_three_row_chart_second_ring_cover", "unique_outward_cells"), 786431),
    mutate(("certified_second_enlarged_ac_square", "size_three_row_chart_second_ring_cover", "minimum_R_lower"), -1.0),
    mutate(("certified_second_enlarged_ac_square", "size_three_row_chart_second_ring_cover", "q_tiles_contiguous"), False),
    mutate(("certified_second_enlarged_ac_square", "size_three_row_chart_second_ring_cover", "six_packets_exactly_cover_new_ring_with_transpose"), False),
    mutate(("certified_second_enlarged_ac_square", "size_three_column_chart_side_strips_are_exact_transpose"), False),
    mutate(("certified_second_enlarged_ac_square", "k199_first_enlarged_square_inherited"), False),
    mutate(("certified_second_enlarged_ac_square", "two_side_strips_and_four_corners_covered"), False),
    mutate(("certified_second_enlarged_ac_square", "exact_join_to_k199_square"), False),
    mutate(("certified_second_enlarged_ac_square", "second_enlarged_a_c_square_complete"), False),
    mutate(("certified_second_enlarged_ac_square", "complete_a_c_domain_covered"), True),
    ...
    mutate(("decision", "two_one_step_side_strips_certified"), False),
    mutate(("decision", "four_one_step_corner_boxes_certified"), False),
    mutate(("decision", "second_enlarged_a_c_square_certified"), False),
    mutate(("decision", "farther_a_c_complement_covered"), True),
    mutate(("release_test", "second_enlarged_a_c_square_outwardly_certified"), False),
