#!/usr/bin/env python3
"""Certify the zero-safe radial scaling needed by the K384 Peano terms."""
...
OUTPUT = ROOT / "lab/process/k385-order-nine-zero-safe-radial-contract.json"
...
def build() -> dict[str, Any]:
    ...
    order_row = next(row for row in k279["orders"] if row["order"] == 9)
    if order_row["small_rho_power"] != 8:
        raise AssertionError("K279 order-nine origin power changed")
    if k380["fixed_control"]["positive_time_variables"] != 20:
        raise AssertionError("K380 axis census changed")
    if k384["one_axis_identity"]["kernel_zero_order_at_origin"] != 2:
        raise AssertionError("K384 Peano zero order changed")
    ...
    raw_kernel_count = 9 + 2
    derivative_cost = 2
    radial_jacobian_degree = 20 - 1
    peano_gain = 2
    raw_second_degree = -(raw_kernel_count + derivative_cost)
    final_degree = radial_jacobian_degree + raw_second_degree + peano_gain
    if final_degree != 8:
        raise AssertionError("order-nine radial degree does not replay K279")

    return {
        "schema_version": "1.0",
        "result_id": "K385-ORDER-NINE-ZERO-SAFE-RADIAL-CONTRACT",
        "created": "2026-09-23",
        ...
        "all_zero_radial_ledger": {
            ...
            "twenty_axis_radial_jacobian_degree": radial_jacobian_degree,
            "active_peano_zero_gain": peano_gain,
            "final_radial_degree": final_degree,
            "K279_order_nine_small_rho_power_replayed": final_degree == order_row["small_rho_power"],
            "origin_integrable": final_degree > -1,
            "scope": "the all-twenty-axis origin of the first K384 hybrid; later hybrids have fixed positive preceding nodes and require the K386 partial-face ledger",
        },
        "decision": {
            ...
            "next_exact_input": "compile the exact K380 zero and coalescence masks reachable in every K384 hybrid domain, then exercise the complete coherent derivative evaluator away from those faces",
        },
        "release_test": {
            ...
            "radial_degree_eight_replayed": final_degree == 8,
            "raw_zero_evaluation_not_used": True,
            "complete_order_nine_remainder_not_overclaimed": True,
            "native_K152_interval_not_emitted": True,
        },
        "ledger_effect": k384["ledger_effect"],
        "source_routing": k384["source_routing"],
        "claim_ceiling": "Exact rational zero-inclusive envelopes for the value, first and second derivatives of 2*K1, a global K_256(t)<=t^2/2 Peano majorant, and the complete degree-eight all-zero radial integrability ledger for the first order-nine hybrid. Partial coalescence-face preconditioners, recursive whole-domain coverage, all twenty numerical hybrid integrals, the complete order-nine remainder and integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public and physical posture remain open.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["order"], fixed["raw_time_axes"], fixed["maximum_active_axis_derivative_order"], fixed["raw_kernel_factors_per_gram_entry"]) != (9, 20, 2, 11):
        raise AssertionError("K385 fixed census changed")
    ...
    if bank["raw_Bessel_evaluation_at_zero_used"] or not bank["all_bounds_exact_rational"] or bank["continuous_zero_limits"] != ["2", "2", "4"]:
        raise AssertionError("K385 zero-safe bank changed")
    if not all(row["contained"] for row in payload["peano_majorant"]["controls"]):
        raise AssertionError("K385 Peano controls failed")
    ledger = payload["all_zero_radial_ledger"]
    if ledger["final_radial_degree"] != 8 or not ledger["K279_order_nine_small_rho_power_replayed"] or not ledger["origin_integrable"]:
        raise AssertionError("K385 radial ledger changed")
    ...
    if not decision["all_zero_origin_integrability_closed"] or decision["partial_face_preconditioners_emitted"] or decision["complete_hybrid_integrals_emitted"]:
        raise AssertionError("K385 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K385 release test failed")
