#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K520."""
...
spec = importlib.util.spec_from_file_location(
    "k520_probe_target", HERE / "k520_order_ten_face_shards_057_064.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K520")
K520 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K520)
...
def checks(payload):
    ...
    return [
        ...
        fixed["first_shard_id"] == "order10-face-057",
        fixed["last_shard_id"] == "order10-face-064",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"]
        == K520.K519.K518.K517.K516.K515.K514.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 260,
        decision["unexecuted_remaining_face_programs"] == 656,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 655),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K520.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K520 controls: {sum(controls)}/{len(controls)}; "
