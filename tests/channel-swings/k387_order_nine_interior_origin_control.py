#!/usr/bin/env python3
"""Exercise the complete K382 derivative evaluator on all hybrid interiors.

This is a numerical control for the atlas architecture, not a whole-domain
enclosure.  Preceding K384 axes are fixed at 1/256.  The active and following
...
K383 = ROOT / "lab/process/k383-order-nine-node-directional-jet-bank.json"
...
OUTPUT = ROOT / "lab/process/k387-order-nine-interior-origin-control.json"
...
K383_BACKEND = load_module(K383_MODULE, "k383_for_k387")
...
def cumulative(raw: dict[str, arb]) -> tuple[dict[int, arb], dict[int, arb]]:
    left = {position: sum((raw[f"s{i}"] for i in range(position, 11)), arb(0)) for position in range(1, 11)}
    right = {position: sum((raw[f"v{i}"] for i in range(position, 11)), arb(0)) for position in range(1, 11)}
    return left, right
...
def complete_second(axis: str, raw: dict[str, arb]) -> tuple[arb, list[dict[str, str]]]:
    ...
    for (seed, signature), terms in K383_BACKEND.group_terms().items():
        ...
        for left in terms:
            for right in terms:
                jet = K383_BACKEND.gram_jet(left, right, cumulative_s, cumulative_v, axis)
...
def complete_value(raw: dict[str, arb]) -> arb:
    cumulative_s, cumulative_v = cumulative(raw)
    total = arb(0)
    for terms in K383_BACKEND.group_terms().values():
        for left in terms:
            for right in terms:
                jet = K383_BACKEND.gram_jet(left, right, cumulative_s, cumulative_v, "s1")
                multiplier = int(left["exact_operator_coefficient"]) * int(right["exact_operator_coefficient"])
...
def build() -> dict[str, Any]:
    k383 = json.loads(K383.read_text())
    k385 = json.loads(K385.read_text())
    k386 = json.loads(K386.read_text())
    if k386["fixed_control"]["native_axes"] != list(AXES):
        raise AssertionError("K386 native axis order changed")
    if not k385["decision"]["all_zero_origin_integrability_closed"]:
        raise AssertionError("K385 radial contract unavailable")
    ...
    for radius in RAY_RADII:
        raw = {axis: arb(q(radius / 20)) for axis in AXES}
        second, groups = complete_second("s1", raw)
        scaled = abs(second) * arb(q(radius**13))
        radial_coefficient = scaled / (2 * 20**2)
        ray_controls.append({
            "total_radius": q(radius),
            "raw_time_per_axis": q(radius / 20),
            "raw_second_derivative_lower": lower_text(second),
            "raw_second_derivative_upper": upper_text(second),
            "rho13_abs_second_upper": abs_upper_text(scaled),
            "rho_minus8_peano_radial_coefficient_upper": abs_upper_text(radial_coefficient),
            "all_20_group_intervals_sha256": digest(groups),
        })

    local = next(row for row in k383["axis_node_jets"] if row["axis"] == "s1")
    ...
    if not node_replayed:
        raise AssertionError("K387 failed to replay the K383 s1 node control")

    return {
        "schema_version": "1.0",
        "result_id": "K387-ORDER-NINE-INTERIOR-ORIGIN-CONTROL",
        "created": "2026-09-23",
        "classification": "INTERNAL_NUMERICAL_CONTROL_ONLY",
        "direction": "observed_to_native",
        "fixed_control": {
            "predecessor_manifests": [
                "lab/process/k383-order-nine-node-directional-jet-bank.json",
        ...
        "composition_contract": {
            ...
            "K385_quadratic_peano_majorant_used": True,
            "raw_Bessel_evaluation_at_zero_used": False,
            "reference_bank_is_not_a_projective_cell_cover": True,
        },
        "control_summary": {
            "all_20_hybrid_reference_intervals_finite": all(math.isfinite(float(row["complete_second_derivative_abs_upper"])) for row in rows),
            "all_reference_arguments_strictly_positive": all(float(row["minimum_cumulative_argument_lower"]) > 0 for row in rows),
            "all_20_group_digests_present": all(row["all_20_group_intervals_sha256"].startswith("sha256:") for row in rows),
            "K383_s1_node_second_derivative_replayed": node_replayed,
            "all_zero_ray_scaled_controls_finite": all(math.isfinite(float(row["rho13_abs_second_upper"])) for row in ray_controls),
        ...
        "decision": {
            ...
            "K383_local_control_replayed": True,
            "proper_face_evaluator_released": False,
            "complete_order_nine_remainder_emitted": False,
            "next_exact_input": "construct mask-native scaled determinant preconditioners on K386's reachable proper faces, join them to recursive positive-interior cells and analytic radial tails, then integrate all twenty K384 kernels",
        },
        "release_test": {
            "twenty_hybrid_rows_present": len(rows) == 20,
            "all_89600_ordered_axis_entry_evaluations_covered": 20 * 4480 == 89600,
            ...
            "K383_node_control_replayed": node_replayed,
        ...
        "ledger_effect": k386["ledger_effect"],
        "source_routing": k386["source_routing"],
        "claim_ceiling": "Rigorous 180-digit Arb execution of the complete ordered determinant and coherent-group second-derivative evaluator on a positive barycentric radial reference cell for all twenty K384 hybrid domains, plus three all-zero barycentric rays with the K385 rho^13 scaling and a replay of K383's s1 node control. The reference bank is not a projective cover. Proper-face preconditioners, recursive whole-domain coverage, analytic tails, numerical hybrid integrals, the complete order-nine remainder and integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public and physical posture remain open.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["hybrid_terms"], fixed["paths"], fixed["coherent_groups"], fixed["ordered_directional_entries_per_axis"], fixed["axis_entry_evaluations"]) != (20, 256, 20, 4480, 89600):
        raise AssertionError("K387 fixed census changed")
    rows = payload["hybrid_interior_reference_bank"]
    if len(rows) != 20 or [row["moving_dimension"] for row in rows] != list(range(20, 0, -1)):
        raise AssertionError("K387 hybrid reference bank changed")
    ...
    if contract["raw_Bessel_evaluation_at_zero_used"] or not contract["all_ordered_orientations_retained"] or not contract["reference_bank_is_not_a_projective_cell_cover"]:
        raise AssertionError("K387 composition boundary changed")
    summary = payload["control_summary"]
    if not summary["all_20_hybrid_reference_intervals_finite"] or not summary["all_reference_arguments_strictly_positive"] or not summary["K383_s1_node_second_derivative_replayed"]:
        raise AssertionError("K387 controls failed")
    if summary["proper_face_preconditioners_complete"] or summary["recursive_whole_domain_cover_complete"] or summary["complete_hybrid_integrals_emitted"]:
        raise AssertionError("K387 overclaimed global coverage")
    decision = payload["decision"]
    if not decision["complete_coherent_hybrid_interior_evaluator_executed"] or not decision["all_zero_scaled_ray_control_executed"] or decision["proper_face_evaluator_released"] or decision["complete_order_nine_remainder_emitted"]:
        raise AssertionError("K387 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K387 release test failed")
