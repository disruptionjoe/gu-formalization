#!/usr/bin/env python3
"""Deterministic replay and independent finite-difference controls for K408."""
...
PRODUCER = Path(__file__).with_name("k408_order_ten_node_directional_jet_bank.py")
STORED = ROOT / "lab/process/k408-order-ten-node-directional-jet-bank.json"


def load():
    spec = importlib.util.spec_from_file_location("k408_probe_target", PRODUCER)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load K408 producer")
...
def raw_complete_value(module, raw: list[arb]) -> arb:
    cumulative_s = {position: sum(raw[position - 1 : 11], arb(0)) for position in range(1, 12)}
    cumulative_v = {position: sum(raw[11 + position - 1 :], arb(0)) for position in range(1, 12)}
    ...
    def gram(left, right):
        ...
        left_occurrences = module.K407_MODULE.occurrences(left)
        right_occurrences = module.K407_MODULE.occurrences(right)
...
def main() -> int:
    ...
    if stored != module.build():
        raise AssertionError("deterministic K408 replay differs")
    rows = {row["axis"]: row for row in stored["axis_node_jets"]}
    raw = [arb(1) / 256 for _ in range(22)]
    h = arb("1e-8")
    independent_checks = []
    independent_rows = []
    for axis in ["s1", "s6", "s11", "v1", "v6", "v11"]:
    ...
    checks = [
        len(rows) == 22,
        stored["fixed_control"]["axis_entry_evaluations"] == 292600,
        stored["bank_summary"]["all_axis_jets_finite"],
        stored["bank_summary"]["complete_value_coefficient_replays_K406"],
        ...
        not stored["bank_summary"]["complete_order_ten_remainder_emitted"],
        all(independent_checks),
    ]
    if not all(checks):
        raise AssertionError(f"K408 control failed; independent relative errors={independent_rows}")
    mutations = [
        ...
        lambda p: p["fixed_control"].__setitem__("symmetric_node_upper_triangle_entries", 6889),
        lambda p: p["fixed_control"].__setitem__("ordered_directional_entries_per_axis", 13299),
        lambda p: p["fixed_control"].__setitem__("axis_entry_evaluations", 292599),
        lambda p: p["axis_node_jets"].pop(),
        lambda p: p["bank_summary"].__setitem__("all_axis_jets_finite", False),
        lambda p: p["bank_summary"].__setitem__("complete_value_coefficient_replays_K406", False),
        ...
        lambda p: p["bank_summary"].__setitem__("complete_order_ten_remainder_emitted", True),
        lambda p: p["release_test"].__setitem__("all_22_axes_evaluated", False),
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K408 hostile rejection changed: {rejected}/{len(mutations)}")
    print(f"K408 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
