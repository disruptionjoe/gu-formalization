#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K515."""
...
spec = importlib.util.spec_from_file_location(
    "k515_probe_target", HERE / "k515_order_ten_face_shards_017_024.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K515")
K515 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K515)
...
def checks(payload):
    ...
    return [
        ...
        fixed["first_shard_id"] == "order10-face-017",
        fixed["last_shard_id"] == "order10-face-024",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"] == K515.K514.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 100,
        decision["unexecuted_remaining_face_programs"] == 816,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 815),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K515.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K515 controls: {sum(controls)}/{len(controls)}; "
