#!/usr/bin/env python3
r"""Resumable full-evaluator coverage: second uncovered size-six S3 orbit.

The byte-pinned predecessor commit certifies exactly 4/380 canonical
...
The durable result is at most 5/380.  It does not execute the preregistered
...
PREDECESSOR = CHANNEL / "pw2fr2b2b2i2_resumable_first_size6_full_evaluator_probe.py"
PREDECESSOR_REVISION = "40e93a9fa3a0e091b81be01fbbc73fd24608b0d8"
PREDECESSOR_LEDGER_PATH = "lab/process/pw2fr2b2b2i2-resumable-full-evaluator-coverage.json"
PREDECESSOR_PROBE_SHA256 = "bf9ece02d5a61062b463729e809ce24148587128fbb3868f487102427d109f1a"
PREDECESSOR_LEDGER_SHA256 = "49f9ad8c7f66812910cb6b700b6982b03ed44cbec65e4a04ec1770d2b0766fdd"


def load_predecessor():
    spec = spec_from_file_location("gu_i2_resumable_first_size6", PREDECESSOR)
...
EXPECTED_REPRESENTATIVE = ((0, 0), (0, 0, 2, 2))
...
def main() -> int:
    print(
        "PW2F-R2B2B2I2 RESUMABLE SECOND-UNCOVERED SIZE6 FULL-EVALUATOR CERTIFICATE",
        flush=True,
    )
    failures: list[str] = []
    ...
    exact(
        "durable 4/380 predecessor probe and committed ledger bytes are pinned",
        dependency_checks == 2,
    ...
    heldout_orbits = tuple(
        ...
        for seed in S.DENSE_HELDOUT_SEEDS
    )
    ...
    exact("durable predecessor ledger resolves to exactly four representatives", len(predecessor_covered) == 4)
    ...
    exact("six dense heldout seeds remain outside the 1,925-cell lattice", all(seed not in label_set for seed in S.DENSE_HELDOUT_SEEDS), "6/6")
    ...
    typed("one additional size-six orbit advances durable coverage to at most 5/380")
    ...
    reject("PLANT begin Green/Helmholtz from a third non-fixed orbit")
    ...
    print("representative_coverage=5/380")
    print("remaining_representatives=375/380")
    ...
    if failures:
        return 1
    print(
        "VERDICT: the second uncovered canonical representative and its "
        "complete six-cell orbit transport exactly under both S3 generators; "
        "durable coverage is 5/380 and the other 375 representatives remain open"
    )
