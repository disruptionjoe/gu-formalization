#!/usr/bin/env python3
"""Execute K388-preconditioned controls away from K392's equal-normal line."""
...
OUTPUT = ROOT / "lab/process/k396-order-nine-anisotropic-projective-controls.json"

ctx.dps = 180
ctx.threads = 1

spec = importlib.util.spec_from_file_location("k392_for_k396", K392_PRODUCER)
...
def raw_times(program: dict[str, Any], rho: arb, proportions: dict[str, Fraction]) -> dict[str, arb]:
    ...
    if set(proportions) != zeroed or sum(proportions.values(), start=Fraction(0)) != 1:
        raise AssertionError("K396 projective proportions changed")
    return {
...
def build() -> dict[str, Any]:
    ...
    codimensions = k394["fixed_control"]["reachable_codimensions"]
    ...
    for codimension in codimensions:
        ...
        try:
            second, groups, singular_histogram, confluent_histogram, operations = K392.complete_second(
                program, rho, singular, confluent
            )
        finally:
            K392.raw_times = original_raw_times
        direct, direct_groups = K392.K387_BACKEND.complete_second(program["axis"], raw)
        ...
        if not second.is_finite() or not direct.is_finite() or minimum_argument <= 0 or not overlap:
            raise AssertionError(f"K396 anisotropic control failed at codimension {codimension}")
        ...
        controls.append({
            ...
            "preconditioned_second_lower": K392.lower_text(second),
            "preconditioned_second_upper": K392.upper_text(second),
            "direct_second_lower": K392.lower_text(direct),
            "direct_second_upper": K392.upper_text(direct),
            ...
            "preconditioned_group_digest": K392.digest(groups),
            "direct_group_digest": K392.digest(direct_groups),
    ...
    return {
        "schema_version": "1.0",
        "result_id": "K396-ORDER-NINE-ANISOTROPIC-PROJECTIVE-CONTROLS",
        "created": "2026-09-23",
        "classification": "INTERNAL_NUMERICAL_CONTROL_ONLY",
        "direction": "observed_to_native",
        "fixed_control": {
            "predecessor_manifests": [str(K390.relative_to(ROOT)), str(K394.relative_to(ROOT)), str(K395.relative_to(ROOT))],
            ...
            "ordered_descriptor_control_evaluations": len(controls) * 4480,
        ...
        "release_test": {
            "exactly_14_controls": len(controls) == 14,
            "all_62720_ordered_descriptor_controls_covered": len(controls) * 4480 == 62_720,
        ...
        "claim_ceiling": "Rigorous 180-digit Arb execution of one non-equal positive rational projective direction for every reachable K390 codimension. All fourteen controls reuse K388's determinant-preserving preconditioners, retain 4,480 ordered descriptors and 20 coherent groups, have positive argument floors, remain finite and overlap the direct complete evaluator. These are point controls, not positive-width projective cells, zero-inclusive boundary majorants, a recursive interior cover, analytic tails, K384 hybrid integrals, the complete order-nine remainder/integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public or physical claim.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["selected_controls"], fixed["ordered_descriptors_per_control"], fixed["ordered_descriptor_control_evaluations"], fixed["coherent_groups_per_control"]) != (14, 4480, 62_720, 20):
        raise AssertionError("K396 control census changed")
    rows = payload["anisotropic_control_bank"]
    if len(rows) != 14 or [row["codimension"] for row in rows] != fixed["reachable_codimensions"]:
        raise AssertionError("K396 codimension bank changed")
    if any(not row["proportions_sum_to_one"] or not row["direction_is_not_equal_normal"] or float(row["minimum_cumulative_argument_lower"]) <= 0 or not row["preconditioned_direct_intervals_overlap"] or row["coherent_group_count"] != 20 or row["direct_coherent_group_count"] != 20 for row in rows):
        raise AssertionError("K396 anisotropic control changed")
    ...
    if contract["raw_Bessel_evaluation_at_zero_used"] or not contract["point_controls_are_not_positive_width_projective_cells"] or not contract["all_ordered_orientations_retained"]:
        raise AssertionError("K396 execution boundary changed")
    ...
    if not decision["anisotropic_projective_point_interface_executed"] or any(decision[key] for key in ("positive_width_projective_cells_evaluated", "zero_inclusive_projective_boundary_majorants_complete", "recursive_positive_interior_cover_complete", "complete_hybrid_integrals_emitted")):
        raise AssertionError("K396 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K396 release test failed")
