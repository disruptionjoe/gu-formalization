#!/usr/bin/env python3
"""K200 second enlarged a/c-square ring certificate.

K199 closed 504/1024 <= a,c <= 520/1024. K200 certifies the exact
one-step ring outside that square as two row-chart side strips and four corner
boxes. Exact row/column transpose supplies the companion c strips. Together
with K199, the six packets close 503/1024 <= a,c <= 521/1024. Farther shells
...
K199_PATH = ROOT / "tests/channel-swings/k199_order_six_first_ac_corners.py"
K199_MANIFEST = ROOT / "lab/process/k199-order-six-first-ac-corners-wave.json"
OUTPUT = ROOT / "lab/process/k200-order-six-second-ac-ring-wave.json"
CHECKPOINT = ROOT / "_local/k200-order-six-second-ac-ring-checkpoint.json"
...
K199 = load_module("k199_for_k200", K199_PATH)
K198 = K199.K198
K197 = K199.K197
K196 = K199.K196
K193 = K199.K193
...
INNER = K199.EXPANDED
OUTER = (Fraction(503, 1024), Fraction(521, 1024))
LOW = (OUTER[0], INNER[0])
HIGH = (INNER[1], OUTER[1])
Q_CELLS = K199.Q_CELLS
PACKETS = (
...
def checkpoint_fingerprint() -> str:
    ...
    digest.update(K199_MANIFEST.read_bytes())
...
def load_checkpoint() -> dict[str, Any]:
    ...
    if state.get("schema_version") != "1.0" or state.get("fingerprint") != fingerprint:
        raise AssertionError("K200 checkpoint does not match the current generator and predecessor")
    return state
...
def certify_rectangle_or_bisect(
    ...
    if depth >= MAX_SHAPE_DEPTH:
        raise AssertionError(f"K200 ring packet lost positivity after refinement: {failure}")
...
def build_ring(progress: bool = False, checkpoint: bool = False) -> dict[str, Any]:
    ...
    for index, q_bounds in enumerate(Q_CELLS):
        if index < len(q_tiles):
            if progress:
                print(
                    f"K200 size=3 base_q_cell={index + 1}/{len(Q_CELLS)} "
            ...
            continue
        if index != len(q_tiles):
            raise AssertionError("K200 checkpoint q-tile sequence is not contiguous")
        current = state.get("current_q")
        if current is None:
            current = {"base_index": index, "accepted_ring_tiles": []}
        if current.get("base_index") != index:
            raise AssertionError("K200 checkpoint current q tile is out of sequence")
...
def independent_controls(ring: dict[str, Any]) -> dict[str, Any]:
                ...
                for a in shape_boundaries:
                    for c in shape_boundaries:
                        ...
                        if not Decimal(str(ring["minimum_R_lower"])) <= value <= Decimal(str(ring["maximum_R_upper"])):
                            raise AssertionError("independent K200 control escaped outward range")
                        rows.append({
                            "scale": ftext(scale),
    ...
    return {
        ...
        "role": "independent convergent-series second-ring boundary-grid controls; directed Arb common cells carry the proof",
    }


def build(progress: bool = False, checkpoint: bool = False) -> dict[str, Any]:
    predecessor = json.loads(K199_MANIFEST.read_text())
    ring = build_ring(progress, checkpoint)
    if not ring["q_tiles_contiguous"]:
        raise AssertionError("K200 q tiles are not contiguous")
    ...
    return {
        ...
        "fixed_control": {
            "predecessor_manifest": "lab/process/k199-order-six-first-ac-corners-wave.json",
            ...
            "k199_inner_a_c_range": [ftext(value) for value in INNER],
            "second_expanded_a_c_range": [ftext(value) for value in OUTER],
        ...
        "certified_second_enlarged_ac_square": {
            "size_three_row_chart_second_ring_cover": ring,
            "size_three_column_chart_side_strips_are_exact_transpose": True,
            "k199_first_enlarged_square_inherited": True,
            "two_side_strips_and_four_corners_covered": True,
            "exact_join_to_k199_square": True,
            "second_enlarged_a_c_square_complete": True,
        ...
        "complete_family_propagation": {
            ...
            "new_bound_applies_only_to_the_declared_second_enlarged_a_c_square": True,
        },
        "decision": {
            "two_one_step_side_strips_certified": True,
            "four_one_step_corner_boxes_certified": True,
            "k199_square_join_preserved": True,
            "second_enlarged_a_c_square_certified": True,
            "farther_a_c_complement_covered": False,
            "next_exact_input": "continue adaptive symmetric a/c shells beyond 503/1024<=a,c<=521/1024; only after complete shape coverage, differentiate the common-cell regularizer and compose the Duffy/Jacobi remainder",
        },
        "release_test": {
            ...
            "second_enlarged_a_c_square_outwardly_certified": True,
