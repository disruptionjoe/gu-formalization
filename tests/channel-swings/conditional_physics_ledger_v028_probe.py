#!/usr/bin/env python3
"""Fail-closed migration audit for conditional physics ledger v0.28."""
...
old = strict("lab/process/conditional-physics-ledger-v0.27.json")
new = strict("lab/process/conditional-physics-ledger-v0.28.json")
...
check("exact", "schema advances 0.27 to 0.28", old["schema_version"] == "0.27" and new["schema_version"] == "0.28")
...
for rid in touched:
    ...
    check("exact", f"{rid}: mapping grade records local principal chain", "LOCAL_PRINCIPAL" in after["mapping_grade"])
    check("exact", f"{rid}: mapping grade retains nonlinear boundary", "FULL_NONLINEAR" in after["mapping_grade"])
    check("exact", f"{rid}: evidence points to physical-response report", after["evidence"] == "selected-action-physical-soldering-observation-compose-2026-08-06.md")

migrations = [m for m in new["migrations"] if m.get("from_version") == "0.27" and m.get("to_version") == "0.28"]
...
check("type", "rank one begins at second jets", "second observation and soldering jets" in rank_one["why"])
check("type", "rank one forbids rebuilding first-jet owners", "do not rebuild" in rank_one["why"] and "first-jet equation dual" in rank_one["why"])
check("type", "rank one retains Euler and presymplectic classes", "Euler" in rank_one["why"] and "presymplectic" in rank_one["why"])
...
check("type", "local chain adds no quotient", registry["quotient_change"] is False)

for label in (
    "local principal chain is not full nonlinear stationarity",
    "connection current is not Hilbert stress",
    "first-jet no-leakage is not global shell equivalence",
    "preboundary potential is not BFV reduction",
    "second jets remain a construction burden",
