#!/usr/bin/env python3
"""Independent replay and hostile mutation probe for K352."""
...
PRODUCER = ROOT / "tests/channel-swings/k352_order_eight_mask_native_preconditioner_compiler.py"
STORED = ROOT / "lab/process/k352-order-eight-mask-native-preconditioner-compiler.json"


spec = importlib.util.spec_from_file_location("k352_probe_target", PRODUCER)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K352 producer")
...
def main() -> int:
    ...
    checks = [
        stored == rebuilt,
        stored["fixed_control"]["determinant_matrix_face_uses_replayed"] == 4_876_344,
        stored["template_summary"]["rank_histogram"] == {"1": 2, "2": 5, "3": 10, "4": 17},
        stored["fixed_control"]["unique_confluent_templates"] == 38,
    ...
    if not all(checks):
        raise AssertionError("K352 independent control failed")
    ...
    mutations = [
        ...
        lambda p: p["confluent_divided_difference_contract"].__setitem__("template_count", 37),
        ...
        lambda p: p["release_test"].__setitem__("exactly_34_unique_templates", False),
    ]
    rejected = 0
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K352 hostile rejection failed: {rejected}/{len(mutations)}")
    print(f"K352 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
