#!/usr/bin/env python3
"""Fail-closed migration audit for conditional physics ledger v0.26."""
...
old = strict("lab/process/conditional-physics-ledger-v0.25.json")
new = strict("lab/process/conditional-physics-ledger-v0.26.json")
registry = strict("lab/process/moving-gimmel-hodge-frame-owner.json")

print("A. FROZEN METER AND PROGRAM FENCES")
check("exact", "schema advances 0.25 to 0.26", old["schema_version"] == "0.25" and new["schema_version"] == "0.26")
...
for rid in touched:
    ...
    check("exact", f"{rid}: mapping grade records TT density progress", "TT_GIMMEL_DENSITY_D1_ZERO" in after["mapping_grade"])
    check("exact", f"{rid}: mapping grade fuses Hodge and coframe", "HODGE_COFRAME_FUSED" in after["mapping_grade"])
    check("exact", f"{rid}: evidence points to moving-gimmel report", after["evidence"] == "moving-gimmel-hodge-frame-owner-2026-08-06.md")

migrations = [m for m in new["migrations"] if m.get("from_version") == "0.25" and m.get("to_version") == "0.26"]
...
check("type", "rank one retains one fused gimmel frame packet", "owner fusion" in rank_one["why"] and "Phi" in rank_one["why"] and "Krein" in rank_one["why"])
check("type", "rank one forbids Hodge/coframe recounting", "do not recount Hodge and coframe" in rank_one["why"])
...
check("type", "owner fusion does not add quotient", registry["constraint_cost"]["new_quotients"] == 0)

for label in (
    "fixed-frame Hodge motion is not an independent field",
    "frame compensation is not selected-action Euler cancellation",
    "TT density zero is not conformal density zero",
    "pointwise (7,7) inertia is not positive hyperbolicity",
    "owner fusion is not residue reduction",
