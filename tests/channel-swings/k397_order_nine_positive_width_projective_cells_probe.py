#!/usr/bin/env python3
"""Independent regeneration and hostile probe for K397."""
...
PRODUCER = ROOT / "tests/channel-swings/k397_order_nine_positive_width_projective_cells.py"
STORED = ROOT / "lab/process/k397-order-nine-positive-width-projective-cells.json"
spec = importlib.util.spec_from_file_location("k397_probe_target", PRODUCER)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K397 producer")
...
def main() -> int:
    ...
    checks = [
        stored == rebuilt,
        len(stored["chart_cell_bank"]) == 758,
        stored["fixed_control"]["program_chart_cell_uses"] == 4320,
        len(stored["executed_cell_bank"]) == 14,
        ...
        all(row["coherent_group_count"] == 20 for row in stored["executed_cell_bank"]),
        stored["projective_cell_contract"]["cells_are_pilots_not_a_chart_cover"],
        all(stored["release_test"].values()),
    ]
    if not all(checks):
        raise AssertionError("K397 independent control failed")
    mutations = [
        lambda p: p["fixed_control"].__setitem__("unique_maximum_charts", 757),
        lambda p: p["fixed_control"].__setitem__("program_chart_cell_uses", 4319),
        ...
        lambda p: p["executed_cell_bank"][0].__setitem__("coherent_group_count", 19),
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K397 hostile rejection failed: {rejected}/{len(mutations)}")
    print(f"K397 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
