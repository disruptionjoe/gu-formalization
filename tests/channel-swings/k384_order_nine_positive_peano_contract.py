#!/usr/bin/env python3
"""Freeze the exact tensor Peano contract consumed after K382--K383."""
...
OUTPUT = ROOT / "lab/process/k384-order-nine-positive-peano-contract.json"
...
def build() -> dict:
    ...
    rows = []
    local_rows = {row["axis"]: row for row in k383["axis_node_jets"]}
        ...
        rows.append(
            {
                ...
                "hybrid_global_integrand": f"abs(partial_{axis}^2 H) with {index} preceding raw times at 1/256, {axis}=t against K_256(t), and {19-index} following raw times under exp(-256*x) dx",
    ...
    return {
        "schema_version": "1.0",
        "result_id": "K384-ORDER-NINE-POSITIVE-PEANO-CONTRACT",
        "created": "2026-09-23",
        ...
        "fixed_control": {
            ...
            "axis_node": "1/256",
            "axis_weight": "1/256",
            "product_weight": k380["positive_product_gauss_laguerre_rule"]["product_weight"],
        ...
        "tensor_telescoping": {
            "identity": "I^20-Q^20=sum_{a=1}^{20} Q^{a-1} tensor (I-Q)_a tensor I^{20-a}",
            "mixed_derivatives_required": False,
            "pure_second_directional_terms": 20,
            "axis_contracts": rows,
        ...
        "decision": {
            "complete_twenty_axis_remainder_formula_emitted": True,
            "complete_twenty_axis_remainder_numerically_bounded": False,
            "complete_order_nine_integral_emitted": False,
            "rank_five_order_nine_global_cover_released": False,
            "next_required_object": "zero-safe whole-orthant determinant-preserving interval atlas for the twenty K382 pure-second coherent functionals",
        },
        "release_test": {
            "all_20_axis_contracts_present": len(rows) == 20,
            "preceding_and_following_counts_partition_19": all(row["preceding_axes_fixed_at_node"] + row["following_axes_integrated_exactly"] == 19 for row in rows),
            ...
            "complete_order_nine_integral_not_overclaimed": True,
        },
        "ledger_effect": k383["ledger_effect"],
        "source_routing": k383["source_routing"],
        "claim_ceiling": "Exact positive Peano-kernel and tensor-telescoping contract for the native twenty-axis order-nine rule. It identifies the twenty complete coherent pure-second hybrid integrals, their exact node/integration ordering and kernel mass 1/33554432, and forbids substituting K383's local node jets for global bounds. It emits no numerical remainder, complete order-nine integral, action column, R_ref residual, K152 interval, source/ledger move, canon, paper, public or physical claim.",
    }
...
def validate_payload(payload: dict) -> None:
    ...
    if fixed["native_axes"] != list(AXES) or fixed["axis_node"] != "1/256" or fixed["axis_weight"] != "1/256":
        raise AssertionError("K384 native rule changed")
    ...
    if not identity["kernel_nonnegative"] or not identity["kernel_continuous_at_node"] or identity["kernel_zero_order_at_origin"] != 2 or identity["kernel_mass"] != "1/33554432":
        raise AssertionError("K384 Peano kernel contract changed")
    tensor = payload["tensor_telescoping"]
    if tensor["mixed_derivatives_required"] or tensor["pure_second_directional_terms"] != 20 or len(tensor["axis_contracts"]) != 20:
        raise AssertionError("K384 tensor contract changed")
    if not tensor["node_jet_substitution_for_global_integral_forbidden"] or not tensor["global_entrywise_supremum_before_group_assembly_forbidden"]:
        raise AssertionError("K384 forbidden shortcut admitted")
    decision = payload["decision"]
    if not decision["complete_twenty_axis_remainder_formula_emitted"] or decision["complete_twenty_axis_remainder_numerically_bounded"] or decision["complete_order_nine_integral_emitted"] or decision["rank_five_order_nine_global_cover_released"]:
        raise AssertionError("K384 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K384 release test failed")
