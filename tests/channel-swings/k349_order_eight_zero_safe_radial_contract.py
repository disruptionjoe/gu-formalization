#!/usr/bin/env python3
"""Certify the zero-safe radial scaling needed by the K348 Peano terms."""
...
OUTPUT = ROOT / "lab/process/k349-order-eight-zero-safe-radial-contract.json"
...
def build() -> dict[str, Any]:
    ...
    order_row = next(row for row in k279["orders"] if row["order"] == 8)
    if order_row["small_rho_power"] != 7:
        raise AssertionError("K279 order-eight origin power changed")
    if k344["fixed_control"]["positive_time_variables"] != 18:
        raise AssertionError("K344 axis census changed")
    if k348["one_axis_identity"]["kernel_zero_order_at_origin"] != 2:
        raise AssertionError("K348 Peano zero order changed")
    ...
    raw_kernel_count = 8 + 2
    derivative_cost = 2
    radial_jacobian_degree = 18 - 1
    peano_gain = 2
    raw_second_degree = -(raw_kernel_count + derivative_cost)
    final_degree = radial_jacobian_degree + raw_second_degree + peano_gain
    if final_degree != 7:
        raise AssertionError("order-eight radial degree does not replay K279")

    return {
        "schema_version": "1.0",
        "result_id": "K349-ORDER-EIGHT-ZERO-SAFE-RADIAL-CONTRACT",
        "created": "2026-09-22",
        ...
        "all_zero_radial_ledger": {
            ...
            "eighteen_axis_radial_jacobian_degree": radial_jacobian_degree,
            "active_peano_zero_gain": peano_gain,
            "final_radial_degree": final_degree,
            "K279_order_eight_small_rho_power_replayed": final_degree == order_row["small_rho_power"],
            "origin_integrable": final_degree > -1,
            "scope": "the all-eighteen-axis origin of the first K348 hybrid; later hybrids have fixed positive preceding nodes and require the K350 partial-face ledger",
        },
        "decision": {
            ...
            "next_exact_input": "compile the exact K344 zero and coalescence masks reachable in every K348 hybrid domain, then exercise the complete coherent derivative evaluator away from those faces",
        },
        "release_test": {
            ...
            "radial_degree_seven_replayed": final_degree == 7,
            "raw_zero_evaluation_not_used": True,
            "complete_order_eight_remainder_not_overclaimed": True,
            "native_K152_interval_not_emitted": True,
        },
        "ledger_effect": k348["ledger_effect"],
        "source_routing": k348["source_routing"],
        "claim_ceiling": "Exact rational zero-inclusive envelopes for the value, first and second derivatives of 2*K1, a global K_256(t)<=t^2/2 Peano majorant, and the complete degree-seven all-zero radial integrability ledger for the first order-eight hybrid. Partial coalescence-face preconditioners, recursive whole-domain coverage, all eighteen numerical hybrid integrals, the complete order-eight remainder and integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public and physical posture remain open.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["order"], fixed["raw_time_axes"], fixed["maximum_active_axis_derivative_order"], fixed["raw_kernel_factors_per_gram_entry"]) != (8, 18, 2, 10):
        raise AssertionError("K349 fixed census changed")
    ...
    if bank["raw_Bessel_evaluation_at_zero_used"] or not bank["all_bounds_exact_rational"] or bank["continuous_zero_limits"] != ["2", "2", "4"]:
        raise AssertionError("K349 zero-safe bank changed")
    if not all(row["contained"] for row in payload["peano_majorant"]["controls"]):
        raise AssertionError("K349 Peano controls failed")
    ledger = payload["all_zero_radial_ledger"]
    if ledger["final_radial_degree"] != 7 or not ledger["K279_order_eight_small_rho_power_replayed"] or not ledger["origin_integrable"]:
        raise AssertionError("K349 radial ledger changed")
    ...
    if not decision["all_zero_origin_integrability_closed"] or decision["partial_face_preconditioners_emitted"] or decision["complete_hybrid_integrals_emitted"]:
        raise AssertionError("K349 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K349 release test failed")
