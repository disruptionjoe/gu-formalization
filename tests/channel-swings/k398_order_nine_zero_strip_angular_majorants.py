#!/usr/bin/env python3
"""Prove exact measure-only majorants for K394 ratio-zero strips."""
...
K397 = ROOT / "lab/process/k397-order-nine-positive-width-projective-cells.json"
OUTPUT = ROOT / "lab/process/k398-order-nine-zero-strip-angular-majorants.json"
...
def build() -> dict[str, Any]:
    ...
    k397 = json.loads(K397.read_text())
    rows = []
    for codimension in k394["fixed_control"]["reachable_codimensions"]:
        ...
        rows.append({
            ...
            "union_of_all_zero_strips_mass_upper": q(min(chart_mass, ratio_count * EPSILON)),
            "recursive_intersection_majorants": intersections,
            "union_bound_replays_zero_when_epsilon_is_zero": True,
        })
    return {
        "schema_version": "1.0",
        "result_id": "K398-ORDER-NINE-ZERO-STRIP-ANGULAR-MAJORANTS",
        "created": "2026-09-24",
        "classification": "INTERNAL_STRUCTURAL_ONLY",
        "direction": "observed_to_native",
        "fixed_control": {
            "predecessor_manifests": [str(path.relative_to(ROOT)) for path in (K394, K395, K397)],
            "epsilon": q(EPSILON),
            "reachable_codimensions": k394["fixed_control"]["reachable_codimensions"],
            "codimension_rows": len(rows),
            "unique_maximum_charts": k397["fixed_control"]["unique_maximum_charts"],
            "program_chart_uses": k397["fixed_control"]["program_chart_cell_uses"],
        },
        "majorant_contract": {
            ...
            "majorants_do_not_bound_the_K388_preconditioned_integrand": True,
            "uniform_integrand_weighted_boundary_envelope_still_required": True,
        },
        "codimension_strip_bank": rows,
        "strip_summary": {
            "all_14_reachable_codimensions_covered": len(rows) == 14,
        ...
        "decision": {
            ...
            "next_exact_input": "compose K374/K375/K388 scaling into determinant-preserving zero-inclusive envelopes and bind them to K399-owned strips",
        },
        "release_test": {
            "epsilon_is_one_over_64": EPSILON == Fraction(1, 64),
            "exactly_14_codimension_rows": len(rows) == 14,
            "all_758_charts_in_scope": k397["fixed_control"]["unique_maximum_charts"] == 758,
            "all_4320_program_chart_uses_in_scope": k397["fixed_control"]["program_chart_cell_uses"] == 4320,
        ...
        "ledger_effect": k397["ledger_effect"],
        "source_routing": k397["source_routing"],
        "claim_ceiling": "Exact one-sided and recursive ratio-zero strip angular-mass majorants for every reachable K394 codimension at epsilon=1/64. The bounds use only the exact chart density and chart mass and vanish with their declared epsilon order. They do not bound the K388-preconditioned integrand, close zero-inclusive interval evaluation, supply a recursive interior cover, analytic tails, any complete K384 hybrid, the order-nine remainder/integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public or physical posture.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["epsilon"], fixed["codimension_rows"], fixed["unique_maximum_charts"], fixed["program_chart_uses"]) != ("1/64", 14, 758, 4320):
        raise AssertionError("K398 census changed")
    rows = payload["codimension_strip_bank"]
    if len(rows) != 14 or any(len(row["recursive_intersection_majorants"]) != row["ratio_coordinates"] for row in rows):
        raise AssertionError("K398 strip bank changed")
    contract = payload["majorant_contract"]
    if not contract["majorants_control_angular_measure_only"] or not contract["majorants_do_not_bound_the_K388_preconditioned_integrand"] or not contract["uniform_integrand_weighted_boundary_envelope_still_required"]:
        raise AssertionError("K398 scope boundary changed")
    ...
    if not decision["one_sided_zero_strip_angular_mass_majorants_complete"] or any(decision[key] for key in ("zero_inclusive_integrand_weighted_boundary_majorants_complete", "recursive_positive_interior_cover_complete", "complete_hybrid_integrals_emitted")):
        raise AssertionError("K398 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K398 release test failed")
