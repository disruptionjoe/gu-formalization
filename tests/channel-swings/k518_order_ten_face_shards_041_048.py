#!/usr/bin/env python3
"""Execute K511 shards 041--048 through the certified K517/K508 backend."""
...
OUTPUT = ROOT / "lab/process/k518-order-ten-face-shards-041-048.json"
...
K517 = load("k517_for_k518", "k517_order_ten_face_shards_033_040.py")
K517.K516.K515.K514.SHARD_START = SHARD_START
K517.K516.K515.K514.SHARD_STOP = SHARD_STOP


def build() -> dict:
    payload = K517.build()
    payload["result_id"] = "K518-ORDER-TEN-FACE-SHARDS-041-048"
    payload["created"] = "2026-09-26"
    payload["decision"] = {
        "shards_041_through_048_released": True,
        "cumulative_K511_shards_executed_including_K512": 49,
        "cumulative_remaining_face_programs_executed": 196,
        "unexecuted_remaining_face_programs": 720,
        ...
        "next_exact_input": (
            "Continue K511 at shard 049 under serialized resource control, then "
    ...
    payload["claim_ceiling"] = (
        ...
        "K511 shards 041 through 048, across 224 positive-width normal cells. "
        "Together with K512--K517 this completes 196 of the 916 K508-omitted "
        "programs. It is not the other 720 programs, a complete face bank, "
...
def validate(payload: dict) -> None:
    ...
    if actual != expected:
        raise AssertionError("K518 fixed census changed")
    required = (
    ...
    if not all(bank[key] for key in required):
        raise AssertionError("K518 backend evidence failed")
    ...
    if actual_program_ids != expected_program_ids:
        raise AssertionError("K518 stored program identity changed")
    if bank["bank_sha256"] != K517.K516.K515.K514.K508.digest(rows):
        raise AssertionError("K518 stored bank digest changed")
    stored_uses = sum(
    ...
    if bank["divided_difference_operation_uses"] != stored_uses:
        raise AssertionError("K518 operation census changed")
    direct_ids = [row["program_id"] for row in payload["direct_overlap_controls"]]
    if direct_ids != expected_program_ids:
        raise AssertionError("K518 direct-control identity changed")
    for stored, expected_shard in zip(
        ...
        if (
            ...
            raise AssertionError("K518 stored shard provenance changed")
        shard_rows = [row for row in rows if row["shard_id"] == stored["shard_id"]]
        if stored["program_rows_sha256"] != K517.K516.K515.K514.K508.digest(shard_rows):
            raise AssertionError("K518 stored shard digest changed")
    if not decision["shards_041_through_048_released"]:
        raise AssertionError("K518 release boundary changed")
    if decision["cumulative_remaining_face_programs_executed"] != 196:
        raise AssertionError("K518 cumulative census changed")
    if decision["unexecuted_remaining_face_programs"] != 720:
        raise AssertionError("K518 remainder census changed")
    forbidden = (
    ...
    if any(decision[key] for key in forbidden):
        raise AssertionError("K518 overclaimed the order-ten cover")
    if payload["source_and_ledger_effect"] != "none":
        raise AssertionError("K518 moved source or ledger state")
    if not all(
        math.isfinite(float(cell["complete_second_derivative_abs_upper"]))
        for row in rows
        for cell in row["cells"]
    ):
        raise AssertionError("K518 stored nonfinite cell")
