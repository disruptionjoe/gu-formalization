...
check("registry", "registry closes K125 covariance but retains K126 lower-order coefficients", registry["fixed_chart_recombination"] == "EXACT" and registry["curved_lower_order_coefficients"] == "OPEN_K126")
...
check("repo", "current state advances through K125", "K125" in current and "K126" in current)
check("repo", "roadmap routes the curved lower-order packet", "K126" in roadmap[:6000])
check("repo", "context preserves K124 principal coefficients", "-12 q^2" in context[:7000] and "K125" in context[:7000])
...
print("CURVED_LOWER_ORDER_SYMMETRIC_TT_PACKET=3_PARAMETERS_OPEN_K126")
