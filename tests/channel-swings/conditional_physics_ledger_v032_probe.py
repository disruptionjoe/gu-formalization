#!/usr/bin/env python3
"""Fail-closed migration audit for conditional physics ledger v0.32."""
...
old = strict("lab/process/conditional-physics-ledger-v0.31.json")
new = strict("lab/process/conditional-physics-ledger-v0.32.json")
...
check("exact", "schema advances 0.31 to 0.32", old["schema_version"] == "0.31" and new["schema_version"] == "0.32")
...
required = (
    "METRIC_ONLY_WARD_RANK34_AFFINE21_SCOPED",
    "CONNECTION_LIE_SYMBOL_RANK4",
    "COUPLED_WARD_RANK98_AFFINE198",
    "ACTUAL_SAME_I1B_BLOCK_HESSIAN",
    "OPEN",
)
for row_id in touched:
    ...
    check("exact", f"{row_id}: coupled target and action burden are typed", all(token in after["mapping_grade"] for token in required))
    check("exact", f"{row_id}: evidence points to coupled retype", after["evidence"] == "selected-action-coupled-diffeomorphism-ward-retype-2026-08-06.md")

migrations = [m for m in new["migrations"] if m.get("from_version") == "0.31" and m.get("to_version") == "0.32"]
...
check("type", "rank one names actual same-I1B coupled blocks", all(token in rank_one["why"] for token in ("same-I1B", "metric-plus-connection", "block Ward")))
check("type", "rank one rejects arbitrary metric completion", "do not search" in rank_one["why"] and "ten-by-ten metric companion" in rank_one["why"])
check("type", "rank one rejects booking affine freedom as residue", "198 affine directions" in rank_one["why"] and "residue" in rank_one["why"])
...
for label in (
    "metric-only theorem is retained rather than erased",
    "coupled diagnostic completion is not action derivation",
    "198 affine directions are not 198 parameters",
    "full-field radical is not BV cohomology",
