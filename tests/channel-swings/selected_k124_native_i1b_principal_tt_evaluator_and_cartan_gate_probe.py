...
check("registry", "registry records principal completion without claiming full nonlinear closure", registry["completion_grade"] == "FULL_14D_CARRIER__LOCAL_PRINCIPAL_TT_BULK_AND_GREEN_ONLY")
check("registry", "registry retains lower-order Cartan gate", registry["full_O_K123_complete"] is False and "K125" in registry["next_gate"])
check("repo", "current state advances through K124", "K124" in current and "-12 q^2" in current)
check("repo", "roadmap routes K125 after the principal result", "K125" in roadmap[:5000])
...
print("\nC_THH_PRINCIPAL=-12*q^2*DEWITT_TT_PAIRING")
