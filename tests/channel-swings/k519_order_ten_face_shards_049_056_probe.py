#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K519."""
...
spec = importlib.util.spec_from_file_location(
    "k519_probe_target", HERE / "k519_order_ten_face_shards_049_056.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K519")
K519 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K519)
...
def checks(payload):
    ...
    return [
        ...
        fixed["first_shard_id"] == "order10-face-049",
        fixed["last_shard_id"] == "order10-face-056",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"]
        == K519.K518.K517.K516.K515.K514.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 228,
        decision["unexecuted_remaining_face_programs"] == 688,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 687),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K519.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K519 controls: {sum(controls)}/{len(controls)}; "
