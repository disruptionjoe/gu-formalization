#!/usr/bin/env python3
"""Stored-result probe and hostile mutations for K518."""
...
spec = importlib.util.spec_from_file_location(
    "k518_probe_target", HERE / "k518_order_ten_face_shards_041_048.py"
)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K518")
K518 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(K518)
...
def checks(payload):
    ...
    return [
        ...
        fixed["first_shard_id"] == "order10-face-041",
        fixed["last_shard_id"] == "order10-face-048",
        fixed["shard_count"] == 8,
        ...
        bank["bank_sha256"]
        == K518.K517.K516.K515.K514.K508.digest(payload["face_cell_bank"]),
        ...
        decision["cumulative_remaining_face_programs_executed"] == 196,
        decision["unexecuted_remaining_face_programs"] == 720,
...
def main() -> int:
    ...
    mutations = [
        ...
        lambda row: row["decision"].__setitem__("unexecuted_remaining_face_programs", 719),
    ...
    for mutate in mutations:
        ...
        mutate(hostile)
        try:
            K518.validate(hostile)
        except AssertionError:
            rejected += 1
    print(
        f"K518 controls: {sum(controls)}/{len(controls)}; "
