...
PW2FR2B2A = ROOT / "lab/process/pw2fr2b2a-second-frechet-c4-graph-registry.json"
PW2FR2B2B1 = ROOT / "lab/process/pw2fr2b2b1-source-comoving-second-graph-registry.json"
...
def validate(data: dict[str, object]) -> list[str]:
    errors: list[str] = []
    if data.get("status") != "PW2FR2B2B1_FINITE_SOURCE_SPLIT_COMOVING_SECOND_GRAPH_PASS_COMPLETE_NATIVE_C5_C4_BLOCKED_PW2FR2B2B2_NEXT":
        errors.append("status must record the finite PW2F-R2B2B1 prerequisite and PW2F-R2B2B2 next")
    ...
    expected_statuses = ["CONDITIONAL_PASS_PW2_ENABLED", "PW2FR2B2B1_FINITE_SOURCE_SPLIT_COMOVING_SECOND_GRAPH_PASS_COMPLETE_NATIVE_C5_C4_BLOCKED"] + ["BLOCKED_ON_DEPENDENCIES"] * 8
    ...
    pw2fr_review = waves[1].get("pw2fr_review_receipts", {}) if len(waves) > 1 else {}
    if pw2fr_review.get("pre_assessment", {}).get("status") != "COMPLETE":
        errors.append("PW2F-R pre-assessment receipt incomplete")
    if pw2fr_review.get("post_review", {}).get("status") != "COMPLETE_AFTER_REPAIR":
        errors.append("PW2F-R hostile post-review receipt incomplete")
    if pw2fr_review.get("post_review", {}).get("must_fix"):
        errors.append("PW2F-R hostile post-review retains must-fix items")
    if len(pw2fr_review.get("post_review", {}).get("rerun_receipts", [])) < 4:
        errors.append("PW2F-R hostile post-review rerun receipts missing")
    pw2fr2a_review = waves[1].get("pw2fr2a_review_receipts", {}) if len(waves) > 1 else {}
    if pw2fr2a_review.get("pre_assessment", {}).get("status") != "COMPLETE":
        errors.append("PW2F-R2A pre-assessment receipt incomplete")
    if pw2fr2a_review.get("post_review", {}).get("status") not in {"COMPLETE", "COMPLETE_AFTER_REPAIR"}:
        errors.append("PW2F-R2A hostile post-review receipt incomplete")
    if pw2fr2a_review.get("post_review", {}).get("must_fix"):
    ...
    pw2fr2b2a_review = waves[1].get("pw2fr2b2a_review_receipts", {}) if len(waves) > 1 else {}
    if pw2fr2b2a_review.get("pre_assessment", {}).get("status") != "COMPLETE":
        errors.append("PW2F-R2B2A pre-assessment receipt incomplete")
    if pw2fr2b2a_review.get("post_review", {}).get("status") != "COMPLETE_BLOCKING_PROMOTION":
        errors.append("PW2F-R2B2A blocking hostile post-review receipt missing")
    if len(pw2fr2b2a_review.get("post_review", {}).get("must_fix_before_complete_c4", [])) < 5:
        errors.append("PW2F-R2B2A complete-C4 blockers were erased")
    if len(pw2fr2b2a_review.get("post_review", {}).get("rerun_receipts", [])) < 3:
        errors.append("PW2F-R2B2A hostile post-review rerun receipts missing")
    pw2fr2b2b1_review = waves[1].get("pw2fr2b2b1_review_receipts", {}) if len(waves) > 1 else {}
    if pw2fr2b2b1_review.get("pre_assessment", {}).get("status") != "COMPLETE":
        errors.append("PW2F-R2B2B1 pre-assessment receipt incomplete")
    if pw2fr2b2b1_review.get("post_review", {}).get("status") != "COMPLETE_AFTER_SCOPE":
        errors.append("PW2F-R2B2B1 hostile post-review receipt incomplete")
    if pw2fr2b2b1_review.get("post_review", {}).get("must_fix"):
        errors.append("PW2F-R2B2B1 hostile post-review retains must-fix items")
    if len(pw2fr2b2b1_review.get("post_review", {}).get("rerun_receipts", [])) < 3:
        errors.append("PW2F-R2B2B1 hostile post-review rerun receipts missing")
    seen: set[str] = set()
    for wave in waves:
...
def main() -> None:
    ...
    pw2fr2b2a = json.loads(PW2FR2B2A.read_text(), object_pairs_hook=unique_object)
    pw2fr2b2b1 = json.loads(PW2FR2B2B1.read_text(), object_pairs_hook=unique_object)
    ...
    for token in [
        ...
        "PW3 stays blocked",
        "PW2F-R2B2B1 FINITE PREREQUISITE / PW2F-R2B2B2 REQUIRED",
        "7 kappa1+6x-3",
        "7 kappa1+8x-10",
    ...
    if not pw2fr["status"].startswith("PW2FR_Z1_GRAPH_PASS_STRUCTURAL_C6"):
        raise AssertionError("PW2F-R scoped status drifted")
    if pw2fr["corrected_metric_graph"]["z1_metric_ranks"] != [10, 10, 10, 10, 10]:
        raise AssertionError("PW2F-R Z1 metric ranks drifted")
    if pw2fr["highest_order_euler"]["C5"]["all_base_conormal_polynomial_nonzero_entries"] != 0:
        raise AssertionError("PW2F-R exercised C5 polynomial drifted")
    if pw2fr["highest_order_euler"]["distortion_norm_principal_z1_kappa1_C4_contribution"]["panel_ranks"] != [10, 10, 10, 10, 0]:
        raise AssertionError("PW2F-R principal-Z1 C4 contribution ranks drifted")
    if pw2fr["external_datum"] != "P1/P2/P3 UNCHANGED AND UNUSED":
        raise AssertionError("PW2F-R spent the external datum")
    if not pw2fr2["status"].startswith("PARTIAL_CONSTRUCTION_PASS"):
        raise AssertionError("PW2F-R2A scoped status drifted")
    if pw2fr2["exact_results"]["moving_noncyclic_remainder"] != "-20 directional Euler-covector evaluation on the preregistered fixture":
        raise AssertionError("PW2F-R2A noncyclic Euler remainder drifted")
    if pw2fr2["exact_results"]["curved_reference_distortion_polynomial"] != "-5*r*(3*r+1)":
        raise AssertionError("PW2F-R2A curved-reference scaling drifted")
    if pw2fr2["exact_results"]["universal_constant_kappa1"] != "ABSENT_ON_BOTH_EXACT_BACKGROUND_FAMILIES":
        raise AssertionError("PW2F-R2A universal-kappa discriminator drifted")
    if pw2fr2["verdict_scope"]["not_killed"].find("actual induced-Y14") < 0:
    ...
    if pw2fr2b2a["status"] != "PARTIAL_COMPARATOR_PASS_BLOCKS_COMPLETE_C4":
        raise AssertionError("PW2F-R2B2A partial-comparator boundary drifted")
    if "deltaq" not in pw2fr2b2a["blocking_debt"]["source_tangent"]:
        raise AssertionError("PW2F-R2B2A source-tangent blocker drifted")
    if "theta1" not in pw2fr2b2a["blocking_debt"]["second_graph"] or "Bhat2" not in pw2fr2b2a["blocking_debt"]["second_graph"]:
        raise AssertionError("PW2F-R2B2A second-graph blocker drifted")
    if pw2fr2b2a["partial_exact_results"]["universal_kappa"].find("NONE") < 0:
        raise AssertionError("PW2F-R2B2A partial kappa result drifted")
    if pw2fr2b2a["external_datum"] != "P1/P2/P3 UNCHANGED AND UNUSED":
        raise AssertionError("PW2F-R2B2A spent the external datum")
    if not pw2fr2b2b1["status"].startswith("SCOPED_FINITE_SOURCE_SPLIT"):
        raise AssertionError("PW2F-R2B2B1 finite prerequisite scope drifted")
    if "minus-deltaGamma" not in pw2fr2b2b1["exact_results"]["fixed_source_tangent"]:
        raise AssertionError("PW2F-R2B2B1 source shortcut defect drifted")
    if "nonzero" not in pw2fr2b2b1["exact_results"]["Bhat2"]:
        raise AssertionError("PW2F-R2B2B1 live Bhat2 result drifted")
    if "E-times-D2" not in pw2fr2b2b1["exact_results"]["pullback_hessian"]:
        raise AssertionError("PW2F-R2B2B1 two-term Hessian drifted")
    if pw2fr2b2b1["external_datum"] != "P1/P2/P3 UNCHANGED AND UNUSED":
    ...
    plants = [
        ...
        lambda d: d["waves"][1]["pw2fr2b2a_review_receipts"]["post_review"].update(must_fix_before_complete_c4=[]),
        lambda d: d["waves"][1]["pw2fr2b2b1_review_receipts"]["post_review"].update(must_fix=["live blocker"]),
    ]
    for plant in plants:
