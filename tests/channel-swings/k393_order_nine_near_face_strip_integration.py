#!/usr/bin/env python3
"""Integrate K392's canonical equal-normal cells with K389 powers.
...
K392 = ROOT / "lab/process/k392-order-nine-preconditioned-face-cell-bank.json"
OUTPUT = ROOT / "lab/process/k393-order-nine-near-face-strip-integration.json"
...
def power_integral(left: Fraction, right: Fraction, exponent: int) -> Fraction:
    if exponent <= -1:
        raise AssertionError("nonintegrable K393 normal power")
    return (right ** (exponent + 1) - left ** (exponent + 1)) / (exponent + 1)


def build() -> dict[str, Any]:
    k392 = json.loads(K392.read_text())
    if not k392["decision"]["K388_preconditioned_positive_width_face_cells_released"]:
        raise AssertionError("K392 face cells unavailable")
    expected_cells = [[Fraction(a), Fraction(b)] for a, b in k392["fixed_control"]["normal_cells"]]
    if expected_cells[0][0] != Fraction(1, 4096) or expected_cells[-1][1] != Fraction(1, 1024):
        raise AssertionError("K392 near-face strip changed")
    for left_row, right_row in zip(expected_cells, expected_cells[1:], strict=False):
        if left_row[1] != right_row[0]:
            raise AssertionError("K392 normal cells do not form a contiguous partition")

    rows = []
    by_axis: dict[str, list[arb]] = defaultdict(list)
    active_count = 0
    for face in k392["face_cell_bank"]:
        ...
        if exponent != int(face["minimum_face_normal_power"]):
            raise AssertionError("K393 failed to replay K389 normal exponent")
        ...
        for stored, (left, right) in zip(face["cells"], expected_cells, strict=True):
            if [Fraction(value) for value in stored["normal_interval"]] != [left, right]:
                raise AssertionError("K392 face cell partition changed")
            normal_mass = power_integral(left, right, exponent)
            scaled_upper = arb(stored["rho_to_K389_second_singular_power_abs_upper"])
    ...
    axis_order = [f"s{index}" for index in range(1, 11)] + [f"v{index}" for index in range(1, 11)]
    axis_summary = []
    ...
    return {
        "schema_version": "1.0",
        "result_id": "K393-ORDER-NINE-NEAR-FACE-STRIP-INTEGRATION",
        "created": "2026-09-23",
        "classification": "INTERNAL_NUMERICAL_CONTROL_ONLY",
        "direction": "observed_to_native",
        "fixed_control": {
            "predecessor_manifest": "lab/process/k392-order-nine-preconditioned-face-cell-bank.json",
            "K392_complete_bank_sha256": k392["bank_summary"]["complete_bank_sha256"],
        ...
        "integration_contract": {
            "K392_scaled_second_upper_used": True,
            "K389_face_normal_jacobian_power_used": True,
            "K385_quadratic_peano_factor_used": True,
        ...
        "strip_summary": {
            "all_695_face_strips_integrated": len(rows) == 695,
            "all_4865_cell_contributions_finite_positive": all(
                math.isfinite(float(cell["integrated_abs_upper"])) and float(cell["integrated_abs_upper"]) > 0
                for row in rows for cell in row["cells"]
            ),
            "global_minimum_K389_normal_power": min(row["K389_normal_power"] for row in rows),
        ...
        "decision": {
            ...
            "wide_unsplit_cells_rejected_by_K392_quality_control": True,
            ...
            "next_exact_input": "construct an exact normal-projective partition around every codimension-two-through-twenty face, then join these stable radial cells to recursive positive-interior cells and analytic radial tails",
        },
        "release_test": {
            "exactly_695_face_strip_rows_present": len(rows) == 695,
            "exactly_4865_integrated_cells_present": sum(len(row["cells"]) for row in rows) == 4865,
            "declared_strip_exactly_partitioned": total_width == Fraction(3, 4096),
            "all_contributions_finite_positive": all(
                math.isfinite(float(cell["integrated_abs_upper"])) and float(cell["integrated_abs_upper"]) > 0
                for row in rows for cell in row["cells"]
            ),
            "K389_global_minimum_power_replayed": min(row["K389_normal_power"] for row in rows) == 0,
            "exactly_18_hybrids_have_reachable_faces": sum(row["reachable_face_count"] > 0 for row in axis_summary) == 18,
            "v9_v10_have_no_invented_faces": [row["axis"] for row in axis_summary if row["reachable_face_count"] == 0] == ["v9", "v10"],
            "full_normal_cone_not_overclaimed": True,
            "complete_order_nine_remainder_not_overclaimed": True,
            "native_K152_interval_not_emitted": True,
        },
        "ledger_effect": k392["ledger_effect"],
        "source_routing": k392["source_routing"],
        "claim_ceiling": "Exact integration of K392's seven-cell canonical equal-normal chart on rho in [1/4096,1/1024] for every one of the 695 reachable faces. Each contribution uses K389's exact face-normal power and K385's appropriate quadratic Peano coefficient; the cells are pairwise disjoint up to boundaries and have exact total width 3/4096. The calculation uses unit projective angular density, and distinct face rows overlap, so neither the face-row sums nor hybrid diagnostics are full normal-cone or K384 hybrid integrals. The missing normal-projective cover, origin rho<1/4096, positive interior, analytic tails, complete order-nine remainder/integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public and physical posture remain open.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["reachable_face_programs"], fixed["cells_per_face"], fixed["integrated_face_cells"], fixed["exact_partition_width"]) != (695, 7, 4865, "3/4096"):
        raise AssertionError("K393 fixed census changed")
    rows = payload["face_strip_bank"]
    if len(rows) != 695 or any(len(row["cells"]) != 7 or row["K389_normal_power"] < 0 for row in rows):
        raise AssertionError("K393 face-strip bank changed")
    contract = payload["integration_contract"]
    required = (
        "K392_scaled_second_upper_used", "K389_face_normal_jacobian_power_used",
        "K385_quadratic_peano_factor_used", "seven_cells_pairwise_disjoint_up_to_boundaries",
    ...
    if not all(contract[key] for key in required):
        raise AssertionError("K393 integration contract changed")
    decision = payload["decision"]
    if not decision["all_equal_normal_near_face_strips_integrated"]:
        raise AssertionError("K393 strip release lost")
    if any(decision[key] for key in ("full_projective_normal_cone_complete", "recursive_positive_interior_cover_complete", "analytic_radial_tails_complete", "complete_hybrid_integrals_emitted")):
        raise AssertionError("K393 overclaimed whole-domain closure")
    if not all(payload["release_test"].values()):
        raise AssertionError("K393 release test failed")
