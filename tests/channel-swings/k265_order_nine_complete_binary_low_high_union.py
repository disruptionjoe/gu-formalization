#!/usr/bin/env python3
"""K265: certify the complete 256-box K218 binary low/high union positive."""
...
OUT = ROOT / "lab/process/k265-order-nine-complete-binary-low-high-union.json"

ORDER = 9
PRECISION = 256
POSITIVE_LOWER = Q(6, 10**18)  # 6.0e-18
...
def compile_box(groups: dict, fixed_status: tuple[int, int],
...
def summarize_block(name: str, fixed_status: tuple[int, int],
    ...
    return {
        "block": name,
        "fixed_axis_status": {"0": "high" if fixed_status[0] else "low",
                              "1": "high" if fixed_status[1] else "low"},
        "boxes": 64,
...
def generate() -> dict:
    ...
    blocks = []
    for name, fixed_status in (("low_low", (0, 0)),
                               ("high_low", (1, 0)),
                               ("high_high", (1, 1))):
        layers = []
    ...
    complete_head_by_degree = [
        arb(value) for value in
        k264["complete_union"]["complete_raw_head_by_degree"]
    ]
    complete_tail = Q(k264["complete_union"]["complete_raw_absolute_tail_upper"])
    for block in blocks:
    ...
    return {
        ...
        "input_sha256": {
            ...
            for path in (K185, K218, K230, K261, K262, K263, K264)
        },
        "object": (
            "The exact K218 signed integral on the complete 256-box binary "
            "low/high union, with axes 0 and 1 separately typed and every "
            "axis independently using the low or high interval"
        ),
        "domain": {
            "fixed_axis_blocks": ["low_low", "low_high", "high_low", "high_high"],
        ...
        "k264_low_high_block_ref": "lab/process/k264-order-nine-complete-low-high-union.json",
        "new_fixed_status_blocks": blocks,
        "complete_binary_union": {
        ...
        "certificate": {
            ...
            "result": "strictly_positive_complete_256_box_binary_low_high_union",
        },
        "controls": (
            "An independent reverse-order raw-allocation probe recompiles every exact "
            "degree polynomial, moment and tail; independently reconstructs directed "
            "normalization; checks two- and three-point product quadratures; and applies "
            "absolute-weight, wrong-measure, block-deletion and K264 controls."
        ),
        "source_routing": (
        ...
        "decision": (
            "The three fixed-axis blocks outside K264 are strictly positive, and their "
            "disjoint composition with K264 makes the complete 256-box binary low/high "
            "union strictly positive. The middle and farther complement remain open."
        ),
        "claim_ceiling": (
            "Exact positivity and interval only on the declared complete 256-box binary union. "
...
if __name__ == "__main__":
    ...
    print("[PASS] K265", result["certificate"]["result"])
    print("[PASS] K265 interval", result["complete_binary_union"]["normalized_integral_interval"])
