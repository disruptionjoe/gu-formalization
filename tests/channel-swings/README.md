...
- `selected_k92_rsap_balanced_centralizer11_mixed_census_probe.py`
  ...
  and `84` zero-plus-nonzero configurations. All `673` rows admit exact
  balanced representatives with the sharp `11/80/2/89`
  centralizer/target/fixed/map schedule. It retains all `384` nonoptimal zero
  gradings and excludes `24` fully semisimple rows back to K88. It passes
  `27/27` with Python 3.
...
- `selected_k85_rsap_sustar4_a3_singular_transition_atlas_probe.py`
  ...
  family graph and retain their source centralizers away from the endpoints;
  four rational `SO*(4)` Cayley frames close all six primitive overlaps and
  four base/cotangent triangles. It passes `75/75` with Python 3.
...
- `selected_k83_rsap_su31_a3_singular_transition_atlas_probe.py` constructs a
  ...
  sign controls. Eleven exact paths close the configuration graph; rational
  normalizations close all six primitive overlaps and four base/cotangent
  triangles. It passes `80/80` with Python 3.
...
- `selected_k77_sr1h_action_owned_point_carrier_census_probe.py` inventories
  ...
  proves that none is an eligible VRS-5 background. It preserves future new
  constructions and reranks RSAP without rewriting the historical council
  vote; `48/48` pass with Python 3.
