#!/usr/bin/env python3
"""Independent replay and hostile mutation probe for K413."""
...
PRODUCER = ROOT / "tests/channel-swings/k413_order_ten_mask_native_preconditioner_compiler.py"
STORED = ROOT / "lab/process/k413-order-ten-mask-native-preconditioner-compiler.json"


spec = importlib.util.spec_from_file_location("k413_probe_target", PRODUCER)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load K413 producer")
...
def main() -> int:
    ...
    checks = [
        stored == rebuilt,
        stored["fixed_control"]["determinant_matrix_face_uses_replayed"] == 49_544_352,
    ...
    if not all(checks):
        raise AssertionError("K413 independent control failed")

    mutations = [
        lambda p: p["fixed_control"].__setitem__("ordered_descriptors", 13299),
    ...
    if rejected != len(mutations):
        raise AssertionError(f"K413 hostile rejection failed: {rejected}/{len(mutations)}")
    print(f"K413 probe passed {len(checks)}/{len(checks)} controls and rejected {rejected}/{len(mutations)} hostile mutations")
    return 0
