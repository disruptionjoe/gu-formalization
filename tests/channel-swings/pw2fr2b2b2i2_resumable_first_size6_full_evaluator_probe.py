#!/usr/bin/env python3
r"""Resumable full-evaluator coverage: first uncovered size-six S3 orbit.

The byte-pinned predecessor certifies exactly 3/380 canonical representatives.
This successor creates the first append-only resumable shard by selecting the
lexicographically first uncovered representative before evaluator output.  It
...
The durable result is exactly 4/380.  It does not execute the preregistered
dense heldouts, certify the remaining 376 representatives, promote the engine,
...
PREDECESSOR = CHANNEL / "pw2fr2b2b2i2_affine_first_size3_full_evaluator_probe.py"
PREDECESSOR_REGISTRY = (
    ROOT / "lab/process/pw2fr2b2b2i2-affine-first-size3-full-evaluator-certificate.json"
)
DEPENDENCIES = {
    PREDECESSOR: "9d2a0d87eabfed56801f0dd8d36149ab674b473f89fdf43b5ab95f8aa66c967a",
    PREDECESSOR_REGISTRY: "5d7a177d2e04d14953d6abb4026876bac3049757fda28f358e31f56352ac9318",
}


def load_predecessor():
    spec = spec_from_file_location("gu_i2_affine_size3_durable", PREDECESSOR)
...
EXPECTED_REPRESENTATIVE = ((0, 0), (0, 0, 1, 3))
EXPECTED_MIXED_ACTIONS = (
    sp.Rational(-523, 144),
    sp.Rational(-1379, 288),
    sp.Rational(-523, 144),
    sp.Rational(-1235, 288),
    sp.Rational(3499, 288),
    sp.Rational(3643, 288),
)
DENSE_HELDOUT_SEEDS = (
    ((0, 9), (1, -1, 2, 3)),
    ((1, 8), (1, 2, 3, -2)),
...
def main() -> int:
    print(
        "PW2F-R2B2B2I2 RESUMABLE FIRST-UNCOVERED SIZE6 FULL-EVALUATOR CERTIFICATE",
        flush=True,
    )
    failures: list[str] = []
    ...
    exact(
        "durable 3/380 predecessor probe and registry are byte pinned",
    ...
    remaining = tuple(
        representative
        for representative in representatives
        if representative not in predecessor_covered
    )
    ...
    exact("durable predecessor coverage resolves to exactly three representatives", len(predecessor_covered) == 3)
    ...
    exact("six dense heldout seeds are outside the 1,925-cell lattice", all(seed not in label_set for seed in DENSE_HELDOUT_SEEDS), "6/6")
    exact("each dense heldout seed has a nontrivial closed S3 orbit", all(len(heldout) in (3, 6) for heldout in heldout_orbits), "6/6")
    ...
    if EXPECTED_MIXED_ACTIONS is not None:
        exact(
            ...
            mixed_actions == EXPECTED_MIXED_ACTIONS,
        )
    ...
    typed("one size-six orbit advances durable coverage to exactly 4/380")
    typed("the remaining 376 representatives stay open")
    typed("dense heldouts are preregistered inputs, not executed coverage")
    ...
    reject("PLANT promote the 380-representative engine from 4/380 coverage")
    reject("PLANT count preregistered dense heldouts as executed")
    reject("PLANT replace the 1,925-cell fallback before the other 376 representatives")
    reject("PLANT assemble or merge I1 A4 and I2B C4")
    reject("PLANT begin Green/Helmholtz from a second non-fixed orbit")
    ...
    print("representative_coverage=4/380")
    print("remaining_representatives=376/380")
    ...
    if failures:
        return 1
    print(
        "VERDICT: the lexicographically first uncovered canonical representative "
        "and its complete six-cell orbit transport exactly under both S3 generators; "
        "durable coverage is 4/380 and the other 376 representatives remain open"
    )
