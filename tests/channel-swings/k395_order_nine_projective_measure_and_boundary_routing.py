#!/usr/bin/env python3
"""Freeze K394's exact angular measure and compact boundary recursion."""
...
OUTPUT = ROOT / "lab/process/k395-order-nine-projective-measure-and-boundary-routing.json"
...
def build() -> dict[str, Any]:
    ...
    atlas = k394["mask_atlas"]
    if len(atlas) != 98 or len(k389["face_normal_atlas"]) != 695:
        raise AssertionError("K389/K394 census changed")
    ...
    program_oriented_boundaries = sum(2 * row["codimension"] * (row["codimension"] - 1) for row in k394["program_chart_map"])
    compact_lower_strata = sum(
        row["codimension"] * (2 ** (row["codimension"] - 1) - 1)
        for row in atlas
    )
    normal_powers = [int(row["minimum_second_derivative_face_normal_power"]) for row in k389["face_normal_atlas"]]

    return {
        "schema_version": "1.0",
        "result_id": "K395-ORDER-NINE-PROJECTIVE-MEASURE-AND-BOUNDARY-ROUTING",
        "created": "2026-09-23",
        "classification": "INTERNAL_STRUCTURAL_ONLY",
        "direction": "observed_to_native",
        "fixed_control": {
            "predecessor_manifests": [str(K389.relative_to(ROOT)), str(K394.relative_to(ROOT))],
            "face_programs": 695,
            "unique_zero_masks": 98,
            "unique_maximum_charts": 758,
        ...
        "measure_contract": {
            ...
            "K389_normal_power_already_includes_radial_jacobian": True,
            "angular_mass_must_not_be_applied_to_K393_equal_line_values": True,
        },
        "boundary_routing_contract": {
            ...
            "K385_radial_zero_contract_does_not_by_itself_close_every_projective_coordinate_boundary": True,
        ...
        "routing_summary": {
            "all_reachable_codimensions_measured": len(codimension_rows) == len(k394["fixed_control"]["reachable_codimensions"]),
        ...
        "release_test": {
            "exactly_14540_unique_oriented_coordinate_boundaries": unique_oriented_boundaries == 14540,
            "exactly_66288_program_boundary_uses": program_oriented_boundaries == 66288,
            "exactly_21274406_compact_lower_strata": compact_lower_strata == 21_274_406,
            "all_chart_mass_sums_exact": all(row["chart_sum_replays_simplex_mass"] for row in codimension_rows),
            "all_K389_face_powers_nonnegative": all(power >= 0 for power in normal_powers),
        ...
        "claim_ceiling": "Exact angular masses and compact recursive boundary routing for K394's complete maximum-coordinate projective atlas. Every chart has mass 1/c!, the c charts sum to the simplex mass 1/(c-1)!, and zero/tie routing terminates. Measure-zero boundaries are not interval enclosures: one-sided zero-safe projective boundary majorants, positive-width chart cells, global face-neighborhood ownership, recursive interior, tails, K384 integrals, the complete order-nine remainder/integral, action column, R_ref, K152 interval, source/ledger move, canon, paper, public and physical posture remain open.",
    }


def validate_payload(payload: dict[str, Any]) -> None:
    fixed = payload["fixed_control"]
    if (fixed["face_programs"], fixed["unique_zero_masks"], fixed["unique_maximum_charts"]) != (695, 98, 758):
        raise AssertionError("K395 atlas census changed")
    if (fixed["unique_oriented_coordinate_boundaries"], fixed["program_oriented_coordinate_boundary_uses"], fixed["compactly_represented_nonempty_lower_strata"]) != (14540, 66288, 21_274_406):
        raise AssertionError("K395 boundary census changed")
    if not all(row["chart_sum_replays_simplex_mass"] for row in payload["codimension_measure_bank"]):
        raise AssertionError("K395 chart mass changed")
    measure = payload["measure_contract"]
    if not measure["K389_normal_power_already_includes_radial_jacobian"] or not measure["angular_mass_must_not_be_applied_to_K393_equal_line_values"]:
        raise AssertionError("K395 measure ownership changed")
    routing = payload["boundary_routing_contract"]
    required = ("zero_routing_terminates", "tie_routing_terminates", "boundaries_have_angular_measure_zero", "interval_closures_still_require_one_sided_zero_safe_majorants", "K385_radial_zero_contract_does_not_by_itself_close_every_projective_coordinate_boundary")
    if not all(routing[key] for key in required):
        raise AssertionError("K395 boundary routing changed")
    ...
    if not decision["exact_angular_measure_compiled"] or not decision["compact_projective_boundary_recursion_compiled"] or any(decision[key] for key in ("positive_width_projective_cells_evaluated", "zero_inclusive_projective_boundary_majorants_complete", "recursive_positive_interior_cover_complete", "complete_hybrid_integrals_emitted")):
        raise AssertionError("K395 decision boundary changed")
    if not all(payload["release_test"].values()):
        raise AssertionError("K395 release test failed")
