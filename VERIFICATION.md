...
| A supplied three-stage action complex has gauge images inside cycles; chain maps preserve cycles and gauge equivalence; corrected observation is a chain map when its projector fixes observed gauge images and is invisible to the observed equation map; the frozen strict packet cannot select any injectively parameterized action family | `Lean/GUFormalization/SourceNativeActionObservationComplex.lean`; Lean `gaugeImage_isCycle`, `corrected_mapsCycle`, `corrected_mapsCohomologous`, and `strictSource_does_not_select_injective_actionFamily`; exact rational hostile probe; default-target axiom receipt | L1 conditional linear/homological interface; not a source-selected action, split, domain, positive physical Hilbert space or state quotient |
