# published-papers/candidates/

Papers that have **graduated out of `draft-papers/`** and are staged for publication, but are **not yet
...
1. **`draft-papers/`** -- work in progress. All drafts, notes, and **previous/superseded versions** of a
   paper live and stay here. Edit freely.
2. **`published-papers/candidates/`** (this folder) -- a paper graduates here **only when Joe has explicitly
   said he wants to publish it.** The current version moves; its earlier versions stay in `draft-papers/`.
   Treat as near-final; substantive changes should be deliberate.
3. **`published-papers/`** (the parent) -- the public record. A paper moves up here **only after Joe informs
   that it has actually been published** (arXiv id / DOI / live URL recorded). Append-mostly.
