...
1. **Delete nothing.** Every file is preserved. Cleanup = `git mv` (history-preserving moves) + new
   index files. Retiring something means moving it under `lab/archive/`, never removing it.
...
| `papers/drafts/` | 29 | the live arXiv paper is buried among 8 `what-GU-needs-next` versions + superseded drafts + process docs |

Most other directories (`canon/`, `lab/process/`, `lab/roadmap/`, `lab/sources/`, `lab/literature/`, `lab/active-research/`,
`lab/archive/`) already carry a `README`/`INDEX`. The convention exists; it just isn't applied to the loose dirs.
...
papers/drafts/        <- work in progress; the "Located, Not Forced" submission candidate is surfaced at the
                        top of its README, superseded drafts grouped below
papers/published/    <- EXISTING convention; empty until Joe hand-moves a paper here on public posting
...
lab/process/  lab/roadmap/  lab/sources/  lab/literature/  lab/active-research/  lab/specifications/  lab/automation/  Lean/  lab/archive/
```
...
- **KEPT at root by design:** the operational status trio `NEXT-STEPS.md` (85 live referencing files),
  ...
  Root drops 14 -> 10 `.md`.
- **No new `paper/` folder.** The repo already has the deliberate `papers/drafts/` (work in progress) vs
  `papers/published/` (hand-moved on public posting) split.
- **DONE (commit f1f6e4c) -- the paper-staging workstream.** A workflow census + light adversarial triage
  found exactly two publishable papers; both graduated into `papers/candidates/`:
  ...
  - Lifecycle codified: `papers/drafts/` (WIP + superseded versions) -> `candidates/` (graduated on Joe's
    explicit publish-intent) -> `papers/published/` (only after Joe confirms posting). See
    `papers/published/README.md` and `candidates/README.md`.
...
- **RESOLVED -- no `paper/` folder.** Use the existing `papers/drafts/` vs `papers/published/` split; surface
  the submission candidate inside `papers/drafts/` (Phase 1).
- **RESOLVED -- run all 5 phases** (Joe, 2026-06-29), per-phase commits.
- **Separate workstream -- `papers/candidates/`.** Stage the genuinely publishable papers
