...
| P-H22 | Retracted verdict alive in machine-readable frontmatter: `external-datum-ledger…07-29.md` title/outcome still "P3-IS-NOT-EXTERNAL" + body table row, against its own correction banner; companion `source-action-term-by-term…` title carries retracted D3 | **PARTIAL 2026-08-26.** Hostile replay exposed a larger live footprint: `tests/channel-swings/external_datum_ledger_probe.py` still prints `P3-IS-NOT-EXTERNAL`, says the pieces reduce from three to two, and withdraws P3 while returning exit 0. Correcting only the prose/frontmatter would leave executable evidence contradicting current truth, so the attempted content edit was reverted. Re-admit the two explorations together with that probe and its current callers/pins; preserve the exact multiplicity checks while changing their claim ceiling and verdict. | H | XS |
