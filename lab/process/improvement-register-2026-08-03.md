...
| M-H5 | **VERIFIED LIVE (D1-D3 advanced 2026-08-27).** On the fixed `RP³` spine, `ν=R+Sym²(Q*)` has rank 7 and `w1=w2=w3=0`, so it is trivial in the stable range and its sphere bundle is noncanonically `RP³×S⁶`; the sphere row is untwisted and the model is stably parallelizable. For the conditional global extension, `w2(TL)=pi*w2(TX)` and the free/rational degree-four obstruction is `p1(TL)=7pi*p1(TX)`. | Supply the actual global normal/TX packet or D4 reframing orbit; separately prove reflection compatibility with spin/horizontal data and the full base eta-form integral | H | M |
