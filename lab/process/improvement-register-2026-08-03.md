...
|---|---|---|---|---|
| M-C1 | **FC3 fired, unrecorded**: repo's own cert computes ch₂(S(6,4))[K3] = −1152 ≠ 0; `ind_H = Â·rank = 16` arithmetically dead; 16+8=24 chain over; live consumers remain (decider PART C, NEXT-FRONTIER H1(c)) | **VERIFIED LIVE 2026-08-08, still unexecuted.** `tests/gen_ch2_sx_from_codazzi.py` hard-asserts `ch2_normal == -1152` and the cert reports `ch2(S_X)[K3] = -5376`, decision `CH2_NONZERO_OTHER_INDEX`, "genuinely nonzero, decisively NOT 24". That kills the `Â·rank = 16` arithmetic, which assumed `ch2 = 0`. **Fence, from the cert itself:** `pi_!` is `NOT_DEFINED` on the non-convex fibre, so `ch2(S_X)[K3]` "is not yet THE index" — the chain's premise is dead while its replacement is unknown, which is why the action says retire the chain rather than assert a new count. Retiring is verdict-adjacent and needs the CS workflow plus hostile review. Original action: record FC3 FIRED; banner oq-rk2 + consumers; retire the chain from live docs (CS) | C | S |
