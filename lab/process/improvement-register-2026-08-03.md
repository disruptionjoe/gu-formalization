...
| P-H6 | Reproduction has no committed baseline: no green-run receipt, no expected pass count/runtime; a skeptic's RED is undiagnosable; sweep never flushes stdout (0 bytes for 30 min) | Commit a dated 780/780 receipt; add `flush=True` (reproduce_all.py:190) | **CAUSE FOUND 2026-08-08 (side track).** The missing baseline receipt hid a live break: `scipy` is declared in `requirements.txt:14` and pinned in `requirements.lock:75` at `==1.18.0`, **95 of the 1109 certificates under `tests/` import it, and it is NOT INSTALLED in this environment** — so ~8.6% of the certificate corpus cannot execute at all. Zero `process_gates/` files import scipy, so the ~49 gate failures are unrelated and that analysis is unaffected. This is exactly what a dated green-run receipt exists to catch, and its absence is why nobody noticed. Blocks `M-H17` (ii)-(iv), whose Koszul-Tate bicomplex is a scipy consumer — that item is blocked on environment, not on mathematics. | S |
