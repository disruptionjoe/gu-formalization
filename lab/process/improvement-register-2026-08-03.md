...
| P-M7 | "Positive controls first" is a real convention in 141 files but documented nowhere, and violated in 3 named certs (controls after the claim assert — machinery-dead vs claim-false indistinguishable on failure) | **PARTIAL 2026-08-26.** `tests/README.md` now defines the execution-order convention and its exception rule. The original register and introducing commit do not preserve the three filenames, so no certificate was guessed or reordered. The row remains live until those exact historical targets are recovered or a separately admitted current census identifies a replacement population. | M | S |
...
| P-M10 | Repo-wide status vocabulary drift: ~8 undeclared values (active, staged, preregistration, completed, complete, blocked, verified…) vs the declared seven — makes the map non-machine-checkable | **PARTIAL / PREMISE EXPANDED 2026-08-26.** A tracked Markdown-frontmatter census finds 2,830 status-bearing documents: 1,378 use the declared seven document-role values and 1,452 use 911 other distinct values (918 total), mixing verdicts, run/review phases and append-only ledger outcomes. `GRADES.md` now states the semantic boundary and forbids string-substitution normalization. The row remains live: first freeze a complete protected population and select a schema that separates document role, claim verdict and operational state; only then add a lint/ratchet. | M | M |
