...
| P-M25 | PS-chain verification file: signature-stale (rooted at Spin(7,7), no banner, updated 06-20), ships inside the LNF zenodo package while canon asserts (9,5) | **VERIFIED LIVE; PROTECTED APPLY REMAINS (2026-08-27).** The required fence must distinguish the settled source algebra at split `Cl(7,7)` / `Spin(7,7)` from the still-open ambient-signature fork between the repository's `(9,5)` reconstruction and source `(7,7)` presentation, and classify the complex `Spin(10)` / Pati--Salam branching as a real-form-insensitive conventional comparator. The unattended protected-surface guard correctly rejected editing `lab/active-research/`; apply the notice only in a separately reviewed protected pass. Reopen the content question if the source real-form or ambient-signature disposition changes, or an explicit typed real bridge is constructed. | M | XS |
...
| P-L12 | H44 fixed-point tol 1e-12 below machine epsilon for its own quantity — loop never exits early; converged_delta is roundoff | **EXECUTED (verified 2026-08-27).** The fixed-point stop is now a dimensionless relative residual with an explicit unit floor, while the absolute update remains separately reported. H44 passes with relative residual `1.8e-16` and absolute update `1.8e-15`; every scientific check and verdict is unchanged. Reopen only if the solver scaling or fixed-point quantity changes. | L | XS |
| P-L13 | W129 f0_9 log-interp across a 2.5× grid gap; brentq already used elsewhere in the file | **EXECUTED (verified 2026-08-27).** The first increasing `Delta chi2=9` straddle is now refined by a direct bracketed Brent solve through the calibrated likelihood, returning `f0_9=0.0302, 0.0418, 0.2213` for `M2=8,7,3`. The band verdict remains `HOLDS-BAND-WIDE` and the minimum CPL-matched penalty remains `Delta chi2=+33.46`. Reopen only if the likelihood, calibration, model or scan bracket changes. | L | XS |
| P-L14 | **EXECUTED (verified 2026-08-27).** RB6 and the three affected Rarita–Schwinger C2 certificates now use one scale-covariant singular-value rank helper: the cutoff is relative to each nonzero matrix's largest singular value or, for commutator noise, a covariantly scaling parent-word magnitude; the zero matrix remains rank zero. The coupled probe preserves rank under `1e-12`, `1`, and `1e12` rescalings and catches independently planted absolute-cut regressions in all four files. | Reopen only if a certificate changes its rank resolution policy or introduces another absolute/unit-floored rank decision | L | S |
...
| M-H13 | **PREMISE CORRECTED (2026-08-27).** W187's `r=|Sigma_ext|/|Sigma_internal|` is an open-system self-energy ratio, not a density, equation of state or coefficient in `H(z)`; only `dr/dN>0` is directionally native and the square-root exponent is modelled. The actual record-sourced mean `Lambda=c/sqrt(N)` decreases under accretion, while the W230 record-current/connection bridge is conditional on unowned `c_kin=0`. A direct `r(N(z))` refit is therefore a homonym. | Reopen only with an owned record-to-FLRW bridge, an owned backreacted `N(z)` definition and an exact coupling target/normalization | H | M |
...
| M-M5 | **EXECUTED (semantic currency reconciled 2026-08-27).** The direct noncompact route already fails for the actual object. For `SL(4,R)/SO_0(3,1)`, scalar relative discrete series fail at rank `3 != 1`; the displayed Lorentz-spinor coefficient is nonunitary, so the ordinary Hilbert `L^2(G x_H tau_RS)` object is not defined as stated; `(sl(4,R),so(3,1))` is absent from the Kobayashi--Oshima discretely-decomposable classification; and the compact spinor families retain the nonzero diagonal asymptotic cone. The proposed `rank_correction(tau_RS)=2` is therefore not a licensed analytic proof. The independent physical `C^32 -> C^16 -> dim_H=8` count remains reconstruction-grade and is not upgraded or erased. | Reopen only with a precise nonunitary Casselman--Wallach/hyperfunction problem or a primary vector-bundle theorem whose hypotheses cover this coefficient and pair; do not rerun the failed rank-subtraction route | M | M |
...
| M-M14 | **EXECUTED (verified 2026-08-27 at the family-reduction typing ceiling).** IDX-02 now separates the fixed round-product facts `A-hat[S6]=0`, `eta(S6)=0`, and zero kernel rank from the surviving Bismut–Cheeger family term `integral_B A-hat(TB) wedge eta-tilde(D^Z)`. GU's actual horizontal family/superconnection and base integral are unconstructed, so the fibre reduction leaves the GAUGE/TANGENTIAL fork `UNRESOLVED`; the independent integer controls remain unchanged and still do not force three. | Reopen only with an owned actual GU operator family, horizontal superconnection and evaluated base integral; do not infer family eta-form vanishing from fixed-fibre symmetry | M | S |
...
| M-M20 | **VERIFIED LIVE; CONDITIONAL AT THE ASSUMPTION-FENCE CEILING (2026-08-27).** A minimal 13-dimensional defect action would become a shared surplus object only after two presently unproved identifications: the 13-dimensional count boundary with the firewall adapter, and that firewall adapter with the boundary-value-problem boundary. No artifact establishes either map; firewall canon calls the relation a candidate. | Reopen only with D3's owned boundary operator/domain, D4's source/action ownership and an exhibited map for each identification; until then do not schedule the boundary-action consequence as established | M | M |
| M-M21 | **EXECUTED AT THE CONVENTIONAL-COMPARATOR/TYPE CEILING (semantic currency reconciled 2026-08-27).** The exact Spin(10) chain already answers the named conventional question: a `126` rank-drop direction leaves matter parity `(-1)^(3(B-L))`, a physical `Z/2`, not a residual `Z/3`. The repository's count/boundary arena is separately `Z/3`; `Hom(Z/2,Z/3)=0`, and no owned carrier/action/observation map identifies the two. This excludes only the standard `126` residual-`Z/3` route and is not evidence for or against Weinstein's curvature-induced connection mechanism. | Reopen only with an explicit typed bridge that owns both carriers, real structures, action/reduction maps and the transported observable; otherwise preserve `Z/2` matter parity and boundary/count `Z/3` as distinct objects | M | M |
| M-M22 | **PREMISE CORRECTED (2026-08-27).** The exact PS matching system separates the bought standard `sin²θ_W=3/8` boundary normalization from scale dynamics. `M_PS` and `M_U` are identifiable only after interval beta coefficients are owned and their one-loop determinant is nonzero; a two-loop run additionally needs the mass spectrum, breaking assignments, gauge matrices, Yukawa/scalar inputs, scheme and threshold matching. None is supplied by current GU representation content, so the honest ceiling is `RGE_UNCOMPUTABLE_PREBETA`. | Reopen on one owner-frozen target-blind spectrum, beta and matching packet; decay/oscillation outputs need their further operator owners | M | M |
...
| M-M25 | **PREMISE SHIFTED (semantic currency reconciled 2026-08-27).** A local Crandall--Rabinowitz normal form on the incumbent homogeneous branch cannot decide curved continuation before the branch has an owned coercive completion. The frozen mixed Gram has rank nine with the full-trace line as kernel, and a nonzero `D0=C=0` commuting configuration lies on that line; the necessary coercivity condition therefore fails. No source-owned derivative, curvature, parent-action or section term currently removes the witness. | Reopen the bifurcation calculation only after an owned term removes the exact commuting-kernel witness and the full action/domain supplies the Fredholm/transversality hypotheses; do not compute a normal form for the noncoercive surrogate and call it curved continuation | M | M |
| M-M26 | **EXECUTED (verified 2026-08-27 at the primary-source scope ceiling).** Victor S. Shulman's 1997 theorem supplies a precise Tomita analogue and double-commutant theorem for a weakly closed `J`-symmetric algebra on a pre-existing `Pi1` space with a cyclic and separating vector. It does not derive the indefinite form or `J`, and the cited paper supplies no higher-rank extension. The former absolute literature-nonexistence wording is retired: the honest residual is that the higher-rank/infinite-index construction needed by GU remains unestablished here. | Reopen only with a primary theorem whose hypotheses cover higher negative index or the actual GU infinite-index algebra; absence from the Pi1 paper is not a literature no-go | M | S |
...
| M-M32 | **EXECUTED (verified 2026-08-27 at the assumption-fence ceiling).** M-M20 now declares its dependence on A1 and A2: count boundary = firewall adapter and firewall adapter = BVP boundary. Neither identification is owned or established; the firewall relation remains a candidate. No boundary action, surplus, or physical consequence is inferred. | Reopen only with D3's owned boundary operator/domain, D4's source/action ownership and an exhibited map for each identification | M | XS |
