...
- `conditional-physics-ledger-v0.16.json` - current machine ledger; 82 active targets, exact fixed-background field-mixing TT `C`, classified Jordan/complex/disconnected/scalar loci and preserved full nonlinear/Fock/domain boundary, with verdict counts `33/19/24/6` and residue `84 + >=19 functions + 9 forks`. Four scoped quotients remain ranked without booking a global residue reduction; v0.15 remains frozen history.
- `first-perturbative-background-c-operator.json` - machine receipt for the action-owned fixed-background TT Hessian, unique free-connected positive spectral `C`, zero-freedom first-order lift, exceptional-locus classification and hostile-review boundary.
