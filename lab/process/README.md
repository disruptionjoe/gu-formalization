...
Current append-only progress surface: ledger v0.131. The old fixed-carrier
Casimir test is non-discriminating and cannot select the proposed `192`; all
exact fixed-`W` theorems survive conditionally. The v0.130 action-owned moving-
reduction gate remains rank one and now also owns a discriminator against
...
promotion, downgrade, or correction work routes through the claim-status
...
- `pw2fr2b2b2h2-i2b-second-residual-primalizer-pairing-registry.json` -
  ...
  `(981/64,0,4293/128,0)`, exact mixed action `-103/256`, and complete
  five-family reassembly with three live routes. The scoped fixed-background
  dependency closes; the global/source-epsilon curvature graph and both
  complete C4 banks remain open.
...
- `pw2fr2b2a-second-frechet-c4-graph-registry.json` - PW2F-R2B2A ledger:
  ...
  one co-moving frame, `h/theta1/Bhat2` second graph, five coefficient slots,
  and separate `I1`/`I2B` Hessians remain open for R2B2B.
- `pw2fr2b1-section-jvp-source-coordinate-registry.json` - PW2F-R2B1
  ...
  exact rank-35 quartic/global-`kappa1` gate. R2B2A used that gate but exposed
  a source-tangent/second-graph block now owned by R2B2B.
...
- `conditional-physics-ledger-v0.36.json` - immutable predecessor; the complete grade-one Schur correction generically lifts the graph-only null tensor pair and isolates the positive N2 two-mode locus for typing.
- `selected-action-grade1-dbt-schur-observation.json` - immutable exact grade-one Hessian, full source-cross, Schur, observation and coefficient-locus receipt.
...
- `selected-cubic-reduced-numerator-completion-fork.json` - Wave-1 receipt for the exact free-pencil legs, diagonal shell zero, rival mixed completions, symplectic bulk/preboundary fence and full-moving-third-derivative successor.
- `trace-q-higgs-chirality-admission-test.json` - pre-wave SOURCE/COMPOSE/VERIFY receipt: bare `gamma(q)` is not admitted as a K-definite chiralizer on the legacy carrier; `q` remains the D9.16 receiver and the corrected `T_omega -> sigma_epsilon -> h_omega` route stays conditional without moving the ledger or consuming P1/P2/P3.
- `conditional-physics-ledger-v0.17.json` - frozen predecessor ledger for the zero-field scalar-Hessian correction and three-species odd-channel denominator shells before the numerator computation.
...
- `first-perturbative-background-c-operator.json` - frozen predecessor receipt for the action-owned fixed-background TT Hessian, unique free-connected positive spectral `C`, zero-freedom first-order lift, exceptional-locus classification and hostile-review boundary.
- `conditional-physics-ledger-v0.15.json` - frozen predecessor ledger for first-interaction free-parity obstruction, finite-local constant-mode closure, conditional global zero-mode screening and algebraic super-IG source correction.
