...
| `Lean/GUFormalization/GroupActionMackey.lean` | Complete pure set-level subgroup Mackey decomposition: `K`-orbits of `Res_K^G Ind_H^G(1)` are `K\\G/H`; actual intrinsic index fibers give a canonical representative-free `K`-equivariant coproduct decomposition; equivariant seed maps preserve the index and act naturally on the fibers; every transported-intersection summand is equivariantly equivalent to its intrinsic fiber; and same-double-coset representatives are explicitly equivalent with unchanged target assembly | `LEAN-VERIFIED`; 2026-08-30 focused and serialized 8,662-job default-target integration; intrinsic index invariance, seed-map naturality, fiber actions, representative-to-fiber bijectivity, representative-change equivariance, inverse laws and quotient dependencies are explicit; theorem receipt exposes only the established `propext`, `Classical.choice` and `Quot.sound` surface; W99 checks the nonnormal `S3` seed involution and an alternate representative of the four-class double coset, including equal fiber images, bijection, `K`-equivariance and unchanged assembly |
