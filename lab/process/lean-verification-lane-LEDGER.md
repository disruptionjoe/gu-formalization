...
| `Lean/GUFormalization/GroupActionMackey.lean` | Complete pure set-level subgroup Mackey decomposition: `K`-orbits of `Res_K^G Ind_H^G(1)` are `K\\G/H`; actual intrinsic index fibers give a canonical representative-free `K`-equivariant coproduct decomposition; equivariant seed maps preserve the index and act naturally on the fibers; and transported-intersection summands with a classical representative choice are explicitly compared to it | `LEAN-VERIFIED`; 2026-08-30 focused and serialized default-target integration; intrinsic index invariance, seed-map naturality, fiber actions, inverse laws, representative comparison and quotient dependencies are explicit; W99 checks the nonnormal `S3` seed involution is quotient-well-defined, index-preserving, `K`-equivariant and involutive, alongside the chosen 2+4 summand assembly over all six induced classes |
