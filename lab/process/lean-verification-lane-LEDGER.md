...
| `Lean/GUFormalization/SourceNativeActionObservationComplex.lean` | Three-stage action complex, chain-map descent on cycles and gauge-equivalent representatives, corrected-observation chain-map weld under two exact compatibility laws, and frozen-packet nonselection for injective action families | `LEAN-VERIFIED`; 2026-08-31 focused Wave 2 integration; Mathlib linear-map substrate plus independent exact hostile controls; no source-selected action, split, analytic domain, positive physical Hilbert space or GU verdict |
