...
| `Lean/GUFormalization/GroupActionMackey.lean` | Complete pure set-level subgroup Mackey decomposition: `K`-orbits of `Res_K^G Ind_H^G(1)` are `K\\G/H`; actual intrinsic index fibers give a canonical representative-free `K`-equivariant coproduct decomposition; and transported-intersection summands with a classical representative choice are explicitly compared to it | `LEAN-VERIFIED`; 2026-08-30 focused and serialized default-target integration; intrinsic index invariance, fiber actions, inverse laws, representative comparison and quotient dependencies are explicit; W99 checks both canonical nonnormal `S3` fibers and the chosen 2+4 summand assembly over all six induced classes |
