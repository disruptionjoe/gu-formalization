---
title: "Agent Context Pack: the one-read distillation for GU work agents"
status: process
...
> It has `Cl1` domain and is `xi -> k tensor xi`; the source adjoint map has
> `Cl2` domain, shape `196 x 91`, and selected rank `25`. The Euler target
...
> one was constraint propagation and gauge typing. The corrected principal
> complex above resolves the non-null symbol kernel but not source gauge;
> lower-order characteristic assembly, physical reduction, then nonlinear
> atlas/observation and BV ownership remain. The
...
> Ledger v0.70 preserves the v0.69 diagonal two-connection Ward and boundary
...
> perspective vote is planning evidence, never scientific evidence.
...
> This predecessor fence is absorbed into the v0.36 grade-one packet.
...
> identity owns the unrestricted preboundary potential. Do not rebuild the
...
> Clifford scalar pairings are exactly natural in the co-moving frame. A
> selected intrinsic action witness is nonzero (`-7/2`) with zero pure-frame
...
- Constraint-surplus doctrine: posits with more independent constraints than
  free parameters are informative; count, don't argue. Label findings
...
Read `lab/methods/research-evidence-contract-v1.0.md` and its JSON
contract after `lab/process/RESEARCH-AGENDA.json`. Lanes remain purpose-bearing; Build, Compose,
...
- CARRIER SPLIT (GEOMETER-VS-PHYSICS-OBJECTS.md:24): Lorentzian (3,1)+(6,4)
  vs compact (4,0)+(5,5) [the finite census uses the compact side] — plus a
...
The imposter = the relative gamma-trace mode, (10,−4) antidiagonal,
allocation-invariant; unique bridge (cut vertex) between the two native
blocks; reunification asymmetry computed (10⊗16 = 144⊕16; spin-3/2 slots
stranded); blocks are 3:9:1, not peers; count killed five ways; strong
textual hinge form falsified by the 2021 draft (only the third term is a
generation; the "2" come from a different decomposition). 4+10 is forced
...
- lab/process/eleven-lens-audit-2026-08-03.md — verified findings + fix
  packet. Meta-pattern: artifacts honest, dashboards stale-optimistic;
...
from the private orchestration overlay root. Never commit unless you are the orchestrator.
