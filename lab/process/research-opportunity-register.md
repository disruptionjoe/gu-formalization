...
Ranked backlog of candidate research moves: what each would move if it succeeded,
which lane owns it, what it would cost, and what currently gates it. The process
...
| --- | --- | --- | --- | --- | --- |
| **ROR-01** | Action-owned physical fermion/BV operator producing half-asymmetric cohomology or a fermion projector that discriminates `W` / mirror / random 192s / `H640` / `832` without fitting the large trace-`Hq` fibre | 1 | The whole carrier-selection problem. Would convert connection-arena structure into something that actually *selects* a physical carrier | high effort, well-typed | **This is the live front, not backlog.** Verbatim the repo's `current_question`; `next_condition` prescribes it. Do not rerun `Hq` compatibility; do not apply the bosonic projector to fermion subspaces |
| **ROR-02** | Principal-*changing* source-action / `Q_B` owner with zero torsion class or controlled absorption through cubic/quartic order | 1 | The missing source action itself. Principal-*preserving* absorption is already universal; the changing case is the open half | high effort | **Explicitly typed a disjoint fallback in `next_condition`.** Genuinely parallel to ROR-01 by construction, so it is the strongest candidate for a non-colliding second track |
...
**The top cluster is mis-billed as newly tractable.** ROR-01 through ROR-03 are the
unbuilt source action, which is the standing gate blocking SHIAB selection, the
generation count, RS-BRST and dark energy *simultaneously*. They rank highest
because they are the blocker, not because a workflow has newly opened them. Expect
months, not sessions.

**The genuinely newly-runnable entries are ROR-08 and ROR-09**, and they are the
answer to "what can run in parallel without colliding." Lane 3 hardens frozen
results, so by construction it does not contend with Lane 1's construction work.

**ROR-02 is the best candidate for a second construction track**, because the repo
has already typed it as a disjoint fallback rather than because it is easy.
