...
- Before proposing new work, read this file and `CURRENT-STATE.yaml`. If the move is
...
- **P-H29 / M-C2 gate:** truth-status research admits no new interior wave until the RB6/RB7/W177
...
- **Multi-writer protocol:** the scheduled cadence owns `NEXT-STEPS.md`, the
  conditional-build ledgers, and `lab/process/CURRENT-RESEARCH-CONTEXT.md`;
...
`CURRENT-STATE.yaml` and `NEXT-STEPS.md` at `06084deb`.
...
| **ROR-03** | Physical tangent / BV graph with a full Layer-0 fingerprint reset, shown consistent with existing stationary jets and Ward data | 1 | Closes the compositional gate truth-status research names as its next parallel gate | moderate; composition + type checking | Named in truth-status research's own summary. Layer-0 reset is the load-bearing part, not the composition |
...
| ID | Gate |
| --- | --- |
| ROR-06 | prediction research's open internal decision, plus the standing do-not-rerun-DE-AMP instruction. Not a source-action gate |
| ROR-10 | Needs a candidate source-action packet to compare against, so it genuinely sits downstream |

### Remaining honest notes

**ROR-08 and ROR-09 are the safest parallel track**, because result-hardening research hardens frozen
results and by construction cannot contend with truth-status research's live work. ROR-02 is the
...
- Re-anchor the table against `CURRENT-STATE.yaml` and `CURRENT-STATE.yaml` whenever a
