---
title: "Exploration absorption and construction priorities"
status: active_research
artifact_type: priority_reconciliation
created: 2026-08-10
---

# Exploration absorption and construction priorities
...
## Cheap integrity queue

- **Immediate disjoint Verify task:** apply the literature-settled H10-01
  remediation before its result is reused. The spin-2 ghost Yukawa coefficient
  is `-4/3`, the spin-0 scalaron coefficient is `+1/3`, and the current file
  deletes the wrong term when GU has no `R^2`. Correct the nine test sites and
  three exploration sites, rerun wave22, and preserve the literature/source
  receipt. This need not block the action-owned Build if scopes are disjoint.
- Rename the overloaded `sigma` only through a provenance-preserving migration.
- Run the long-owed `F_2`/UNSAT frustration test as an independent,
  non-conflicting fallback; it decides uniqueness versus a domain of section
  structures and does not depend on the action reduction.

## Deferred until dependencies exist

Do not spend Build priority on triality analogies, b-calculus, GHP contour
choices or dark-matter interpretation. Each becomes useful only after the
action-owned carrier and domain expose the object it would act on. At scoped
grade, `Q(B)` is deprioritized as a GU-native observational discriminator
because it lies inside published Stelle quadratic-gravity phenomenology; use
those constraints rather than re-deriving it as a GU-specific selector. This
does not establish the phenomenology of the complete GU action.
