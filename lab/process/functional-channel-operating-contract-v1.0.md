...
`lab/process/conditional-physics-ledger-v0.133.json`, paired with
`explorations/conditional-build/conditional-physics-ledger-v0.133.md`.
...
`192` carrier. v0.133 closes the zero-branch bosonic action question: the full
connection-displacement norm gives the rank-8256 complement a nonzero Hessian,
so a hard moving-Spin restriction is not generated by that action. The
distinct Weyl-compatible connection restriction excludes the known odd-
`Phi1` nonzero invariant branch. Build computes the complete nonzero-branch
normal Hessian next, then constructs the induced K77 Dirac/RS operator before
using its kernel/cohomology against mirror/random/`640`/`832` controls.
