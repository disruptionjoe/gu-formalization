---
title: "GU functional-channel operating contract v1.0"
status: ratified
artifact_type: operating_contract
ratified_by: "Joe direct chat, 2026-08-05"
machine_contract: research-evidence-contract-v1.0.json
---

# GU functional-channel operating contract v1.0

## Outcome

GU keeps its purpose-bearing Lanes 1, 2, 3 and A. Work inside those Lanes is
now routed through four functional channels: **Build, Compose, Source and
Verify**. The channels do not create new scientific lanes and do not replace
the repository portfolio. They make the method of reaching a Lane outcome
explicit.

The conditional-physics ledger is the standing compositional deliverable and
progress meter. Construction is not complete merely because its target set is
fully typed. Progress means that coverage is honest, row distances or residue
fall, a prediction becomes sharper, or a scoped adverse result rules out a
...
hypercharges. Fixed q then leaves post-Higgs `SU(3)xU(1)`, dimension 9. Orbit
plus one radial coefficient has the four real components of a weak doublet,
but the 20-dimensional J family and the radial full/two-half `varpi` owner are
unselected. Build next derives the radial coefficient's observation descent,
kinetic term, potential, Yukawa placement and stationary nonzero amplitude.
Carrier, action owner and physical Higgs remain distinct.
...
this principal fingerprint because their distinction is lower order. v0.136
...
fundamental symmetry, closed domain and Fredholm realization.
...
exactly Spin-native. A block-preserving `U(32,32) x U(32,32)` action on the two
`C^(32,32)` Weyl halves generates a `16,382`-dimensional closure with three
invariant pairing coordinates. Full `U(64,64)`, which can mix the halves,
generates `sl(128,C)`, dimension `16,383`, with one invariant line. Thus the
two Weyl halves and full group are both retained, but neither large group
...
unrestricted endpoints. The source and local action do not select boundary
gauge redundancy versus charged physical symmetry. Full boundary gauge plus
generic nonzero action momentum conditionally selects the unique minimal edge
horn, with zero coefficient freedom and no net reduced physical dimensions;
the full-boundary-gauge predicate remains unowned and the charged horn remains
a comparator. The actual conditional K77 `H`/action trace and full
...
chirality-preserving block product. The closure theorem above now types that
...
unrestricted boundary moment map. v0.101 proves source/action boundary
...
`-J_2D` targets have unique all-grade lifts, preserve full Bianchi, and descend
with the full labelled reduction. The source owns the response but is silent
...
Ledger v0.64 derives that action-owned companion from the actual Fréchet
owners: `D_B^!(E_B-E_T)+(D_epsilon S)^!K_S`. Exact all-entry
differentiation keeps both connection Euler owners and moving Shiab live,
rejects naive `D_A E_T`, and separates the generally nonzero primitive
epsilon Euler equation from the zero homogeneous moving-Shiab Ward
...
diagonal two-connection Ward descent, with live unrestricted boundary charges.
...
Ledger v0.83 closes the source-owned horizontal `D_varpi Upsilon` block: it
is injective on 24 dimensions with 56 live output coordinates, while its
diffeomorphism interface has rank three in every causal class. The older
metric-only diagnostic has rank-four Ward load, so it cannot be imported as
the common-field stationary `g-g` Gram block on the fixed-`epsilon` horn. This
is not an absolute kill. Source `epsilon` is an independent action variable;
Build must construct physical `D_g Upsilon` and `D_epsilon Upsilon` together,
prove complete `J R=0`, then derive the residual pairing, formal adjoint and
...
metric and varpi torsion responses cancel on all four orbit columns, but the
full varpi derivative retains a rank-three curvature packet. The moving
Shiab/Hodge/curvature/density/observation target is therefore narrowed from
...
0C. Thin automation triggers should point to the owner and let `lab/process/RESEARCH-AGENDA.json`
...
All three causal representatives and both exact branches have full ranks
`9/91/97`, horizontal ranks `9/6/12`, and off-slice ranks `4/88/89`. This
kills only frozen-Hodge/Phi/Shiab/frame/observation closure. The next Build
must construct the total moving source-coordinate Hessian and test exact
cancellation of that rank-89 target before choosing 321 versus 1,571 or
expanding parents. No datum, verdict, residue or quotient moves.

### Predecessor v0.121 construction frontier
...
points are noncritical in that direction and it is not source-owned; an exact
coordinate change alters the determinant. On the actual scalar `varpi` line,
both branches have the same inertia class for each distinct action. Build must
therefore port the first-action epsilon/Cl1 cross and residual-square
`g/varpi/epsilon` Jacobian separately to both branches and each retained
parent. Never transfer real positivity or Hessian data by Galois conjugacy.

### Predecessor v0.117 construction frontier
...
Build constructs or varies a mathematical object against named ledger rows.
It starts from the typed object and its source/geometry owner, not from a
generic wish to harden the repository.

A Build output succeeds by doing at least one of the following:

- reducing a row's `distance` or the global residue;
- supplying a missing map, action term, quotient, domain or datum;
- sharpening a prediction or kill condition;
- proving a scoped no-go and naming the nearest constructive replacement; or
- discovering that the target was mistyped and returning it to Compose.

Every Build wave declares its ledger rows before computation and emits the
current ledger meter plus the rows' before/after states. If no row changes, it
must say `ledger_row_changes: none` and give an evidence-backed reason. Build
does not infer physical recovery from carrier containment or familiar names.

### Compose

Compose owns the denominator, aliases, migration edges, row typings,
reason-kind taxonomy, progress meter, residue inventory, contradiction
discovery and cross-composition of already established results. It is a
first-class deliverable, not spare-cycle support for Build.

Compose succeeds when it adds a target at grade, raises honest coverage,
changes a row's distance/tightness/residue, finds an over-determined row, or
uses existing evidence to retire, redirect or reduce a Build task. A newly
found over-determined row is valuable even before its disposition.

Rows carry a verdict and an extensible `reason_kind`, plus `distance`,
`revival_trigger`, evidence and mapping grade. A case that fits no existing
kind creates `NEW_KIND_REQUIRED`; forced fitting is forbidden. In particular:

- `SAME` distinguishes derived, conditionally/partially derived, imported and
  put-in agreements;
...
The finder of an over-determined row escalates it and may not adjudicate it.
An independent owner assigns one of the four dispositions and preserves the
original evidence and migration edge.

Compose runs after three material Build outputs and immediately when any
verdict, residue count, discrete fork, high-fanout premise, source correction
or adverse row changes. This is also the breadth/archaeology checkpoint that
asks whether the repository already solved, superseded or contradicted the
next expensive target.

### Source

Source reconstructs what Weinstein, Curt and other cited sources actually
claim, with timestamp/page, object type, stated role and omissions. A decisive
Eric-lane pass, kill or no-go must record exactly one return:

- `SOURCE-CONFIRMS`: the primary source states the same typed object and
  relation;
- `SOURCE-CORRECTS`: the primary source changes the question, type, fork or
  attribution; or
- `SOURCE-SILENT`: the source does not decide the construction burden.

Source evidence can redirect a construction and correct attribution. It
cannot by itself prove the mathematics, select a datum or promote observed
physics. When a construction hits a failure in the Eric lane, Source checks
the Weinstein material for that exact object before the failure is propagated.

### Verify

Verify attacks new, changed or high-fanout claims. It does not consume a run
by replaying an unchanged packet merely because Build is occupied. Replays are
...
- **Analytic lens — whenever the artifact makes a domain, spectrum, index or
  ...
  the 41 filed perspective passes contained zero coverage of complex analysis, path
...
independent reviewer per perspective is not required. Controls verify the stated computation;
...
test and scope. This adaptive core-plus-triggered rule replaces perspective-count
...
There are no fixed channel percentages. Select the highest-information
eligible condition:

1. If the target object or current premise is semantically/source-uncertain,
   run Source or Compose before Build.
2. If an over-determined row awaits independent disposition and has high
   downstream fanout, adjudicate it before dependent construction.
3. If a typed row has a buildable distance and its premises are current, run
   Build.
4. If three material Build outputs have landed, or a high-fanout fact changed,
   run Compose before another expensive Build.
5. Run Verify on changed/high-fanout claims or a named integrity risk.
6. If a GU writer holds the central claim, another automation may do useful
   read-only Source, Compose or Verify work on a disjoint gate. It may not
   create a competing owner write or call unchanged replay progress.
...
1. the Runtime central owner claim reserves GU repository writes for one
   closed execution envelope; and
2. each Run declares its gate, ledger rows and exact writable paths so other
   work can identify a non-conflicting read-only channel or wait.

Before closeout, reconcile the result against current `HEAD`, the latest ledger
...
The current standing denominator is the append-only conditional-physics
ledger. Its axes are Lagrangian/equation terms, representation requirements and
anomaly/consistency conditions—not particles as a separate counting axis.

Every row-touching wave emits a line of this form from the machine ledger:

```text
Ledger v0.N — mapped/total target rows mapped (percent of frozen denominator)
SAME · DIFFERS · NEEDS · OVER-DETERMINED
Residue — continuous + function-valued + discrete forks; quotients ranked
```

The ledger is versioned append-only. A new target appends a row and increments
the minor version. A changed meaning creates a successor and migration edge.
Prior verdicts are never silently overwritten. The freshness gate is red when
a Build or Compose result touches a row but neither advances the ledger nor
records an explicit no-change reason.

## Active scientific directive: dynamic cosmological sector

Directive `GU-COSMO-DYNAMIC-01` is binding before the next scientific wave
...
3. a literal constant `Lambda g` term;
4. the olive/`varpi`/augmented-torsion distortion and its claimed variable VEV;
5. the coupling or co-variation with Einstein curvature;
6. VEV selection, sign, magnitude and normalization; and
7. the effective cosmological observable, including any `w(z)` claim.
...
massive GR-3 partner. The written connection-difference norm is now composed
with the correctly typed horizontal Gauss block. Its receiver has rank 100,
its insertion is the exact action-adjoint right inverse and the action
restricts to full `|II|^2`; tracing first has rank ten and loses 90 traceless
directions. The Gauss-induced direct Einstein term therefore changes the TT determinant exactly to
`z*(alpha_II*kappa_1-z)`: one simple massless Einstein pole plus one distinct
massive GU partner. The repair target is to physically classify that split,
not reflexively erase the second pole. `P2_norm` is derived on this canonical
sector; the homonymous external `P2_datum` remains unchanged. The latest
source return is `SOURCE-CONFIRMS-INGREDIENTS__REPO-DERIVES-COMPOSITION`.

The selected non-cyclic moving-K77 scalar action is computed on its
Spin-invariant constant branch. Including the one-third path-average eddy,
`I(t)=1456 t^3+7 kappa_1 t^2` and the nonzero algebraic stationary point is
`t=-kappa_1/312`. The radial `Cl1` Hessian is `-14 kappa_1`, but it is not the
gravitational Hessian. Direct restriction of the same action to the distinct
`Cl2` Gauss carrier gives exact trace/traceless coefficients
`100*kappa_1/117` and `124*kappa_1/117`. Its native inertia is `(54,46)`, so
the sector is nondegenerate but not positive.

Keeping metric and distortion coupled gives a second-order operator whose
kinetic matrix has determinant `-1`. On the inherited globally hyperbolic
observed defect, harmonic gauge therefore supplies a common normally-
hyperbolic Krein/Green domain. The partner mass is
`124*alpha_II*kappa_1/117`, and its residue is opposite the Einstein pole.
That is a positive-Hilbert ghost warning and a precise native Krein/BV
physical-cohomology gate; Green hyperbolicity does not decide it.

At the selected stationary branch, the complete even Ward owner linearizes to
`R^!H=0=HR`. The action-derived current/stress chain is
`T_reduced=E_g^direct+(D_g A)^!J_A`, not a current-only algebraic equality or
literal diagonal `VU`. The Gauss adjoint closes the `II` sector, while the
full metric/coframe derivative of the unified soldered connection remains
open. The latest composed-locus return is `SOURCE-CORRECTS`: Weinstein's
...
isolation. The intrinsic lower-order homogeneous orbit is subsequently exact
...
vacuum-selection rule and observable behavior. The historical Einstein
“fine marble/cheap wood” characterization is useful motivation but remains
...
2. Preserve the raw rank-four residual orbit, but retract v0.43's owner
   inference: the background-subtracted Hessian is a difference of Gram forms,
...
five-state spin-two type. Its immediate owner is the actual source-owned
prolongation and the complete raw `D Upsilon` on all four graph-orbit
directions. Only if those columns close naturality and match the conditional
full-`II` map should it build transverse source directions. Only
...
This reset changes GU's owner-local operating method. It does not change the
fleet scheduler, triggers, activation grants, Lane count, canon, external
P1/P2/P3 or public posture. It does not itself establish a complete source
action, external datum, Einstein equation or dynamical dark-energy solution.
The owner contract is mandatory and machine-tested for agents that load GU's
declared context. Whether every fleet runner interprets the new
`channel_contract_refs` directly is a separate System integration question and
is not claimed here.
