...
The result-hardening research convergent-hardening run type. Its job is to turn already-stable finite theorems into
machine-checked Lean, advancing `lab/process/lean-verification-lane-LEDGER.md` when the daily portfolio
selects the relevant result-hardening research work item.

The hourly default is `meaningful-hourly-progress-swing.md`. Lean is a method inside result-hardening research, not a lane
or an automatic priority. Its finishability must not displace truth-status research truth testing merely because it has
a monotone success condition.

## The investigation loop

1. **Confirm portfolio selection.** The investigation receipt states why result-hardening research's strongest eligible item won the
   cross-agenda ranking without demoting truth-status research's protected purpose. Then follow the
...
4. **Typecheck through the serialized guard:** `./lab/automation/check-lean.ps1`. The helper acquires
   ...
   If it does not, fix it within the investigation or leave it LEAN-PARTIAL with the exact blocker. Do not commit
