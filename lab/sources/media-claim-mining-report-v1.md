...
This starter pass mined sources that already had captured material in `lab/archive/execution-log.md` and the prior self-source capture notes summarized there. The mining did not invent claims; it converted captured source-native wording into the claim-ledger row format and added one row family from the Oxford host-institution event page and one from Weinstein's author site.
...
| --- | --- | --- |
| 1. `lab/sources/claim-ledger.md` has 20-30 timestamped claim rows | partial | 16 rows drafted in `claim-ledger-populated-v1.md` (local draft, not yet promoted to public repo). 20-30 row target requires the next-batch JRE pass. |
