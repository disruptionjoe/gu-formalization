---
title: "One-time triage: under-explored lanes, source-action-independent movable work, and the 13 objections"
status: roadmap
doc_type: roadmap
created: 2026-06-30
...
This is a rating pass, not a proof pass. Each lane was scored on **Novelty / Publishability / Profundity (0-5)**, given a **decisive first test**, and assigned **PURSUE / PARK / KILL**.

## Calibration note (read first)

Scores were re-normalized to one absolute rubric:

- **5** = new structure that reframes a field / top-journal standalone
- **4** = solid new GU-independent theorem, real-journal grade
- **3** = referee-checkable note or lemma with genuine new content (clean arXiv-note grade)
- **2** = expository / negative / dictionary note; restates known facts in new framing
- **1** = appendix lemma / internal honesty fix; textbook content applied to the repo
- **0** = internal process bookkeeping, no external value

**The single most important fact: nothing in this set scores above 3 on any axis.** The whole program is reconstruction/note-grade. A "3" here is a clean arXiv note, not a result that moves a field. Composite = N + P + Prof; it measures *promise of content*, not *remaining work*.

`src` = the lane's decisive gate routes through the unbuilt RS/IG **source action** (Y / N / partial).
...
| 6 | MOVE-2 FLRW/DESI | 1 | 2 | 2 | 5 | N | Real integrator built; cheap closure = fix stale canon (currently backwards on sign) + run the unrun M_KK/f0 robustness box; must state sign-only, magnitude-discordant, NOT a DESI "match." |
| 7 | MOVE-6 "Higgs is an illusion" | 2 | 1 | 2 | 5 | partial | ‖F‖² block expansion; source-action-free part is textbook Connes-Chamseddine; pursue for delimiting/falsification value (does the (1,2,½) quartic vanish?); Mexican-hat sign is source-action-gated. |
| 8 | MOVE-4 Majorana channel dim | 1 | 1 | 1 | 3 | N | One-pass retarget of verified D₇ machinery to the classical S⊗S Fierz grading, checksum 2¹⁴; textbook, but a legit honesty-hardening of SHIAB-04. |
...
| 25 | OBJ-CMPR C_MPR/BvN wall | 1 | 1 | 1 | 3 | partial | Genuine naming exercise (undefined C_GW codomain); only the demotion is earned and should be propagated now; only the upgrade is source-action-gated. |
| 26 | OBJ-DEFLATE | 0 | 0 | 1 | 1 | N | Internal false-negative audit; spot-check shows recent downgrades warranted; weakest surviving lane, KILL-adjacent, run reactively only. |

### KILL

| # | id | N | P | Pr | comp | src | one-line |
|---|----|---|---|----|------|-----|----------|
| 27 | OBJ-TAF time-as-finality | 2 | 2 | 2 | 6 | Y | Flipped PARK→KILL: after 55 files, only computed nulls; every object reduces to Čech / rate-independent restatement / record-audit; the null note IS the kill artifact. High composite but content exhausted — do not re-open on a source-action trigger the lane doesn't own. |
| 28 | OBJ-CONVERGE loop QA | 0 | 1 | 1 | 2 | N | 2 same-day log entries + rising issue counts already falsify "converging" (thrashing/stalled). KILL as a research lane — but the thrashing read MUST spawn a loop-redesign/halt ops action, not vanish. |
...
1. **MOVE-1** — Sp(1)-vs-Sp(64) octic recheck. One-pass, existing verified machinery; the repo's own commutant note predicts it flips the non-factorizability red flag to an artifact. Best ROI in the set.
2. **MOVE-3** — first variation of ∫‖II‖² on Schwarzschild. In-repo, falsifiable (O(M/r³)=falsify vs O(M²)=safe); verifier already showed the blocker is gone and both canon notes wrong. Scope to the single EL-variation contraction.
3. **OBJ-NGUYEN** — fetch verbatim Nguyen §3.1 PDF + write the deflated existence/non-uniqueness note. De-risks the entire lane; scrub retracted "dissolves" language.
4. **MOVE-5** — extract the Krein no-go as a standalone lemma + chiral-import falsification. Single most referee-survivable GU-independent artifact; falsification already confirming in the sibling sandbox.
5. **MOVE-2** — fix the backwards dark-energy canon + run the unrun M_KK/f0 robustness box; cap at sign-only. Also resolves the MOVE-2/OBJ-DESI split.

Honorable mention: MOVE-4 is cheapest of all but lowest gain — do it opportunistically.

## 4. Clear KILLs

- **OBJ-CONVERGE (high confidence).** Loop is demonstrably thrashing; no math/physics artifact. Convert the finding into a loop-redesign/halt ops action — don't let it vanish.
- **OBJ-TAF (medium confidence — verifier flip from PARK).** 55 files of computed nulls; kill condition met; close with the null note as the kill artifact; do NOT park on a source-action trigger it doesn't own.
- **OBJ-DEFLATE (borderline).** Pure meta-hygiene; run reactively only.
