...
**Specialist update (2026-06-23).** The formal real-even sign package itself is not the immediate obstruction. The open problem is whether the KO-6 package controls finite SM chirality strongly enough in a Type II_1 or semifinite setting to act as a finite-control selector.

**GU-contact update (2026-06-23).** Ordinary GU section pullback does not provide the needed KO-6 sign package: `s*(J_GU)^2 = -1`, while the finite Connes control case requires `J^2 = +1`. This pushes any positive GU/Type-II_1 contact toward a twisted or newly specified real structure.
...
**Specialist update (2026-06-23).** Option (a) is now the only defensible positive reading from the first specialist pass: principal graphs fail as a full SM representation source and remain conditionally viable only as generation-count selectors.

**GU-contact update (2026-06-23).** GU inner fluctuations are not a substitute for the missing subfactor/finite-algebra selector. They preserve an input `Sp(64)` connection orbit and do not recover the SM gauge group or finite Connes one-form bimodule.
