#!/usr/bin/env python3
"""Fail-closed scope audit for the I2 resumable second-size-six certificate."""
...
REPORT = ROOT / "explorations/pw2fr2b2b2i2-resumable-second-size6-full-evaluator-certificate-2026-08-05.md"
REVIEW = ROOT / "lab/process/hostile-reviews/2026-08-05-pw2fr2b2b2i2-resumable-second-size6-review.md"
PROBE = ROOT / "tests/channel-swings/pw2fr2b2b2i2_resumable_second_size6_full_evaluator_probe.py"
...
def main() -> None:
    ...
    assert data["gate"] == "PW2F-R2B2B2I2-RESUMABLE-SECOND-UNCOVERED-SIZE6-FULL-EVALUATOR"
    assert data["status"] == "SECOND_UNCOVERED_SIZE6_CERTIFIED__DURABLE_5_OF_380__UNIVERSAL_375_OPEN__DENSE_HELDOUTS_0_OF_6_EXECUTED"
    assert data["run_id"] == "RUN-20260805-104942-gu-formalization-pw2fr2b2b2i2-resumable-second-size6"
    assert data["resumption_contract"]["next_resume_index"] == 5
    assert data["resumption_contract"]["shard_unit"] == "ONE_COMPLETE_CANONICAL_S3_ORBIT"
    assert data["coverage"] == {
        "joint_labels": 1925,
        "canonical_representatives": 380,
        "durable_representatives_predecessor": 4,
        "new_size_six_representatives_certified": 1,
        "durable_representatives_certified": 5,
        "remaining_representatives": 375,
        "representative_coverage": "5/380",
    ...
    assert entries[0]["coverage_index"] == 4
    ...
    entry = entries[1]
    assert entry["coverage_index"] == 5
    assert entry["representative"] == {
        "owner_pair": [0, 0],
        "quartic_point": [0, 0, 2, 2],
    }
    assert entry["orbit_size"] == 6
    assert entry["predecessor_ledger_revision"] == "40e93a9fa3a0e091b81be01fbbc73fd24608b0d8"
    assert entry["predecessor_ledger_sha256"] == "49f9ad8c7f66812910cb6b700b6982b03ed44cbec65e4a04ec1770d2b0766fdd"
    ...
    assert checks["mixed_action_fingerprint"] == [
        "-31/144", "-697/144", "-31/144", "-409/144", "871/72", "1015/72",
    ]
    ...
    assert data["admission"]["remaining_full_evaluator_transport"] == "OPEN_375_OF_380_REPRESENTATIVES"
    ...
    assert data["next_gate"].endswith("REMAINING-375-REPRESENTATIVE-FULL-EVALUATOR-COVERAGE-PLUS-EXECUTE-DENSE-HELDOUTS-THEN-SEPARATE-C4-BANKS")
    ...
    for checkpoint in (campaign["latest_successor_checkpoint"], council["latest_successor_checkpoint"]):
        ...
        assert "5/380" in checkpoint["scope"]
        assert "375" in checkpoint["scope"]
        assert "heldout" in checkpoint["scope"]
        assert "1,925-cell fallback remains live" in checkpoint["scope"]

    for token in (
        "5/380", "other `375`", "0/6` executed", "1,925-cell fallback stays live",
        "-31/144", "1015/72", "P1/P2/P3",
        "FORMALLY_SEPARATE_INSIDE_ERIC_LANE", "TG-1 AND TG-2 AND TG-3",
    ):
        assert token in report or token in review, token

    for token in (
        "representative_coverage=5/380", "remaining_representatives=375/380",
    ...
    print("pw2fr2b2b2i2_resumable_second_size6_scope_audit: PASS")
    print("  append-only 5/380 coverage, heldouts 0/6, remaining 375, live fallback, separate banks, Curt, and TG boundary retained")
