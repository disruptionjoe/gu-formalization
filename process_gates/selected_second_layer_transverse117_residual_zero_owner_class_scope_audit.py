...
ledger = strict("lab/process/conditional-physics-ledger-v0.52.json")
...
assert ledger["schema_version"] == "0.52"
...
assert "raw-Upsilon normal first jet" in ledger["next_work_queue"][0]["why"]

assert contract["standing_ledger"]["ref"] == "lab/process/conditional-physics-ledger-v0.52.json"
directive = contract["active_scientific_directives"][0]
assert "CONNECTION_CLASS_KILLED_AT_RESIDUAL_ZERO" in directive["status"]
...
assert "CURRENT TRANSVERSE-117 OWNER-CLASS FENCE" in context
assert "TRANSVERSE-117 RESIDUAL-ZERO OWNER RETYPE" in next_steps
assert "ledger v0.52" in status
...
print("PASS: residual-zero connection/operator class is scoped away from transverse 117, nonzero-background and normal-jet routes remain open, and ledger/contract wiring forbids action, quotient, datum or posture inflation")
