...
assert contract["standing_ledger"]["ref"] == "lab/process/conditional-physics-ledger-v0.51.json"
directive = contract["active_scientific_directives"][0]
assert "FIXED_B_DB_T_SUPPORT28_PARTIAL_OWNER_EXACT" in directive["status"]
assert directive["next_run_method"]["target"] == "MOVING_REFERENCE_EPSILON_SOLDERING_OWNER_FOR_TRANSVERSE_117_THEN_NULL_SCREEN"
...
for token in ("CURRENT TRANSLATION-CURVATURE PARTIAL-OWNER FENCE", "Ledger v0.51"):
    assert token in context
assert "FIXED-B TRANSLATION-CURVATURE PARTIAL OWNER" in next_steps
assert "ledger v0.51" in status
assert "conditional-physics-ledger-v0.51.json" in lanes
...
print("PASS: fixed-B D_B T owns exactly support 28, support 117 stays moving-reference work, and ledger/contract wiring forbids Euler, quotient, datum, canon or posture inflation")
