...
for relative in ("LANES.yaml", "NEXT-STEPS.md", "RESEARCH-STATUS.md",
    ...
    check(f"current pointer {relative}", "v0.115" in read(relative))
check("contract pointer", contract["standing_ledger"]["ref"].endswith("v0.115.json"))
check("contract gate", contract["active_scientific_directives"][0]["next_gate"]
      == registry["next_gate"])
check("inventory", "(488 Python + 77 Sage)" in read("tests/README.md"))
