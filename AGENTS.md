...
When a run is routed through CapacityOS System stewardship, the System-owned overlay is `CapacityOS/system/stewards/gu-formalization.md`. Treat it as routing context, not as a replacement for this file or GU local steward context.
...
- Canon promotion is agent-owned. When an exploration clears the `RESEARCH-STATUS.md` Promotion Rule you may promote it into `canon/` / `CANON.md` yourself - this no longer pauses for Joe. Every executed promotion MUST drop an awareness note in `CapacityOS/system/mailboxes/joeops/` using `lab/process/templates/canon-promotion-joeops-notice.md`. The note is awareness-only, not an approval gate; the promotion is already done. Canon = public-spine framing, not a verdict.
- Cross-repo actions are not executed directly and no longer pause for Joe: drop a proposal note in the target surface's mailbox (`CapacityOS/system/mailboxes/<surface>/`) and let that surface's steward decide whether to act. Writing the proposal note is itself allowed and is not a cross-repo action.
