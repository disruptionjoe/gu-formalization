#!/usr/bin/env python3
"""Regression checks for the RS/BRST carrier packet.

Run: python tests/test_rs_brst_packet.py
"""
...
def rs_brst_candidate() -> lc.SourceCandidate:
    return lc.SourceCandidate(
        name="rs-brst-security-budget",
        description=(
            "Attempts to use a GU-internal full BV bicomplex / RS-BRST "
            "closure loss as the candidate-specific security-budget carrier."
        ),
        assumptions=(
            "No target normalization is used.",
            "The bare commutator anchor is preserved.",
            "BRST vocabulary is not treated as a source action by itself.",
            "Nilpotency or closure must be non-vacuous and candidate-coupled.",
            "Clean RS-sector decoupling is rejected as the acausal trap.",
        ),
    )


class RsBrstPacketTests(unittest.TestCase):
    def test_computable_guards_remain_clean(self):
        candidate = rs_brst_candidate()
    ...
    def test_rs_brst_channel_is_the_named_blocker(self):
        candidate = rs_brst_candidate()

        with self.assertRaises(lc.MissingCarrierError) as raised:
            lc.l_rs_brst(candidate)
