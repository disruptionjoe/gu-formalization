---
title: "Nielsen-Ninomiya as a Distributed-Systems Protocol Contract — Pilot Map"
...
**Source basis.** `literature/04-spectral-triples-anomaly-chirality-distributed-systems-analogies.md`, `process/syntheses/00e-problem-shape-distributed-systems-meta-synthesis.md`, `roadmap/15-perspective-pathway-ranking.md` (pathway E ranks 6/16 on the 15-perspective panel; "best heterodox operational bridge"). Cross-built against the six-axis protocol (internal origin artifact) and the no-go-forgetful image map (internal origin artifact).
...
This matches internal origin artifact's cross-artifact finding exactly: assumption (5) is the cleanest exit, and the protocol-side analog is modified-consistency-model.
...
In the no-go-map's language (internal origin artifact), Nielsen-Ninomiya's forgetful operation is:
...
5. **Coordination-list propagation.** Update internal origin artifact's cross-artifact finding for #27 with the verdict that the protocol-analogy clears the "more than metaphor" bar specifically on assumption (5), and that the CALM-monotonicity correspondence is the proposed theorem-shape.

## 9. Six-axis position

In the internal origin artifact six-axis protocol vocabulary, this candidate is the first to populate L3 with class (g) "FLP / CAP / BFT protocol-class pairing." The full sextuple is:
...
- **internal origin artifact (sibling #24) — six-axis specification protocol.** This candidate is the first L3 = (g) instance and validates that the L3 axis is independently populatable. Finding to propagate: L3 = (g) is exercisable as a standalone axis-drop, not only as a refinement of L2/L3 in combination. This may warrant a coordination-pass note in internal origin artifact's README about L3 = (g) being independently meaningful.
- **internal origin artifact (sibling #25) — no-go-forgetful-image map.** This pilot directly confirms internal origin artifact's cross-artifact prediction for #27: "assumption (5) — exact on-site chiral symmetry — is the protocol-side analog of GW/overlap, and modified-consistency-model is the expected cleanest evasion." This pilot operationalizes that prediction with the CALM correspondence and identifies the same forgetful functor in two vocabularies.
- **internal origin artifact / Cartan-G_2 guardrail (sibling #32).** Not engaged in this pilot. The Cartan-G_2 route is an L1 substrate-side proposal; this pilot's L1 is the standard static lattice. A second-pass cross-product (Cartan-G_2 L1 × protocol-class L3) is conceivable but is not this pilot's job.
- **internal origin artifact / Type II_1 spectral SM checklist (sibling #26).** Not directly engaged. The Type II_1 candidate is an L1 substrate move that holds L3 to Cartesian/Connes-channel pairing; this pilot is the orthogonal axis-drop. They could be combined (Type II_1 substrate paired by FLP/CAP/BFT protocol-class) but that is a sextuple change, not a pilot.
